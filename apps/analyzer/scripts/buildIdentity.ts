...
const GENERATED_PATH = path.join(
  ...
  "analysis",
  "src",
  "privateAnalyzerReleaseV1.generated.ts",
);
...
export function normalizeImplementationIdentitySlot(bundle: Uint8Array): Uint8Array {
  ...
  const identityStarts: number[] = [];
  let searchFrom = 0;
  while (searchFrom < text.length) {
    const markerStart = text.indexOf(PRIVATE_ANALYZER_IMPLEMENTATION_MARKER_PREFIX, searchFrom);
    ...
    if (
      ...
      text.slice(suffixStart, suffixStart + PRIVATE_ANALYZER_IMPLEMENTATION_MARKER_SUFFIX.length) ===
        PRIVATE_ANALYZER_IMPLEMENTATION_MARKER_SUFFIX
    ) identityStarts.push(identityStart);
    searchFrom = markerStart + PRIVATE_ANALYZER_IMPLEMENTATION_MARKER_PREFIX.length;
  }
  if (identityStarts.length === 0) {
    throw new Error("Private analyzer implementation identity slot is missing.");
  }
