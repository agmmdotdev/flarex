...
5. **Only the first private analyzer-to-Postgres mutation slice is assembled.**
   ...
   activation, an active reader, a public API, or production routing;
    ...
    work.
6. **The second private vertical begins with read authority, not query
   execution.** Roadmap 44 separates `PQV-A1`, `PQV-A2`, and `SAP05`. Completed PQV-A1
   derives one Scope-revoked capability from the coherent FSV05 selection,
   pins the authoritative target-native commit sequence, revalidates the
   generation/fence/epoch, active head, and inclusive retained-history floor
   before every bounded point-document read, and delegates to the existing
   ...
   and query-only syscall ABI; completed SAP05 owns the private System/Standard invocation
   and returns only the validated query value. No
...
and R2 artifact authority. Completed SAP05 exposes
`invokeApplicationPointQueryV1` through a thin Standard consumer and genuine
route-independent Workerd composition. None of these
...
introduced. This makes one first-slice difference from Convex explicit: if a
parent catches an application-owned child failure, writes already settled by
