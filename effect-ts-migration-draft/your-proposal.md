...
- Concrete execution checklist: use `roadmaps/22-effect-migration-checklist.md`
  as the tick-box plan for the remaining migration. Each Effect migration turn
  should tick only completed checklist items and update the next active
  checkpoint there.
...
1. Audited scheduler public/internal route body contracts and backend
   compatibility error surfaces.
2. Added protocol-owned scheduler request schemas and Effect decoders.
3. Kept backend route adapters as the JSON read, env fallback, and HTTP
   response mapping edges.
4. Validated protocol plus scheduler focused backend tests before ticking
   C-1b.
...
2. Convert the owned request body validators to reusable Effect decoders with
   tagged validation failures.
3. Keep every current bad request message and route response body unchanged.
4. Preserve executor-http behavior through focused typed decoder tests and
   package gates before ticking E-2.

Completed Goal 359 slice:
...
2. Replace owned bridge casts/parsing with typed Effect decoder functions.
3. Keep scheduler wake, delivery reconnect, and connection route response
   behavior unchanged.
4. Preserve behavior through the S-3 focused tests before ticking S-3.

Completed Goal 357 slice:
...
1. Add `dispatchDeploymentApiMutationRouteInputDirect(...)`, a named Effect
   ...
   `DeploymentServiceApi`, calls the existing generated handler effects, and
   maps their success/error values to HTTP `Response` objects at one adapter
   edge.
2. Preserve generated HttpApi status semantics in the direct adapter: ordinary
   mutation success is `200`, rejected finish responses are `409`, declared
   bad-request/not-found/conflict/storage responses map to `400/404/409/500`.
3. Add focused tests for direct start success, rejected finish status mapping,
   and abandon conflict mapping.
4. Leave `routeDeploymentDurableObject(...)` wiring on the compatibility
   dispatch path until direct read dispatch or split mutation/read routing is
   chosen explicitly.
...
Completed Goal 335 slice:

1. Route `routeDeploymentDurableObject(...)` through
   `decodeDeploymentApiRouteInput(...)` instead of hiding typed route input
   behind the request compatibility decoder.
2. Add `dispatchDeploymentApiRouteInputViaRequestCompatibility(...)`, a named
   Effect adapter that converts typed route input into the generated HttpApi
   `Request` compatibility path and maps generated handler failures to typed
   internal route failures.
3. Add focused tests for the compatibility adapter's success path, generated
   protocol failure propagation, and generated handler operation failure
   mapping.
4. Leave direct generated handler dispatch, generated handler logic,
...
1. Add `deploymentStartAnalyzedPushResponseForHttpApi(...)`, a named Effect
   ...
   `DeploymentService.startAnalyzedPush(...)`, maps typed service failures, and
   validates the declared push-status protocol response.
2. Keep `deploymentStartAnalyzedPushHandler(...)` responsible for composing the
   existing generated-handler input guard with the new service-response
   adapter, mapping input validation failures to declared HttpApi bad-request
   responses before the service call.
3. Add direct adapter tests for start success, validation failure, storage
   failure, and malformed service response mapping.
4. Leave DeploymentDO route decoding, public Worker deployment dispatch,
   DeploymentService/store lifecycle logic, artifact materializer/cache,
   source-package analyzer semantics, PartitionDO SQL/OCC, executor-http,
   protocol parser compatibility wrappers, and `ValidatorJson` unchanged.

Completed Goal 332 slice:
...
1. Add `deploymentStartPushApplicationPlan(...)`, a named
   ...
   start inputs into the exact row values written by the start transaction.
2. Add `deploymentAbandonPushApplicationPlan(...)`, a named Effect helper that
   converts prevalidated abandon input into the exact update values written by
   the abandon transaction.
3. Route `DeploymentPushStore.startAnalyzedPush(...)` and
   `DeploymentPushStore.abandonPush(...)` through those plans before entering
   the Durable Object transaction callback.
4. Add direct helper coverage for analyzed start rows, failed start rows, and
   abandon update values.
5. Leave DeploymentDO routing, generated DeploymentApi response mapping,
   ...
   active metadata parsing, artifact store implementation, PartitionDO
...
1. Add `deploymentSchemaApplicationPlan(...)`, a named DeploymentPushStore
   Effect helper that converts validated deployment schema metadata into the
   exact table/index SQL application rows used by finish activation.
2. Add `deploymentFunctionsApplicationPlan(...)`, a named Effect helper that
   converts validated function metadata into the exact function SQL application
   rows used by finish activation.
3. Add `finishPushActivationApplication(...)`, a named Effect helper that
   combines schema and function application plans before the Durable Object
   transaction begins.
4. Route `DeploymentPushStore.finishPush(...)` through the activation
   application plan, then keep the transaction body focused on applying the
   prebuilt plan and preserving existing SQL writes.
5. Add direct helper coverage for schema plan rows, function plan rows, and the
   combined finish activation application plan.
...
1. Remove the throwing backend `parsePublicInvokePayload(...)` compatibility
   wrapper from `invoke/Requests.ts`.
2. Remove the backend import of protocol
   `parsePublicInvokeRequestBody(...)`; the protocol package keeps its own
   parser as a protocol compatibility surface.
3. Keep public Worker invoke routing on `decodePublicInvokeRouteRequest(...)`,
   `publicInvokeDeploymentIdEffect(...)`, and
   `invokeRequestFromPublicInvokeBodyEffect(...)`.
4. Update public invoke request tests to assert decoder success and typed
   `InvokeProtocolValidationError` failures directly.
5. Leave public invoke Worker routing, artifact runtime dispatch, direct invoke
   execution, active deployment loading, PartitionDO SQL/OCC, executor-http,
   protocol package parser compatibility, and `ValidatorJson` unchanged.

Completed Goal 316 slice:

1. Remove the legacy `registry/HttpBoundary.ts` adapter module, including
   `registryFailureToHttpError(...)`.
2. Remove the matching legacy `registryHttpBoundary.test.ts` compatibility
   test.
3. Keep generated Registry HttpApi handler failures on typed response mappers:
   `mapRegistryStorageFailure(...)` and
   `mapRegistryProtocolResponseFailure(...)`.
4. Leave RegistryService, RegistryStore, RegistryDO routing, generated
   Registry HttpApi web handler behavior, DeploymentDO, PartitionDO SQL/OCC,
   executor-http, and `ValidatorJson` unchanged.

Completed Goal 315 slice:

1. Remove the legacy `deployment/HttpBoundary.ts` adapter module, including
   `deploymentFailureToHttpError(...)` and `finishPushHttpStatus(...)`.
2. Remove the matching legacy `deploymentHttpBoundary.test.ts` compatibility
   test.
3. Remove `deploymentHttpErrorToReadResponse(...)`,
   `deploymentHttpErrorToStartResponse(...)`,
   `deploymentHttpErrorToFinishResponse(...)`, and
   `deploymentHttpErrorToAbandonResponse(...)` from
   `deployment/HttpApiHandlers.ts`.
4. Keep generated Deployment HttpApi handler failures on typed response
   mappers: `deploymentReadFailureToResponse(...)`,
   `deploymentStartFailureToResponse(...)`,
   `deploymentFinishFailureToResponse(...)`,
   `deploymentAbandonFailureToResponse(...)`, and their named Effect adapter
   wrappers.
5. Leave DeploymentService, DeploymentPushStore, DeploymentDO routing, public
...
1. Remove throwing execution request payload compatibility wrappers from
   `execution/Requests.ts` for internal start, public start, syscall, finish,
   and public action payloads.
2. Remove the matching execution protocol parser imports from
   `execution/Requests.ts`.
3. Keep internal ExecutionDO and public Worker execution route boundaries on
   `decodeExecution*Payload(...)` and `decodePublicExecution*Payload(...)`
   Effect decoders.
4. Update execution request tests to assert decoder success and typed
   `ExecutionProtocolValidationError` failures directly, including the
   variants previously covered through the throwing wrappers.
5. Leave ExecutionDO routing/session lifecycle, public Worker execution
   dispatch, syscall/finish semantics, direct invoke, artifact runtime
   dispatch, PartitionDO SQL/OCC, executor-http, and `ValidatorJson`
   unchanged.

Completed Goal 312 slice:

1. Remove throwing deployment push request payload compatibility wrappers from
...
2. Remove the matching protocol parser imports from `deployment/Requests.ts`.
3. Keep production DeploymentDO and public Worker push boundaries on
   `decodeDeployment*Payload(...)` and `decodePublic*Payload(...)` Effect
   decoders.
4. Update deployment request tests to assert decoder success and typed
   `DeploymentProtocolValidationError` failures directly.
5. Leave DeploymentDO routing, HttpApi handler behavior, DeploymentService,
   DeploymentPushStore, artifact storage/materialization, public Worker
   dispatch, executor-http, PartitionDO SQL/OCC, and `ValidatorJson`
   unchanged.
...
1. Remove the Promise-returning `readConnectionInvalidationRequest(...)` and
   `readConnectionLiveQueryDeliveryRequest(...)` wrappers from
   `connection/RouteBoundary.ts`.
2. Remove public throwing route-level
   `parseConnectionInvalidationRequest(...)` and
   `parseConnectionLiveQueryDeliveryRequest(...)` compatibility wrappers.
3. Remove public `parseConnectionInvalidationRequestEffect(...)` and
   `parseConnectionLiveQueryDeliveryRequestEffect(...)` aliases that only
   forwarded to route payload decoders.
4. Remove the throwing `parseConnectionInvalidationPayload(...)`
   compatibility wrapper from `connection/Requests.ts`.
5. Keep `ConnectionDO` internal JSON routing on
   `decodeConnectionInvalidationRequest(...)` and
   `decodeConnectionLiveQueryDeliveryRequest(...)`, with dispatch still on
   `dispatchConnectionInvalidationEffect(...)` and
   `dispatchConnectionLiveQueryDeliveryEffect(...)`.
6. Update connection request and route-boundary tests to assert typed
   `RequestJsonError`, `ConnectionRouteValidationError`, and
   `LiveQueryDeliveryChangePayloadError` channels directly, then separately
   assert existing adapter HTTP mapping.
7. Leave websocket sync semantics, query rerun behavior, executor
   subscription writes, live-query delivery fanout, public Worker dispatch,
   DeliveryDO, SchedulerDO, PartitionDO SQL/OCC, executor-http, and
   `ValidatorJson` unchanged.

Completed Goal 309 slice:

1. Remove the Promise-returning `readExecutionArtifactInvokePayload(...)`
   wrapper from `artifactRuntime/RouteBoundary.ts`.
2. Remove public throwing `parseExecutionArtifactInvokePayload(...)` and
   `parseExecutionArtifactInvokePayloadEffect(...)` compatibility wrappers
   from the same route boundary.
3. Remove the throwing `parseExecutionArtifactInvokePayloadBody(...)`
   compatibility wrapper from `artifactRuntime/Requests.ts`.
4. Keep production artifact runtime invoke routing on
   `decodeExecutionArtifactInvokePayload(...)` and keep payload validation on
   `decodeExecutionArtifactInvokePayloadBody(...)` /
   `decodeExecutionArtifactInvokeRoutePayload(...)` Effect decoders.
5. Update artifact runtime request and route-boundary tests to assert typed
   `RequestJsonError` and `ExecutionArtifactInvokePayloadError` channels
   directly, then separately assert existing adapter HTTP mapping.
6. Leave runtime request authorization, artifact header validation,
   source-package resolution, materializer/cache behavior, artifact invoke
   execution, deployment artifact behavior, public Worker dispatch,
   executor-http, PartitionDO SQL/OCC, and `ValidatorJson` unchanged.

Completed Goal 308 slice:

1. Remove the Promise-returning `readExecutionStartRequest(...)`,
   `readExecutionSyscallRequest(...)`, and `readExecutionFinishRequest(...)`
   wrappers from the internal execution route boundaries.
2. Remove public throwing `parseExecutionStartRouteRequest(...)`,
   `parseExecutionSyscallRouteRequest(...)`, and
   `parseExecutionFinishRouteRequest(...)` compatibility wrappers.
3. Remove public `parseExecution*RouteRequestEffect(...)` aliases that only
   forwarded to the route payload decoders.
4. Keep `executionDO.ts` on `decodeExecutionStartRouteRequest(...)`,
   `decodeExecutionSyscallRouteRequest(...)`, and
   `decodeExecutionFinishRouteRequest(...)`.
5. Keep public execution start decoding on
   `decodePublicExecutionStartRouteRequest(...)` /
   `decodePublicExecutionStartRoutePayload(...)`.
6. Update internal execution route-boundary tests to assert typed
   `RequestJsonError` and `ExecutionProtocolValidationError` channels
   directly, then separately assert existing adapter HTTP mapping.
7. Leave ExecutionDO session routing/runtime behavior, syscall handling,
   finish semantics, public execution dispatch, PartitionDO SQL/OCC,
   executor-http, deployment behavior, and `ValidatorJson` unchanged.

Completed Goal 307 slice:

1. Remove the Promise-returning `deploymentApiRequestForRoute(...)` wrapper
   from `deployment/HttpApiRouteBoundary.ts`.
2. Remove Promise-returning deployment mutation request wrappers:
   `readDeploymentAnalyzedStartPushRouteRequest(...)`,
   `readDeploymentFinishPushRouteRequest(...)`, and
   `readDeploymentAbandonPushRouteRequest(...)`.
3. Remove public throwing `parseDeployment*RouteRequest(...)` and
   `parseDeployment*RouteRequestEffect(...)` compatibility wrappers from the
   same route boundary.
4. Keep `routeDeploymentDurableObject(...)` on
   `decodeDeploymentApiRequestForRoute(...)` and keep route payload validation
   on `decodeDeployment*RouteRequest(...)` /
   `decodeDeployment*RoutePayload(...)` Effect decoders.
5. Update deployment HttpApi route-boundary tests to assert typed
   `RequestJsonError` and `DeploymentProtocolValidationError` channels
   directly, with HTTP response mapping still covered through
   `deploymentRouteErrorToHttpErrorEffect(...)` and
   `deploymentInternalRouteErrorToResponseEffect(...)`.
6. Leave DeploymentDO routing semantics, generated HttpApi handler behavior,
   deployment store/service behavior, artifact materialization/ref validation,
   public deployment dispatch, executor-http, PartitionDO SQL/OCC, and
   `ValidatorJson` unchanged.

Completed Goal 306 slice:

1. Remove exported throwing deployment validation compatibility wrappers from
   `deployment/Validation.ts`: `validateSourcePackage(...)`,
   `validateDiagnostics(...)`, `analyzedStartPushRequest(...)`,
   `startAnalyzedPushInput(...)`, `pushStatusFromRow(...)`,
   `validateSchema(...)`, `validateFunctions(...)`,
   `validateAnalysis(...)`, and `validateCodegenAnalysis(...)`.
2. Remove the exported `DeploymentValidationResult` /
   `parsePushStatusFromRow(...)` result compatibility boundary from
   `deployment/Validation.ts`.
3. Keep production deployment store and HttpApi handler paths on
   `decodePushStatusFromRow(...)`, `decodeAnalyzedStartPushRequest(...)`, and
   `decodeStartAnalyzedPushInput(...)` Effect decoders.
4. Keep `codegenAnalysisFromDeploymentAnalysis(...)` as a pure domain
   transformation helper because it does not perform validation or HTTP
   mapping.
5. Update deployment validation tests so transaction-preflight result matching
   is asserted through `decodePushStatusFromRow(...).pipe(Effect.match(...))`;
   remaining sync helpers are local test fixtures backed by the Effect
   decoders, not exported production compatibility APIs.
6. Leave deployment store transaction behavior, DeploymentDO/HttpApi routing,
   artifact materialization/ref validation, public deployment dispatch,
   executor-http, PartitionDO SQL/OCC, source-package persistence behavior,
   and `ValidatorJson` unchanged.
...
1. Remove unused Promise-returning public deployment push request wrappers from
   `deployment/PublicPushRouteBoundary.ts`.
2. Remove unused throwing `parsePublic*PushRequest(...)` and
   `parsePublic*PushRequestEffect(...)` compatibility wrappers from the same
   route boundary.
3. Keep the production/public Worker surface on
   `decodePublic*PushRequest(...)`, `decodePublic*PushJson(...)`, and
   `decodePublic*PushRoutePayload(...)` Effect decoders.
4. Collapse the redundant JSON-only HTTP adapter effect into the existing
   `publicDeploymentRouteErrorToHttpErrorEffect(...)` route adapter, which
   already maps `RequestJsonError` and `DeploymentProtocolValidationError`.
5. Update public deployment route-boundary tests to assert typed success and
   typed failure channels directly, then separately assert the route-level
   HTTP adapter mapping.
6. Leave analyzer preflight, artifact preflight, public deployment dispatch,
   DeploymentDO/DeploymentService/store behavior, executor-http, PartitionDO
   SQL/OCC, and `ValidatorJson` unchanged.
...
1. Add `delivery/InternalRouteBoundary.ts` as the Cloudflare-free internal
   DeliveryDO adapter module.
2. Route `runDeliveryRoute(...)` recovery through
   `deliveryInternalRouteErrorToResponseEffect(...)` instead of an inline tag
   table.
3. Add `deliveryInternalRouteErrorToHttpError(...)` and
   `deliveryInternalRouteErrorToHttpErrorEffect(...)` for direct adapter-edge
   reuse for non-drain-failure delivery route errors.
4. Keep `DeliveryDrainFailureError` owned by `deliveryDO.ts` and model it
   structurally at the adapter edge so drain failure envelopes keep their
   existing `500` JSON response behavior.
5. Add direct coverage for the named Delivery internal route HTTP and response
   adapters without importing `cloudflare:workers` into lightweight tests.
6. Leave DeliveryDO claim/fanout/ack scheduling, pending-drain storage,
   alarm/retry behavior, public Worker wake dispatch, SchedulerDO behavior,
   PartitionDO SQL/OCC, executor-http, and `ValidatorJson` unchanged.
...
1. Add explicit `RegistryRouteError` for Registry route JSON/protocol
   failures.
2. Add named `registryRouteErrorToHttpErrorEffect(...)` and route
   `registryApiRequestForRoute(...)` plus
   `readRegistryCreateDeploymentRouteRequest(...)` through it while preserving
   the current compatibility behavior where malformed JSON maps to
   `HttpError` and protocol failures remain `ProtocolValidationError`.
3. Route RegistryDO internal recovery through the named
   `registryInternalRouteErrorToResponseEffect(...)` adapter so typed
   route/protocol/operation failures convert to HTTP responses at one Durable
   Object edge.
4. Add direct coverage for named Registry route adapter mapping and named
   Registry internal response mapping.
5. Leave generated Registry HttpApi handlers, RegistryService/store behavior,
   SQL schema, public Worker pass-through routing, DeploymentService/store,
   PartitionDO SQL/OCC, executor-http, and `ValidatorJson` unchanged.
...
1. Remove the unused private `DeploymentValidationResult` bridge helpers
   (`deploymentValidationResultToEffect(...)`,
   `deploymentValidationEffectResult(...)`, and
   `unwrapDeploymentValidation(...)`) from `deployment/Validation.ts`.
2. Remove unused private result-returning compatibility wrappers around
   schema, function, analysis, codegen, placement, route, partition,
   visibility, validator, JSON value, table state, index state, source
   position, and codegen-metadata validation.
3. Keep the public compatibility wrappers that still define caller contracts:
   `validateSourcePackage(...)`, `validateDiagnostics(...)`,
   `analyzedStartPushRequest(...)`, `startAnalyzedPushInput(...)`,
   `pushStatusFromRow(...)`, `parsePushStatusFromRow(...)`,
   `validateSchema(...)`, `validateFunctions(...)`,
   `validateAnalysis(...)`, and `validateCodegenAnalysis(...)`.
4. Preserve the active store/service path where `DeploymentPushStore` consumes
   `decodePushStatusFromRow(...)` directly and propagates
   `DeploymentValidationError` as a typed failure.
5. Leave deployment SQL writes, active deployment metadata, push lifecycle
   service behavior, generated DeploymentDO HttpApi handlers, public Worker
   routing, PartitionDO SQL/OCC, executor-http, and `ValidatorJson`
   unchanged.
...
4. Route public Worker `wake-delivery` dispatch through the helper while
   preserving authorization, route deployment-id overlay, internal `/wake`
   URL, JSON body, and `delivery-wake` `PublicWorkerDispatchError` source.
5. Add direct tests for named delivery wake adapter effects and the dispatch
   helper success/failure channels.
6. Leave public Worker route matching, DeliveryDO drain behavior,
   SchedulerDO wake scheduling, ConnectionDO/live-query fanout, PartitionDO
   SQL/OCC, executor-http, and `ValidatorJson` unchanged.
...
1. Add explicit `PublicDeploymentRouteError` and
   `PublicDeploymentJsonError` aliases for the public deployment push request
   boundary.
2. Add named `Effect.fn(...)` adapter helpers
   `publicDeploymentRouteErrorToHttpErrorEffect(...)` and
   `publicDeploymentJsonErrorToHttpErrorEffect(...)` so compatibility readers
   recover through one typed HTTP adapter edge.
3. Add a shared named `dispatchDeploymentPushEffect(...)` service-binding
   helper for public deployment push dispatch operations, with an explicit
   `PublicDeploymentPushDispatchOperation` source union.
4. Route read, finish-artifact preflight, finish, abandon, start, and
   analyzed-start dispatch through the shared helper while preserving internal
   URLs, JSON bodies, and operation-specific `PublicWorkerDispatchError`
   sources.
5. Add direct tests for named route adapter effects and the shared dispatch
   helper success/failure channels.
6. Leave public Worker route matching, generated DeploymentDO HttpApi
   handlers, DeploymentService/store behavior, analyzer/artifact behavior,
   PartitionDO SQL/OCC, executor-http, and `ValidatorJson` unchanged.

Completed Goal 283 slice:

1. Add explicit `DeploymentReadFailure`, `DeploymentStartFailure`, and
   `DeploymentProtocolResponseFailure` aliases for the remaining generated
   DeploymentApi adapter failure channels.
2. Add named `Effect.fn(...)` helpers
   `deploymentReadFailureResponseEffect(...)`,
   `deploymentStartFailureResponseEffect(...)`, and
   `deploymentProtocolResponseFailureEffect(...)` that convert typed
   read/start/protocol failures to declared DeploymentApi response classes.
3. Route `mapDeploymentReadFailure(...)`,
   `mapDeploymentStartFailure(...)`, and
   `mapDeploymentProtocolResponseFailure(...)` through the named adapter
   effects, matching the existing finish and abandon mapping style.
4. Add direct tests for active-read not-found/storage, start bad-request/storage,
   and protocol response validation storage mappings.
5. Leave DeploymentService/store behavior, generated route wiring,
...
1. Extract every generated `DeploymentApiHandlers` endpoint body into named
   `Effect.fn(...)` helpers:
   `deploymentHealthHandler`, `deploymentGetActiveDeploymentHandler`,
   `deploymentGetPushHandler`, `deploymentStartAnalyzedPushHandler`,
   `deploymentFinishPushHandler`, and `deploymentAbandonPushHandler`.
2. Keep the generated `HttpApiBuilder.group(...)` declaration as route wiring
   while each endpoint helper owns request payload decoding, service call,
   typed failure mapping, and protocol response decoding for that endpoint.
3. Add direct handler-effect coverage for typed success and typed failure
   channels in addition to the existing Worker-compatible web handler tests.
4. Leave `DeploymentDO.fetch()`, internal route matching,
   `makeDeploymentApiWebHandler(...)`, DeploymentService/store behavior,
   public Worker routing, PartitionDO SQL/OCC, executor-http, and
   `ValidatorJson` unchanged.

Completed Goal 279 slice:

1. Add public invoke route failures to `Worker.routeDeployment` so
   deployment-scoped invoke no longer converts failures to `Response` inside
   the `invoke` branch.
2. Move deployment-scoped invoke response conversion to
   `publicWorkerDeploymentRouteErrorToResponse(...)`, preserving
   `invokeErrorResponse(...)` for invoke execution/validation/dispatch
   failures and the existing HTTP body/status contract.
3. Split the deployment route error model into invoke and non-invoke route
   failures so the non-invoke HTTP mapper does not accept invoke-only runtime
   errors.
4. Leave the top-level `/invoke` Worker branch, invoke execution internals,
   generated DeploymentDO HttpApi handlers, PartitionDO SQL/OCC,
   executor-http, and `ValidatorJson` unchanged.

Completed Goal 278 slice:

1. Widen `Worker.routeDeployment` again so public execution and partition route
   failures stay typed until `publicWorkerDeploymentRouteErrorToHttpError(...)`.
2. Remove early `Effect.mapError(...)` calls from the `executions` and
   `partitions` branches while preserving their public response statuses and
   bodies.
3. Centralize outer route mapping for shared `RequestJsonError`, deployment
   protocol errors, execution protocol errors, partition payload errors, public
   deployment path errors, and public execution path errors.
4. Leave public invoke's response-oriented compatibility branch, generated
   DeploymentDO HttpApi handlers, PartitionDO SQL/OCC, executor-http, and
   `ValidatorJson` unchanged for later batches.
