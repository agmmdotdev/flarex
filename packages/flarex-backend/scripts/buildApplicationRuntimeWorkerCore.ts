...
const ENTRY_PATH = path.join(
  ...
  "src",
  "artifactRuntime",
  "ApplicationRuntimeWorkerCore.ts",
);
const GENERATED_PATH = path.join(
  PACKAGE_ROOT,
  "src",
  "artifactRuntime",
  "ApplicationRuntimeWorkerCore.generated.ts",
);
const MAXIMUM_CORE_BYTES = 1_048_576;
...
if (
  ...
  if (mode !== "update" && mode !== "check") {
    throw new Error("Usage: buildApplicationRuntimeWorkerCore.ts <update|check>");
  }
...
async function buildTwice(): Promise<CoreBuildReceipt> {
  ...
  if (first.source !== second.source || first.sha256 !== second.sha256) {
    throw new Error("Application Runtime Worker core builds are not stable.");
  }
  return first;
}
...
async function buildOnce(): Promise<CoreBuildReceipt> {
  ...
  const output = await build({
    ...
    build: {
      ...
      lib: {
        entry: ENTRY_PATH,
        formats: ["es"],
        fileName: () => "application-runtime-core.js",
      },
      rollupOptions: { treeshake: true },
    },
  });
  if (!Array.isArray(output) || output.length !== 1) {
    throw new Error("Application Runtime Worker core build returned an invalid result.");
  }
  const buildResult = output[0];
  if (buildResult === undefined || !Array.isArray(buildResult.output)) {
    throw new Error("Application Runtime Worker core build returned no output.");
  }
  const emitted = buildResult.output;
  if (emitted.length !== 1 || emitted[0]?.type !== "chunk") {
    throw new Error("Application Runtime Worker core must emit one JavaScript chunk.");
  }
  const chunk = emitted[0];
  if (chunk === undefined || chunk.type !== "chunk") {
    throw new Error("Application Runtime Worker core chunk is missing.");
  }
  if (chunk.imports.length !== 0 || chunk.dynamicImports.length !== 0) {
    throw new Error("Application Runtime Worker core retained runtime imports.");
  }
  ...
  if (missingFrameworkExports.length !== 0) {
    throw new Error(
      "Application Runtime Worker core omitted framework exports: " +
        missingFrameworkExports.join(", "),
    );
  }
  if (
    !chunk.exports.includes("resolveApplicationRuntimeFunctionV1") ||
    !chunk.exports.includes("runApplicationRuntimeColdResolutionV1")
  ) {
    throw new Error("Application Runtime Worker core omitted its resolver.");
  }
  const source = chunk.code.replace(/\r\n?/g, "\n");
  const sourceBytes = Buffer.byteLength(source, "utf8");
  if (sourceBytes > MAXIMUM_CORE_BYTES) {
    throw new Error("Application Runtime Worker core exceeds its byte limit.");
  }
  return Object.freeze({
    source,
    sourceBytes,
    sha256: createHash("sha256")
      .update("flarex.application-runtime-dynamic-core.v1\0", "ascii")
      .update(source, "utf8")
      .update("\0", "ascii")
