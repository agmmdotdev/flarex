...
function entrypointSource(
  ...
  return [
    ...
    "  async start(startRequest, capability, queryCapability, mutationCapability) {",
    ...
    "    }",
    "    let ownedMutationCapability;",
    "    try { ownedMutationCapability = mutationCapability.dup(); }",
    "    catch (mutationCapabilityFailure) {",
    "      const cleanupFailures = [];",
    "      try { ownedQueryCapability[Symbol.dispose](); } catch (cause) { cleanupFailures.push(cause); }",
    "      try { ownedCapability[Symbol.dispose](); } catch (cause) { cleanupFailures.push(cause); }",
    "      if (cleanupFailures.length > 0) {",
    "        throw new Error('Application Task capability duplication cleanup failed.', {",
    "          cause: Object.freeze({ mutationCapabilityFailure, cleanupFailures: Object.freeze(cleanupFailures) }),",
    "        });",
    "      }",
    "      throw mutationCapabilityFailure;",
    "    }",
    "    const execution = await startApplicationTaskWorkerSessionV1({",
    "      startRequest, capability: ownedCapability, queryCapability: ownedQueryCapability, mutationCapability: ownedMutationCapability, definition, loadExecution,",
    "    });",
    "    this.ctx.waitUntil(execution.terminal.then(() => undefined, () => undefined));",
    "    return new FlarexTaskWorkerSession(execution);",
    "  }",
    "  run(request, capability, queryCapability, mutationCapability) {",
    "    return executeApplicationTaskWorkerV1({",
    "      request, capability, queryCapability, mutationCapability, definition, loadExecution,",
    "    });",
    "  }",
    "}",
    "",
