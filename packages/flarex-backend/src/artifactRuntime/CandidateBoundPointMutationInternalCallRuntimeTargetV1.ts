...
import {
  ...
  type CandidateBoundMutationInternalCallRuntimeTargetV1Error,
} from "flarex-protocol/internal/candidate-bound-mutation-internal-call-runtime-target-v1";
import {
...
import {
  ...
  POINT_MUTATION_INTERNAL_CALL_MAXIMUM_ARGUMENT_BYTES_V1,
  POINT_MUTATION_INTERNAL_CALL_MAXIMUM_CALLS_V1,
  POINT_MUTATION_INTERNAL_CALL_MAXIMUM_DEPTH_V1,
  POINT_MUTATION_INTERNAL_CALL_MAXIMUM_RESULT_BYTES_V1,
...
import {
  buildPointMutationInternalCallExactRuntimeWorkerDefinitionV1,
  pointMutationInternalCallExactRuntimeWorkerGraphBasisV1,
  PointMutationInternalCallExactRuntimeHostV1Error,
  type PointMutationInternalCallExactRuntimeWorkerDefinitionV1,
} from "./PointMutationInternalCallExactRuntimeHost";
import type {
...
export interface CandidateBoundMutationInternalCallRuntimeTargetAuthorityV1 {
...
export interface CandidateBoundMutationInternalCallRuntimeTargetAuthorityPortV1<Selection, E> {
  readonly claim: (
    selection: Selection,
    functionPath: string,
  ) => Effect.Effect<CandidateBoundMutationInternalCallRuntimeTargetAuthorityV1, E>;
}

export interface CandidateBoundMutationInternalCallRuntimeTargetBudgetV1 {
...
declare const mutationInternalCallRuntimeTargetBrand: unique symbol;
export interface CandidateBoundPointMutationInternalCallRuntimeTargetV1 {
  readonly [mutationInternalCallRuntimeTargetBrand]: true;
}

export interface PreparedCandidateBoundPointMutationInternalCallRuntimeTargetV1 {
  readonly target: CandidateBoundPointMutationInternalCallRuntimeTargetV1;
...
export interface ClaimedCandidateBoundPointMutationInternalCallRuntimeTargetV1
  extends PreparedCandidateBoundPointMutationInternalCallRuntimeTargetV1 {
  readonly canonicalTargetBytes: Uint8Array;
  readonly definition: PointMutationInternalCallExactRuntimeWorkerDefinitionV1;
}

export class CandidateBoundMutationInternalCallRuntimeDispatchV1Error extends Data.TaggedError(
  "CandidateBoundMutationInternalCallRuntimeDispatchV1Error",
)<{
  readonly reason:
    | "invalidBudget"
    | "authorityMismatch"
...
export class InvalidCandidateBoundPointMutationInternalCallRuntimeTargetV1Error
  extends Data.TaggedError("InvalidCandidateBoundPointMutationInternalCallRuntimeTargetV1Error")<{
    readonly reason: "notIssued";
  }> {}

export type PrepareCandidateBoundPointMutationInternalCallRuntimeTargetV1Error<E> =
  | E
  | CandidateBoundMutationInternalCallRuntimeDispatchV1Error
  | CandidateBoundMutationInternalCallRuntimeTargetV1Error
...
interface MutationTargetStateV1 {
  ...
  readonly canonicalTargetBytes: Uint8Array;
  readonly artifact: PointMutationInternalCallExactRuntimeArtifactRefV1;
  readonly function: PreparedCandidateBoundPointMutationInternalCallRuntimeTargetV1["function"];
  readonly definition: PointMutationInternalCallExactRuntimeWorkerDefinitionV1;
}

const targetStates = new WeakMap<CandidateBoundPointMutationInternalCallRuntimeTargetV1,
  MutationTargetStateV1>();

export const prepareCandidateBoundPointMutationInternalCallRuntimeTargetV1 = Effect.fn(
  "CandidateBoundMutationInternalCallRuntimeDispatch.prepareV1",
)(function* <Selection, AuthorityError>(
  selection: Selection,
  functionPath: string,
  authorityPort: CandidateBoundMutationInternalCallRuntimeTargetAuthorityPortV1<
    Selection,
    AuthorityError
  >,
  ...
  PreparedCandidateBoundPointMutationInternalCallRuntimeTargetV1,
  PrepareCandidateBoundPointMutationInternalCallRuntimeTargetV1Error<AuthorityError>,
  Scope.Scope
> {
  ...
  const graphBasis = yield* Effect.try({
    try: () => pointMutationInternalCallExactRuntimeWorkerGraphBasisV1({
      compatibilityDate,
      artifactExecutionModule: authority.function.artifactExecutionModule,
      exportName: authority.function.exportName,
      functionPath,
      internalFunctionCatalog: authority.internalFunctionCatalog,
    }),
    catch: cause => new PointMutationInternalCallExactRuntimeHostV1Error({
  ...
  const commitment = yield* deriveCandidateBoundRuntimeTargetCommitmentV1(
    ...
    graphBasisSha256 => encodeCandidateBoundMutationInternalCallRuntimeTargetV1(
  ...
  const definition = yield* Effect.try({
    try: () => buildPointMutationInternalCallExactRuntimeWorkerDefinitionV1({
      artifact,
      compatibilityDate,
...
export function claimCandidateBoundPointMutationInternalCallRuntimeTargetV1(
  target: unknown,
): Result.Result<
  ClaimedCandidateBoundPointMutationInternalCallRuntimeTargetV1,
  InvalidCandidateBoundPointMutationInternalCallRuntimeTargetV1Error
> {
  const state = claimState(target);
  return Result.map(state, value => Object.freeze({
    ...preparedResult(target as CandidateBoundPointMutationInternalCallRuntimeTargetV1, value),
...
function issueTarget(
  ...
  const target = Object.freeze({}) as CandidateBoundPointMutationInternalCallRuntimeTargetV1;
...
function claimState(target: unknown) {
  if (typeof target !== "object" || target === null) {
    return Result.fail(new InvalidCandidateBoundPointMutationInternalCallRuntimeTargetV1Error({
      reason: "notIssued",
    }));
  }
  const state = targetStates.get(target as CandidateBoundPointMutationInternalCallRuntimeTargetV1);
  return state === undefined
    ? Result.fail(new InvalidCandidateBoundPointMutationInternalCallRuntimeTargetV1Error({
        reason: "notIssued",
      }))
...
const verifyAuthority = Effect.fn(
  "CandidateBoundMutationInternalCallRuntimeDispatch.verifyAuthorityV1",
)(function* (
  authority: CandidateBoundMutationInternalCallRuntimeTargetAuthorityV1,
  requestedFunctionPath: string,
  sha256: DeclarativeV2RuntimeArtifactSha256V1,
  budget: CandidateBoundMutationInternalCallRuntimeTargetBudgetV1,
) {
  ...
  let previousPath = "";
  const ordinals = new Set<bigint>([authority.function.functionOrdinal]);
  const paths = new Set<string>([authority.function.functionPath]);
  for (const entry of authority.internalFunctionCatalog) {
    const separator = entry.functionPath.indexOf(":");
    if ((entry.handlerKind !== "query" && entry.handlerKind !== "mutation") ||
      entry.visibility !== "internal" ||
      entry.entry.handlerKind !== entry.handlerKind ||
      ...
      return yield* failure("functionMismatch", "internalFunctionCatalog");
    }
...
const loadSelectedProjection = Effect.fn(
  "CandidateBoundMutationInternalCallRuntimeDispatch.loadProjectionV1",
)(function* (
  authority: CandidateBoundMutationInternalCallRuntimeTargetAuthorityV1,
  store: DeclarativeV2RuntimeArtifactR2StoreV1,
  sha256: DeclarativeV2RuntimeArtifactSha256V1,
  ...
  for (let index = 0; index < authority.internalFunctionCatalog.length; index += 1) {
    const expected = authority.internalFunctionCatalog[index]!;
    const internalEntry = yield* readFrame(
      store,
      expected.entryReference,
      "function_group_entry",
      admission,
      `internalFunctionCatalog[${index}]`,
    );
  ...
  if (projection.moduleCount !== BigInt(modules.length) ||
    ...
    !authority.internalFunctionCatalog.every(entry =>
      modules.some(module => module.path === entry.artifactExecutionModule))) {
...
function targetFrame(
  ...
  budget: CandidateBoundMutationInternalCallRuntimeTargetBudgetV1,
): CandidateBoundMutationInternalCallRuntimeTargetFrameV1 {
  ...
  return Object.freeze({
    ...
    compatibilityDate,
    exactRuntimeProfile: POINT_MUTATION_INTERNAL_CALL_EXACT_RUNTIME_PROFILE_V1,
    exactRuntimeVersion: POINT_MUTATION_INTERNAL_CALL_EXACT_RUNTIME_VERSION_V1,
    syscallAbiIdentity: POINT_MUTATION_INTERNAL_CALL_EXACT_RUNTIME_SYSCALL_ABI_V1,
    ...
    visibility: "public",
    group: "transaction",
    maximumInternalCalls: BigInt(POINT_MUTATION_INTERNAL_CALL_MAXIMUM_CALLS_V1),
    maximumInternalCallDepth:
      BigInt(POINT_MUTATION_INTERNAL_CALL_MAXIMUM_DEPTH_V1),
    maximumInternalCallArgumentBytes:
      BigInt(POINT_MUTATION_INTERNAL_CALL_MAXIMUM_ARGUMENT_BYTES_V1),
    maximumInternalCallResultBytes:
      BigInt(POINT_MUTATION_INTERNAL_CALL_MAXIMUM_RESULT_BYTES_V1),
    ...
    internalFunctionCatalog: Object.freeze(authority.internalFunctionCatalog.map(
      entry => Object.freeze({
...
function readFrame<K extends DeclarativeV2PhysicalFrameV1["kind"]>(
  ...
  admission: (
    ...
    Effect.Effect<void, CandidateBoundMutationInternalCallRuntimeDispatchV1Error>,
  path: string,
): Effect.Effect<
...
function objectAdmission(budget: CandidateBoundMutationInternalCallRuntimeTargetBudgetV1) {
  let objects = 0;
  let bytes = 0;
...
function captureBudget(input: unknown) {
  ...
  if (!isNonArrayRecord(input) || Reflect.ownKeys(input).length !== keys.length ||
    ...
    return Result.fail(new CandidateBoundMutationInternalCallRuntimeDispatchV1Error({
...
function failure(reason: CandidateBoundMutationInternalCallRuntimeDispatchV1Error["reason"], path: string) {
  return Effect.fail(new CandidateBoundMutationInternalCallRuntimeDispatchV1Error({ reason, path }));
}
function exceeded(path: string, observed: number, maximum: number) {
  return Effect.fail(new CandidateBoundMutationInternalCallRuntimeDispatchV1Error({
