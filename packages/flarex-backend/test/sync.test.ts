...
describe("sync protocol", () => {
  ...
  it("rejects malformed public DeliveryDO wake JSON at the Worker boundary", async () => {
    const harness = await createSyncHarness([], undefined, undefined, {
    ...
    const response = await harness.mf.dispatchFetch(
      "http://flarex.test/deployments/sync-public-delivery-wake-boundary/sync/wake-delivery",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("rejects invalid public DeliveryDO wake envelopes at the Worker boundary", async () => {
    const harness = await createSyncHarness([], undefined, undefined, {
      bindings: { FLAREX_LIVE_QUERY_DELIVERY_TOKEN: "wake-secret" },
    });
    harnesses.push(harness);
    ...
    const response = await harness.mf.dispatchFetch(
      "http://flarex.test/deployments/sync-public-delivery-wake-envelope/sync/wake-delivery",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
  ...
  it("rejects unauthorized live query delivery reconcile before parsing JSON", async () => {
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
      undefined,
  ...
  it("rejects invalid live query delivery reconcile envelopes at the public scheduler boundary", async () => {
    const executorRequests: unknown[] = [];
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
    ...
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toEqual({
      error: "cursor.oldestCreatedAt must be an ISO date string.",
    });
    expect(executorRequests).toEqual([]);
  });
  ...
  it("rejects malformed live query subscription rerun JSON", async () => {
    const executorRequests: unknown[] = [];
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
  ...
  it("rejects unauthorized live query subscription rerun before parsing JSON", async () => {
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
      undefined,
  ...
  it("rejects invalid live query subscription rerun envelopes at the public scheduler boundary", async () => {
    const executorRequests: unknown[] = [];
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
  ...
  it("rejects malformed live query dead-letter JSON", async () => {
    const executorRequests: unknown[] = [];
    const harness = await createSyncHarness(
      [],
      () => ({ user: "Ada" }),
