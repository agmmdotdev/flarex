...
export class LocalMiniflareExecutionArtifactAdapter implements ExecutionArtifactAdapter {
  async analyze(sourcePackage: SourcePackage): Promise<DeploymentAnalysis> {
    ...
    try {
      ...
      return body as DeploymentAnalysis;
    } finally {
...
import execution from ${JSON.stringify(executionImport)};
${schemaImportLine}

function analyze() {
  return {
    functions: analyzeModuleExports(execution),
    schema: analyzeSchema(schema),
  };
}
