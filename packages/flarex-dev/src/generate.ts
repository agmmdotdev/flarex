...
function deploymentSchemaSource(): string {
  return `// Generated by flarex-dev. Do not edit.
import { validatorToJson } from "flarex/server";
import schema from "../schema";

type RuntimeTable = {
  kind: "table";
  validator: Parameters<typeof validatorToJson>[0];
  indexes: Array<{ name: string; fields: readonly string[] }>;
  placement?: { kind: string; field?: string; table?: string };
};
type RuntimeSchemaEntry = RuntimeTable | { kind: string };

const tableEntries = Object.entries(schema.tables as Record<string, RuntimeSchemaEntry>)
  .filter((entry): entry is [string, RuntimeTable] => entry[1].kind === "table")
  .sort(([left], [right]) => left.localeCompare(right));

const tableIds = new Map(tableEntries.map(([name], index) => [name, index + 1] as const));

let nextIndexId = 1;

export const deploymentSchema = {
  version: 1,
  tables: tableEntries.map(([name, table]) => ({
    tableId: tableIds.get(name)!,
    name,
    validator: validatorToJson(table.validator),
    placement: table.placement ?? { kind: "partitionBy", field: "_id" },
  })),
  indexes: tableEntries.flatMap(([tableName, table]) =>
...
export async function finalCodegen(
  ...
  await writeFile(path.join(generatedDir, "functionRegistry.ts"), functionRegistrySource(analysis));
  await writeFile(path.join(generatedDir, "functionMetadata.ts"), functionMetadataSource(analysis));
  await writeFile(path.join(generatedDir, "deploymentSchema.ts"), deploymentSchemaSource());
