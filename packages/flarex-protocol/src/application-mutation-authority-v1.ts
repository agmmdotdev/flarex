...
export const APPLICATION_MUTATION_EXECUTION_AUTHORITY_FORMAT_V1 =
  "flarex.application-mutation-execution-authority";
export const APPLICATION_MUTATION_EXECUTION_AUTHORITY_VERSION_V1 = 1;
export const MAX_APPLICATION_MUTATION_EXECUTION_AUTHORITY_BYTES_V1 = 131_072;

export interface ApplicationMutationExecutionAuthorityV1 {
  readonly format: typeof APPLICATION_MUTATION_EXECUTION_AUTHORITY_FORMAT_V1;
  readonly version: typeof APPLICATION_MUTATION_EXECUTION_AUTHORITY_VERSION_V1;
...
export interface CanonicalApplicationMutationExecutionAuthorityV1 {
...
export class ApplicationMutationExecutionAuthorityV1Error
  extends Data.TaggedError("ApplicationMutationExecutionAuthorityV1Error")<{
...
export const canonicalizeApplicationMutationExecutionAuthorityV1 = Effect.fn(
  "ApplicationMutationExecutionAuthority.canonicalizeV1",
)(function* (
  input: unknown,
): Effect.fn.Return<
...
> {
  ...
  const authorityJson = Object.freeze({
    format: APPLICATION_MUTATION_EXECUTION_AUTHORITY_FORMAT_V1,
    version: APPLICATION_MUTATION_EXECUTION_AUTHORITY_VERSION_V1,
  ...
  const canonicalBytes = UTF8.encode(encodeCanonicalJson(
    ...
    issue => {
      throw new Error(
        `Application mutation authority invariant: ${issue.reason}`,
      );
    },
  ));
  if (
    ...
    canonicalBytes.byteLength >
      MAX_APPLICATION_MUTATION_EXECUTION_AUTHORITY_BYTES_V1
  ...
  } satisfies CanonicalApplicationMutationExecutionAuthorityV1);
});
...
> {
  return Result.try({
    try: () => {
      ...
      if (
        values.format !== APPLICATION_MUTATION_EXECUTION_AUTHORITY_FORMAT_V1 ||
        values.version !== APPLICATION_MUTATION_EXECUTION_AUTHORITY_VERSION_V1 ||
...
function sha256Hex(
  bytes: Uint8Array,
): Effect.Effect<string, ApplicationMutationExecutionAuthorityV1Error> {
  return hash(bytes).pipe(Effect.map(encodeBytesToLowercaseHex));
}

function hash(
  bytes: Uint8Array,
): Effect.Effect<Uint8Array, ApplicationMutationExecutionAuthorityV1Error> {
...
function failure(
  ...
  return new ApplicationMutationExecutionAuthorityV1Error({
