...
} from "./point-mutation-internal-call-exact-runtime";

export const CANDIDATE_BOUND_MUTATION_INTERNAL_CALL_RUNTIME_TARGET_IDENTITY_V1 =
  "flarex.system/candidate-bound-mutation-internal-call-runtime-target/v1" as const;

const DOMAIN = new TextEncoder().encode(
  `${CANDIDATE_BOUND_MUTATION_INTERNAL_CALL_RUNTIME_TARGET_IDENTITY_V1}\0`,
);
const UTF8 = new TextEncoder();
const MAX_U64 = (1n << 64n) - 1n;

export interface CandidateBoundMutationInternalCallRuntimeTargetModuleV1 {
...
export interface CandidateBoundMutationInternalCallRuntimeTargetCatalogEntryV1 {
...
export interface CandidateBoundMutationInternalCallRuntimeTargetFrameV1 {
  ...
  readonly exactRuntimeProfile:
    typeof POINT_MUTATION_INTERNAL_CALL_EXACT_RUNTIME_PROFILE_V1;
  readonly exactRuntimeVersion: typeof POINT_MUTATION_EXACT_RUNTIME_VERSION_V1;
  readonly syscallAbiIdentity:
    typeof POINT_MUTATION_INTERNAL_CALL_EXACT_RUNTIME_SYSCALL_ABI_V1;
  ...
  readonly modules: ReadonlyArray<CandidateBoundMutationInternalCallRuntimeTargetModuleV1>;
  readonly internalFunctionCatalog: ReadonlyArray<
    CandidateBoundMutationInternalCallRuntimeTargetCatalogEntryV1
  >;
}

export interface CandidateBoundMutationInternalCallRuntimeTargetEncodingBudgetV1 {
...
export class CandidateBoundMutationInternalCallRuntimeTargetV1Error extends Data.TaggedError(
...
export function encodeCandidateBoundMutationInternalCallRuntimeTargetV1(
  input: unknown,
  budgetInput: unknown,
): Result.Result<
  Readonly<{
    readonly frame: CandidateBoundMutationInternalCallRuntimeTargetFrameV1;
...
> {
  return Result.gen(function* () {
    ...
    segments.push(u32(frame.internalFunctionCatalog.length));
    for (const entry of frame.internalFunctionCatalog) {
      segments.push(
    ...
    if (!Number.isSafeInteger(byteLength) || byteLength > budget.maximumPreimageBytes) {
      return yield* Result.fail(new CandidateBoundMutationInternalCallRuntimeTargetV1Error({
...
function captureFrame(
  input: unknown,
  budget: CandidateBoundMutationInternalCallRuntimeTargetEncodingBudgetV1,
): Result.Result<CandidateBoundMutationInternalCallRuntimeTargetFrameV1, CandidateBoundMutationInternalCallRuntimeTargetV1Error> {
  return Result.gen(function* () {
    ...
    const modules: CandidateBoundMutationInternalCallRuntimeTargetModuleV1[] = [];
    ...
    if (!Array.isArray(input.internalFunctionCatalog) ||
      input.internalFunctionCatalog.length > budget.maximumCatalogEntries) {
      return yield* fail("invalidInput", "internalFunctionCatalog");
    }
    const internalFunctionCatalog:
      CandidateBoundMutationInternalCallRuntimeTargetCatalogEntryV1[] = [];
    let previousOrdinal = -1n;
    let previousPath = "";
    const paths = new Set<string>([functionPath]);
    const ordinals = new Set<bigint>([functionOrdinal]);
    for (let index = 0; index < input.internalFunctionCatalog.length; index += 1) {
      const value = input.internalFunctionCatalog[index];
      if (!isNonArrayRecord(value) || Reflect.ownKeys(value).length !== 9 ||
        (value.handlerKind !== "query" && value.handlerKind !== "mutation") ||
        value.visibility !== "internal" ||
        value.group !== "transaction") {
        return yield* fail("invalidInput", `internalFunctionCatalog[${index}]`);
      }
      const entryOrdinal = yield* nonNegativeU64(
        value.functionOrdinal, `internalFunctionCatalog[${index}].functionOrdinal`,
      );
      const entryPath = yield* boundedText(
        value.functionPath, `internalFunctionCatalog[${index}].functionPath`,
      );
      ...
      internalFunctionCatalog.push(Object.freeze({
        ...
        logicalExecutionModule: yield* boundedText(
          ...
          `internalFunctionCatalog[${index}].logicalExecutionModule`,
        ),
        artifactExecutionModule: yield* boundedText(
          value.artifactExecutionModule,
          `internalFunctionCatalog[${index}].artifactExecutionModule`,
        ),
        exportName: yield* boundedText(
          value.exportName, `internalFunctionCatalog[${index}].exportName`,
        ),
        handlerKind: value.handlerKind,
        ...
        functionEntryReference: yield* captureReference(
          ...
          `internalFunctionCatalog[${index}].functionEntryReference`,
          budget.maximumTextBytes,
        ),
      }));
    }
    return Object.freeze({
      ...
      ...(digests as unknown as Pick<CandidateBoundMutationInternalCallRuntimeTargetFrameV1,
        typeof DIGEST_FIELDS[number]>),
      compatibilityDate,
...
function captureReference(
  ...
  return Result.gen(function* () {
    ...
    ).pipe(Result.mapError(() => new CandidateBoundMutationInternalCallRuntimeTargetV1Error({
...
function captureText(input: unknown, path: string, maximum: number) {
  ...
  return length <= maximum
    ...
    : Result.fail(new CandidateBoundMutationInternalCallRuntimeTargetV1Error({
...
function fail(reason: CandidateBoundMutationInternalCallRuntimeTargetV1Error["reason"], path: string) {
  return Result.fail(new CandidateBoundMutationInternalCallRuntimeTargetV1Error({ reason, path }));
}
