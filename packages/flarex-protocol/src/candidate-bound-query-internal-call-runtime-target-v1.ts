...
} from "./point-query-internal-call-exact-runtime";

export const CANDIDATE_BOUND_QUERY_INTERNAL_CALL_RUNTIME_TARGET_IDENTITY_V1 =
  "flarex.system/candidate-bound-query-internal-call-runtime-target/v1" as const;

const DOMAIN = new TextEncoder().encode(
  `${CANDIDATE_BOUND_QUERY_INTERNAL_CALL_RUNTIME_TARGET_IDENTITY_V1}\0`,
);
const UTF8 = new TextEncoder();
const MAX_U64 = (1n << 64n) - 1n;

export interface CandidateBoundQueryInternalCallRuntimeTargetModuleV1 {
...
export interface CandidateBoundQueryInternalCallRuntimeTargetCatalogEntryV1 {
...
export interface CandidateBoundQueryInternalCallRuntimeTargetFrameV1 {
  ...
  readonly exactRuntimeProfile:
    typeof POINT_QUERY_INTERNAL_CALL_EXACT_RUNTIME_PROFILE_V1;
  readonly exactRuntimeVersion: typeof POINT_QUERY_EXACT_RUNTIME_VERSION_V1;
  readonly syscallAbiIdentity:
    typeof POINT_QUERY_INTERNAL_CALL_EXACT_RUNTIME_SYSCALL_ABI_V1;
  ...
  readonly modules: ReadonlyArray<CandidateBoundQueryInternalCallRuntimeTargetModuleV1>;
  readonly internalQueryCatalog: ReadonlyArray<
    CandidateBoundQueryInternalCallRuntimeTargetCatalogEntryV1
  >;
}

export interface CandidateBoundQueryInternalCallRuntimeTargetEncodingBudgetV1 {
...
export class CandidateBoundQueryInternalCallRuntimeTargetV1Error extends Data.TaggedError(
...
export function encodeCandidateBoundQueryInternalCallRuntimeTargetV1(
  input: unknown,
  budgetInput: unknown,
): Result.Result<
  Readonly<{
    readonly frame: CandidateBoundQueryInternalCallRuntimeTargetFrameV1;
...
> {
  return Result.gen(function* () {
    ...
    if (!Number.isSafeInteger(byteLength) || byteLength > budget.maximumPreimageBytes) {
      return yield* Result.fail(new CandidateBoundQueryInternalCallRuntimeTargetV1Error({
...
function captureFrame(
  input: unknown,
  budget: CandidateBoundQueryInternalCallRuntimeTargetEncodingBudgetV1,
): Result.Result<CandidateBoundQueryInternalCallRuntimeTargetFrameV1, CandidateBoundQueryInternalCallRuntimeTargetV1Error> {
  return Result.gen(function* () {
    if (!isNonArrayRecord(input)) return yield* fail("invalidInput", "$frame");
    const expectedKeys = 50;
    ...
    const modules: CandidateBoundQueryInternalCallRuntimeTargetModuleV1[] = [];
    ...
    return Object.freeze({
      ...
      ...(digests as unknown as Pick<CandidateBoundQueryInternalCallRuntimeTargetFrameV1,
      ...
      maximumDocumentBytes, maximumResultBytes, maximumInternalCalls,
...
function captureReference(
  ...
  return Result.gen(function* () {
    ...
    ).pipe(Result.mapError(() => new CandidateBoundQueryInternalCallRuntimeTargetV1Error({
...
function captureText(input: unknown, path: string, maximum: number) {
  ...
  return length <= maximum
    ...
    : Result.fail(new CandidateBoundQueryInternalCallRuntimeTargetV1Error({
...
function fail(reason: CandidateBoundQueryInternalCallRuntimeTargetV1Error["reason"], path: string) {
  return Result.fail(new CandidateBoundQueryInternalCallRuntimeTargetV1Error({ reason, path }));
}
