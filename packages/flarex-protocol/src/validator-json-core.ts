...
export type ValidatorJsonV1 =
  ...
  | { readonly type: "union"; readonly value: ReadonlyArray<ValidatorJsonV1> };
...
export function validatorJsonAdmissionIssueV1(
  ...
  const pending: Array<Readonly<{ readonly value: unknown; readonly depth: number }>> = [
    { value: root, depth: 1 },
  ];
  let nodes = 0;
  try {
    while (pending.length > 0) {
      ...
      if (type === MISSING_VALIDATOR_PROPERTY || typeof type !== "string") {
        continue;
      }
      ...
      switch (type) {
        ...
        case "object": {
          ...
          for (const key of keys) {
            ...
            if (field === MISSING_VALIDATOR_PROPERTY || !isObjectContainer(field)) {
              continue;
            }
        ...
        case "union": {
          ...
          if (members === MISSING_VALIDATOR_PROPERTY || !Array.isArray(members)) {
            break;
          }
