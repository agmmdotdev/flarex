...
export const makeLocalProductCommands = Effect.fn("ProductAdapter.commands")(function* () {
  ...
  const workflow = yield* Effect.fromResult(productWorkflowModule(description, commands, graph,
    { created: metadata.tag.createdEvent, updated: metadata.tag.updatedEvent, deleted: metadata.tag.deletedEvent, variantUpdated: metadata.variant.updatedEvent, variantCreated: metadata.variant.createdEvent, productCreated: metadata.product.createdEvent, productUpdated: metadata.product.updatedEvent, collectionUpdated: metadata.collection.updatedEvent }));
