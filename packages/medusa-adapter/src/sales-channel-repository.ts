...
/** Prepared from the checked native model, with no live manager or SQL handle.
 * Scalar types/defaults remain with DML lowering and the core row writer. */
export function prepareSalesChannelRepository(metadata: Metadata) {
  return Result.gen(function* () {
    ...
    if (metadata.tables.length !== 1 || table?.name !== "sales_channel" || primary?.length !== 1 || primary[0]?.name !== "id" || primary[0].type !== "id") {
    ...
    const decodeUpdate = commerceRowDecoder(scalarNames.filter(name => name !== "id"), "unsupportedProfile");
    ...
    const bind = (root: CommerceCommandContext, owner: CommercePromiseOwner) => {
      ...
      const find = Effect.fn("SalesChannelRepository.find")(function* (ctx: CommerceCommandContext, input: unknown, withCount: boolean) {
      ...
      const repository: DAL.RepositoryService = {
        ...
        create: (input, shared) => bridge.execute(shared, ctx => bridge.checked(ctx, Effect.gen(function* () {
          ...
          yield* events.created(inserted.map(entity => ({ modelName: "SalesChannel", entity: { ...entity } })), shared);
          return [...inserted];
        }))),
