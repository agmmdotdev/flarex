...
let policy: ReturnType<typeof productLocalEventPolicy>;
let revision: string;
...
const assemble = async (hooks?: ProductTagWorkflowHooks, suffix = "standard") => {
  const bundle = await runEffect(prepareProductTagWorkflow({ revision: createHash("sha256").update(revision).update(suffix).digest("hex"),
    moduleEvents: policy, subscribers, ...(hooks === undefined ? {} : { hooks }) }));
  const input = { ...currency.hostInput, commands: [bundle.command], events: bundle.events, participants: [
    { participant: bundle.productParticipant, profile: product.prepared.profile, installation: product.installation,
      commands: bundle.productCommands, validate: policy.validate, eventContract: bundle.moduleContract },
    { participant: bundle.currencyParticipant, profile: currency.prepared.profile, installation: currency.installation, commands: bundle.currencyCommands },
  ] };
  return { ...bundle, input, host: await runEffect(makeAtomicCommerceHost(input)) };
};
...
describe("native Medusa Product-tag workflow and committed events", () => {
  beforeAll(async () => {
    ...
    const paths = ["product-tag-workflow.ts", "workflow-runtime.ts", "workflow-value.ts"];
    ...
    const commands = await runEffect(makeLocalProductCommands());
    ...
    product = await commerceHostFixture(resource.persistence, resource.session, prepareLocalProductProfile, [commands.commands.createTags], control,
      descriptor => productLocalEventPolicy(descriptor, catalog, () => Effect.void), {}, currency);
    policy = productLocalEventPolicy(product.descriptor, catalog, () => Effect.die("Durable workflow used local delivery"));
  }, 150_000);
  ...
  it("executes the original business sequence with pending graph reads, both event families and one root commit", async () => {
    ...
    const result = await runEffect(workflow.host.run(key, workflow.command, args("workflow-created")));
    ...
    expect(await runEffect(workflow.host.run(key, workflow.command, args("workflow-created")))).toEqual(result);
    expect(hookCalls).toBe(1);
    expect(await inventory()).toEqual(after);
    const changed = await assemble(undefined, "changed-bundle");
    expect(await runEffectFailure(changed.host.run(key, changed.command, args("workflow-created")))).toMatchObject({ reason: "requestConflict" });
  }, 30_000);
  ...
  it("rolls back module rows and both event families when the hook fails", async () => {
    ...
    const before = await inventory();
    expect(Exit.isFailure(await runEffect(Effect.exit(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("workflow-failed")))))).toBe(true);
  ...
  it("keeps a caught unsupported-resource refusal fatal and revokes escaped services", async () => {
    ...
    const before = await inventory();
    expect(Exit.isFailure(await runEffect(Effect.exit(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("caught-hook")))))).toBe(true);
  ...
  it("cancels a pending hook without committing and releases its scoped runners", async () => {
    ...
    await runEffect(Effect.scoped(Effect.gen(function* () {
      const fiber = yield* Effect.forkScoped(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("cancel-hook")));
  ...
  it("recovers a lost commit acknowledgement and emits nothing for empty input", async () => {
    ...
    const host = await runEffect(makeAtomicCommerceHost({ ...workflow.input, session }));
    const before = await inventory();
    const key = host.newRequestKey();
    const result = await runEffect(host.run(key, workflow.command, args("recover-workflow")));
    expect(result).toMatchObject([{ value: "recover-workflow" }]);
    expect(hookCalls).toBe(1);
    expect((await inventory()).events).toHaveLength(before.events.length + 2);
    expect(await runEffect(host.run(key, workflow.command, args("recover-workflow")))).toEqual(result);
    expect(hookCalls).toBe(1);
    const empty = await assemble();
    const count = (await inventory()).events.length;
    expect(await runEffect(empty.host.run(empty.host.newRequestKey(), empty.command, { product_tags: [] }))).toEqual([]);
  ...
  it("rolls back failures at event, delivery, result, wake and clock publication", async () => {
    ...
    for (const table of ["fx_system_commit_event", "fx_system_commit_event_delivery", "fx_system_idempotency", "fx_system_outbox", "fx_system_scope_clock"]) {
      ...
      try {
        ...
        expect(Exit.isFailure(await runEffect(Effect.exit(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("publication-failed")))))).toBe(true);
        expect(await inventory()).toEqual(before);
      } finally {
  ...
  it("repairs delivery from committed storage after host restart and result expiry", async () => {
    const workflow = await assemble();
    const key = workflow.host.newRequestKey();
    const value = await runEffect(workflow.host.run(key, workflow.command, args("restart-delivery")));
    ...
    expect(await runEffectFailure(workflow.host.run(key, workflow.command, args("restart-delivery")))).toMatchObject({ reason: "resultUnavailable" });
  ...
  it("recovers a lost delivery acknowledgement with stable identity and fenced settlement", async () => {
    const workflow = await assemble();
    await runEffect(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("claim-fence")));
  ...
  it("keeps acknowledged subscribers independent and persists bounded failures without compensating writes", async () => {
    const workflow = await assemble();
    const key = workflow.host.newRequestKey();
    const value = await runEffect(workflow.host.run(key, workflow.command, args("subscriber-failure")));
    const commit = (await inventory()).product.commits.at(-1);
    if (commit === undefined) throw new Error("Missing event commit");
    ...
    expect(await runEffect(workflow.host.run(key, workflow.command, args("subscriber-failure")))).toEqual(value);
  }, 60_000);

  it("refuses incomplete event families and leaves missing handler revisions pending", async () => {
    const workflow = await assemble();
    await runEffect(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("integrity")));
    const before = await inventory();
  ...
  it.runIf(driver === "postgres")("serializes competing claims without claiming the same delivery twice", async () => {
    const workflow = await assemble();
    await runEffect(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("concurrent-delivery")));
    const commit = (await inventory()).product.commits.at(-1);
    if (commit === undefined) throw new Error("Missing event commit");
  ...
  it("keeps foreign event and shared byte/call budget failures fatal after pending writes", async () => {
    ...
    for (const [index, hook] of hooks.entries()) {
      ...
      expect(Exit.isFailure(await runEffect(Effect.exit(workflow.host.run(workflow.host.newRequestKey(), workflow.command, args("bounded-hook"))))), "bounded hook " + index).toBe(true);
      expect(await inventory()).toEqual(before);
    }
  }, 60_000);
