...
export const batchLinkProductsToCategoryStep = createStep(
  { name: batchLinkProductsToCategoryStepId, compensation: "transactionCovered" },
  async (
    ...
    const dbProducts = await service.listProducts(
      { id: [...(data.add ?? []), ...(data.remove ?? [])] },
      {
        ...
        // Native profile admits at most 256 IDs; do not truncate at the service's default page.
        take: 256,
      }
    )
