...
import { getSelectsAndRelationsFromObjectArray } from "@medusajs/utils/common/get-selects-and-relations-from-object-array"
...
export const updateProductVariantsStep = createStep(
  { name: updateProductVariantsStepId, compensation: "transactionCovered" },
  async (data: UpdateProductVariantsStepInput, { container }) => {
    ...
    if ("product_variants" in data) {
      ...
      const prevData = await service.listProductVariants({
        id: data.product_variants.flatMap((p) => p.id === undefined ? [] : [p.id]),
      })
