...
import { WorkflowDefinitionError } from "@medusajs/workflows-sdk"
import { addImagesToVariantStep } from "../steps/add-images-to-variant"
import { removeImagesFromVariantStep } from "../steps/remove-images-from-variant"
import { updateProductVariantsStep } from "../steps/update-product-variants"
...
export const batchVariantImagesWorkflow = createWorkflow(
  ...
  (
    ...
    // Native fork: one transaction executes add, then remove in source order.
    const res = [
      ...
      const shouldUpdateVariantThumbnail = transform(
        ...
        (data) => {
          ...
          if (data.variant === undefined) {
            throw new WorkflowDefinitionError({ reason: "invalidDefinition", detail: "Cannot check the thumbnail of a missing variant" })
          }
