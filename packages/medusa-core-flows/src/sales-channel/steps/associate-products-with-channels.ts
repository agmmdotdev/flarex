...
import type { Link } from "@medusajs/modules-sdk/link"
...
export const associateProductsWithSalesChannelsStep = createStep(
  { name: associateProductsWithSalesChannelsStepId, compensation: "transactionCovered" },
  async (input: AssociateProductsWithSalesChannelsStepInput, { container }) => {
    if (!input.links?.length) {
      return new StepResponse([], [])
    }

    const remoteLink = container.resolve<Pick<Link, "create" | "dismiss">>(ContainerRegistrationKeys.LINK)
