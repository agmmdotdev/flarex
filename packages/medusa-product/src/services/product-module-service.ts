...
import type { ProductCategoryService } from "../runtime-contracts"
...
import type { EntityManager } from "@medusajs/deps/mikro-orm/core"
import type { ProductRepository } from "../runtime-contracts"
import type {
  UpdateCategoryInput,
...
type InternalServiceWithEventSubscriber = {
  setEventSubscriber(subscriber: ModulePersistenceEventSubscriber): void
}
type ProductDimensionField = "weight" | "length" | "height" | "width"
type ProductDimensionCarrier = Partial<Record<ProductDimensionField, unknown>>
  ...
  implements ProductTypes.IProductModuleService
{
  ...
  constructor(
    ...
    this.attachInternalServiceEventSubscribers_()
  }

  private attachInternalServiceEventSubscribers_(): void {
    const subscriber = this.createInternalServiceEventSubscriber_()
    ...
    for (const service of [
      ...
      this.productImageService_,
      this.productTypeService_,
      this.productOptionService_,
      this.productOptionValueService_,
      this.productVariantProductImageService_,
    ]) {
      if (hasEventSubscriberSetter(service)) {
        service.setEventSubscriber(subscriber)
      }
    }
  }

  private createInternalServiceEventSubscriber_(): ModulePersistenceEventSubscriber {
    const service = this as unknown as ModulePersistenceMutationService
  ...
  protected async updateProducts_(
    ...
    if (manager && subscriber && hasMikroOrmEventManager(manager)) {
      manager
        .getEventManager()
    ...
    const updatedProducts = hasDeepUpdate(this.productRepository_)
      ? await this.productRepository_.deepUpdate(
          normalizedProducts,
      ...
      : (
          await this.productService_.upsertWithReplace(
            this.prepareProductsForDrizzleUpsertWithReplace_(
              normalizedProducts,
              originalProducts
            ),
            {
              relations: ["options", "variants", "images", "tags", "categories"],
            },
            sharedContext
          )
        ).entities
  ...
  private prepareProductsForDrizzleUpsertWithReplace_(
    ...
    return products.map((product) => {
      ...
      const options = product.options?.map((option) => ({
        ...option,
        values: option.values?.map(normalizeOptionValueForDrizzle),
      }))
      const variantOptions = options ?? originalProduct?.options
      const optionEntities = variantOptions?.map((option) => ({
        ...option,
        product_id: product.id,
      })) as InferEntityType<typeof ProductOption>[] | undefined
      const variantPayloads = product.variants?.map((variant) => ({
        ...variant,
        product_id: readProductId(variant) ?? product.id,
      }))

      if (variantPayloads?.length && optionEntities?.length) {
        ProductModuleService.validateVariantOptions(
          variantPayloads,
          optionEntities
        )
      ...
      const variants =
        variantPayloads?.length && optionEntities?.length
          ? ProductModuleService.assignOptionsToVariants(
              variantPayloads,
              optionEntities
            )
  ...
  >(
    ...
    for (const productData of normalizedProducts) {
      ...
      normalizeProductDimensionFields(productData)
...
function hasEventSubscriberSetter(
  service: unknown
): service is InternalServiceWithEventSubscriber {
  return (
    service !== null &&
    typeof service === "object" &&
    "setEventSubscriber" in service &&
    typeof service.setEventSubscriber === "function"
  )
}

function hasMikroOrmEventManager(
  manager: unknown
): manager is EntityManager {
  return (
    manager !== null &&
    typeof manager === "object" &&
    "getEventManager" in manager &&
    typeof manager.getEventManager === "function"
  )
}

function hasDeepUpdate(
  repository: unknown
): repository is Pick<ProductRepository, "deepUpdate"> {
  return (
    repository !== null &&
    typeof repository === "object" &&
    "deepUpdate" in repository &&
    typeof repository.deepUpdate === "function"
  )
}

function readProductId(value: object): string | undefined {
  if (!("product_id" in value)) {
    return undefined
  }

  return typeof value.product_id === "string" ? value.product_id : undefined
}

function normalizeOptionValueForDrizzle(
  value: string | { id?: string; value?: unknown }
): { id: string; value: unknown } {
  if (typeof value === "string") {
    return {
      id: generateEntityId(undefined, "optval"),
      value,
    }
  }
  ...
  return {
    ...value,
    id: value.id ?? generateEntityId(undefined, "optval"),
    value: value.value,
  }
}

function normalizeProductDimensionFields(
  productData: ProductDimensionCarrier
): void {
  for (const field of ["weight", "length", "height", "width"] as const) {
    if (!(field in productData) || productData[field] == null) {
      continue
    }
