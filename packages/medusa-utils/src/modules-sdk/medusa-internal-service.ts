...
import { isPresent } from "../common/is-present"
import { isString } from "../common/is-string"
import { lowerCaseFirst } from "../common/lower-case-first"
import { MedusaError } from "../common/errors"
import { mergeMetadata } from "../common/merge-metadata"
import { FreeTextSearchFilterKeyPrefix } from "../dal/filter-keys"
import { DmlEntity } from "../dml/entity"
import { buildQuery } from "./build-query"
import { InjectManager } from "./decorators/inject-manager"
import { InjectTransactionManager } from "./decorators/inject-transaction-manager"
import { MedusaContext } from "./decorators/context-parameter"
import { MedusaInternalServiceSymbol } from "./medusa-internal-service-marker"
import { ContainerRegistrationKeys } from "../common/container"

export {
  isMedusaInternalService,
  MedusaInternalServiceSymbol,
} from "./medusa-internal-service-marker"
...
export function registerInternalServiceEventSubscriber(
  ...
  if (subscriber && persistenceAdapter?.registerEventSubscriber) {
    persistenceAdapter.registerEventSubscriber(context, subscriber)
  }
}
...
>(rawModel: any): InternalService {
    ...
    implements ModulesSdkTypes.IMedusaInternalService<TEntity, TContainer>
  {
    ...
    #eventSubscriber?: ModulePersistenceEventSubscriber
    readonly #persistenceAdapter?: ModulePersistenceAdapter
    ...
    constructor(container: TContainer) {
      ...
      const persistenceAdapter =
        container[ContainerRegistrationKeys.MODULE_PERSISTENCE_ADAPTER]
      this.#persistenceAdapter = isModulePersistenceAdapter(persistenceAdapter)
        ? persistenceAdapter
        : undefined
    }

    setEventSubscriber(subscriber: ModulePersistenceEventSubscriber) {
    ...
    static retrievePrimaryKeys(entity: EntityClass<any> | EntitySchema<any>) {
      if (DmlEntity.isDmlEntity(entity)) {
        const primaryKeys = Object.entries(entity.schema)
          .filter(
            ([fieldName, property]) =>
              (property as PropertyType<unknown>).parse(fieldName).primaryKey
          )
    ...
    async delete(
      ...
      if (deletedIds.length) {
        await Promise.all(
          deletedIds.map((id) =>
            this.#persistenceAdapter?.dispatchMutationEvent?.(
              "afterDelete",
              {
                entity: { id },
                meta: {
    ...
    async #dispatchPerformedActions(
