import {
  isPhysicalIndexBuildActiveInTransactionEffect,
...
import { appIndexPhysicalSpecSha256HexV1ToBytes } from "flarex-protocol/index-definition";
...
export interface LocatedAppOrderedIndexBuildTargetV1 extends LocatedReadCommittedAttemptTargetV1 {}
...
export interface AppOrderedIndexBuildPortsV1 {
  ...
  readonly authority: TrustedScopeAuthorityResolutionPorts<LocatedAppOrderedIndexBuildTargetV1>;
}
...
export class InvalidIntrinsicCreationTimeIndexBuildInputV1Error extends Data.TaggedError(
...
export class IntrinsicCreationTimeIndexDefinitionUnavailableV1Error extends Data.TaggedError(
...
export class IntrinsicCreationTimeIndexBuildStaleAuthorityV1Error extends Data.TaggedError(
...
export class IntrinsicCreationTimeIndexBuildStateV1Error extends Data.TaggedError(
...
export class IntrinsicCreationTimeIndexBuildIntegrationV1Error extends Data.TaggedError(
...
export class IntrinsicCreationTimeIndexBuildDecisionUncertainV1Error extends Data.TaggedError(
...
export class InvalidAppDeveloperOrderedIndexBuildInputV1Error extends Data.TaggedError(
...
export class AppDeveloperOrderedIndexDefinitionUnavailableV1Error extends Data.TaggedError(
...
export class AppDeveloperOrderedIndexBuildStaleAuthorityV1Error extends Data.TaggedError(
...
export class AppDeveloperOrderedIndexBuildStateV1Error extends Data.TaggedError(
...
export class AppDeveloperOrderedIndexBuildIntegrationV1Error extends Data.TaggedError(
...
export class AppDeveloperOrderedIndexBuildDecisionUncertainV1Error extends Data.TaggedError(
...
> {
  const decoded = yield* Effect.fromResult(
    decodeBuildInputResult(
      input,
      (reason) =>
        new InvalidIntrinsicCreationTimeIndexBuildInputV1Error({ reason }),
    ),
  );
...
> {
  const decoded = yield* Effect.fromResult(
    decodeBuildInputResult(
      input,
      (reason) =>
        new InvalidAppDeveloperOrderedIndexBuildInputV1Error({ reason }),
    ),
  );
  const located = yield* resolveLocatedTrustedScopeAuthorityEffect(
    decoded.deploymentId,
...
const runBuildTransaction = Effect.fn("AppOrderedIndexBuild.runTransaction")(
  function* (
  ...
  > {
    const started = startAppOrderedIndexBuildTransaction(target, (tx) =>
      buildInTransaction(
        tx,
        authority,
        definition,
        ...
        Effect.mapError((cause) =>
          cause instanceof AppIndexEntryWritePersistenceError ||
          cause instanceof AppIndexEntryReadPersistenceError
    ...
    const exit = yield* Effect.uninterruptible(
      Effect.exit(
        Effect.tryPromise({
          try: () => started.promise,
          catch: (cause) => cause,
...
interface StartedAppOrderedIndexBuildTransactionV1 {
  ...
  readonly callbackCause: () =>
    Cause.Cause<BuildTransactionErrorV1> | undefined;
}
...
function startAppOrderedIndexBuildTransaction(
  ...
  ) => Effect.Effect<AppOrderedIndexBuildResultV1, BuildTransactionErrorV1>,
...
const buildInTransaction = Effect.fn("AppOrderedIndexBuild.buildInTransaction")(
  function* (
    tx: AppRowTransaction,
    authority: TrustedScopeAuthority,
    definition: LocatedAppIndexDefinitionV1,
  ...
  > {
    ...
    yield* Effect.fromResult(
      requireAuthorityResult(authority, definition.indexDefinitionId, clock),
    );
    ...
    yield* Effect.fromResult(
      validateIndexBuildStateFrontierResult(state, clock.lastCommitSeq),
    );
    ...
    switch (state.lifecycle) {
      case "declared":
        yield* transitionLifecycle(tx, state, "building", null, options);
        return result(state, "advanced", "building", 0, 0, null);
      case "building":
        yield* transitionLifecycle(tx, state, "backfilling", null, options);
      ...
      case "enabled": {
        if (
          ...
          return yield* mismatch(
            state,
            "enabled definition has no authenticated coverage; explicit rebuild required",
          );
        }
        const latest = yield* readAppTableWriteFrontierInTransactionEffect(
          tx,
...
const backfillPage = Effect.fn("AppOrderedIndexBuild.backfillPage")(function* (
  tx: AppRowTransaction,
...
> {
  ...
  const cursorBytes =
    cursor === null ? null : orderedIndexRowIdHexV1ToBytes(cursor);
  const candidates = yield* queryEffect(
    tx
      .selectDistinctOn([fxAppRowRevisions.rowId], {
      ...
      .orderBy(asc(fxAppRowRevisions.rowId), desc(fxAppRowRevisions.commitSeq))
      .limit(pageSize + 1),
  );
  ...
  yield* transitionLifecycle(
    ...
    nextCursor,
    options,
    isDone ? lastCommitSeq : state.coveredThroughCommitSeq,
  );
  return result(state, "advanced", lifecycle, written, replayed, nextCursor);
});

const validateAndEnable = Effect.fn("AppOrderedIndexBuild.validateAndEnable")(
  function* (
    tx: AppRowTransaction,
    scopeUuid: ScopeUuidV1,
    state: IndexBuildStateRecord,
  ...
  > {
    ...
    const expectedRowIds = yield* queryEffect(
      tx
        .select({ rowId: fxAppRowCurrent.rowId })
    ...
    const indexRowIds = yield* queryEffect(
      tx
        .selectDistinct({ rowId: fxAppIndexEntryCurrent.rowId })
        ...
        .orderBy(asc(fxAppIndexEntryCurrent.rowId))
        .limit(pageSize + 1),
    );
    ...
    for (let index = 0; index < page.length; index += 1) {
      ...
      const actualRows =
        yield* readCurrentAppIndexEntriesForRowInTransactionEffect(tx, {
          scopeId: state.scopeId,
          definition,
          rowId,
        });
      ...
      const encodedKey = yield* projectIndexKey(definition, expectedRow, state);
      if (
        rowId !== actualRow.rowId ||
        actualRow.commitSeq > expectedRow.commitSeq ||
    ...
    const hasMore =
      expectedRowIds.length > pageSize ||
      indexRowIds.length > pageSize ||
      mergedRowIds.length > pageSize;
    if (hasMore) {
      yield* transitionLifecycle(tx, state, "validating", lastRowId, options);
      return result(state, "advanced", "validating", page.length, 0, lastRowId);
    }
...
const loadCurrentAppRow = Effect.fn("AppOrderedIndexBuild.loadCurrentAppRow")(
  function* (
    tx: AppRowTransaction,
    scopeUuid: ScopeUuidV1,
    definition: LocatedAppIndexDefinitionV1,
...
const ensureCurrentIndexEntry = Effect.fn(
  ...
  | AppendAppIndexEntryRevisionV1Error
> {
  const encodedKey = yield* projectIndexKey(definition, current, state);
  const rowId = yield* Effect.fromResult(
    orderedIndexRowIdHexV1FromBytesResult(
  ...
  const heads = yield* readAppIndexMembershipHeadsInTransactionEffect(tx, {
    scopeId: state.scopeId,
    positions: [{ definition, encodedKey, rowId }],
  }).pipe(
    Effect.mapError((cause) =>
      cause instanceof AppIndexEntryReadPersistenceError ||
      cause instanceof AppIndexEntryHashError
        ? new AppOrderedIndexBuildIntegrationError({
            phase: "targetTransaction",
            retryable: true,
            cause,
  ...
  if (head !== null) {
    if (
      head.commitSeq > current.commitSeq ||
      (head.isTombstone && head.commitSeq === current.commitSeq)
    ) {
      return yield* mismatch(
        state,
        "index history is ahead of or contradicts the current row",
      );
    }
    if (!head.isTombstone) return "replayed";
  }
  yield* appendBuiltAppIndexEntryRevisionInTransactionEffect(tx, {
    kind: "live",
    scopeId: state.scopeId,
...
function projectIndexKey(
  ...
  current: CurrentAppRowV1,
  state: IndexBuildStateRecord,
): Effect.Effect<OrderedIndexKeyHexV1, AppOrderedIndexBuildStateError> {
  ...
  return Effect.fromResult(
    ...
    Effect.mapError(
      (cause: OrderedIndexKeyTooLargeError) =>
        new AppOrderedIndexBuildStateError({
          scopeId: state.scopeId,
          indexDefinitionId: state.indexDefinitionId,
