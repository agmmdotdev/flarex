...
import {
  makePhysicalConnectionIdentity,
  type PhysicalConnectionIdentity,
  type PhysicalRestore,
  type PhysicalSessionPhase,
  type PhysicalSessionQuarantine,
  type PhysicalSessionErrors,
  type PhysicalInitialSettlement as InitialSettlement,
  type PhysicalRecoverySettlement as RecoverySettlement,
  type PhysicalSessionDriver as SessionDriver,
} from "./model";
export interface PhysicalPostgresPool {
...
export interface PostgresPhysicalSessionOptions {
  readonly lifecycleFault?: (
    input: Readonly<{
...
interface NormalizedPostgresPhysicalSessionOptions {
  readonly lifecycleFault: PostgresPhysicalSessionOptions["lifecycleFault"];
  readonly quarantineDrainTimeoutMilliseconds: number;
}
...
interface CheckedOutConnection {
  ...
  readonly identity: PhysicalConnectionIdentity;
...
/** Multiple pool/schema instances coexist; the caller owns the pool, each operation owns its checkout. */
export function makePostgresPhysicalSessionDriver<ResourceIssue, DeadlineIssue>(
  ownedPool: PhysicalPostgresPool,
  composition: {
    readonly access: (client: PoolClient) => {
      readonly database: FlarexMetadataDatabase;
      readonly transaction: FlarexMetadataTransaction;
    };
    readonly errors: PhysicalSessionErrors<ResourceIssue, DeadlineIssue>;
  },
  optionInput: PostgresPhysicalSessionOptions = {},
): SessionDriver<ResourceIssue> {
  type PhysicalSessionDriver = SessionDriver<ResourceIssue>;
  type PhysicalInitialSettlement<Value, Failure> = InitialSettlement<
    Value,
    Failure,
    ResourceIssue
  >;
  type PhysicalRecoverySettlement<Value, Failure> = RecoverySettlement<
    Value,
    Failure,
    ResourceIssue
  >;
  const deadlines = makePhysicalDeadlineOperations(composition.errors);
  /**
   * Build the bounded node-postgres physical-session adapter.
   *
   ...
   */
  function buildDriver(): PhysicalSessionDriver {
    const options = normalizePostgresPhysicalSessionOptions(optionInput);
    const runReadEffect: PhysicalSessionDriver["runReadEffect"] = Effect.fn(
      "PostgresPhysicalSession.read",
    )(
      <Value, Failure>(
        input: {
        ...
        Effect.uninterruptibleMask((restore) =>
          Effect.gen(function* () {
            const acquisition = yield* Effect.exit(
              restore(acquireConnection(ownedPool, input.deadline, options)),
            );
            ...
            const priorStatementTimeout =
              snapshot.value.rows[0]?.statement_timeout;
              ...
              return yield* failAfterQuarantine(
                ...
                Cause.fail(
                  resourceIssue(
                    "configureReadBudget",
                    new Error(
                      "PostgreSQL returned no statement_timeout setting.",
                    ),
                  ),
                ),
                options,
            ...
            const configure = yield* Effect.exit(
              restore(
                runRawQueryEffect(
                  connection,
                  input.deadline,
            ...
            const drain = yield* Effect.exit(
              runDrainEffect(connection, input.deadline, "read"),
            );
            if (Exit.isFailure(drain)) {
              ...
              return yield* failAfterQuarantine(connection, primary, options);
            }
            ...
            if (observedConnectionError !== undefined) {
              const primary = Cause.fail(
                resourceIssue("resetReadBudget", observedConnectionError),
              );
              return yield* failAfterQuarantine(
                connection,
                callbackCause === undefined
            ...
            if (release.kind === "failed") {
              const releaseCause = Cause.fail(
                resourceIssue("release", release.cause),
              );
              return yield* failAfterQuarantine(
                connection,
                callbackCause === undefined
    ...
    const runInitialTransactionEffect: PhysicalSessionDriver["runInitialTransactionEffect"] =
      Effect.fn("PostgresPhysicalSession.initialTransaction")(function* <
        Value,
        Failure,
      >(
        input: {
      ...
      ): Effect.fn.Return<PhysicalInitialSettlement<Value, Failure>, never> {
        const acquisition = yield* Effect.exit(
          restore(acquireConnection(ownedPool, input.deadline, options)),
        );
        if (Exit.isFailure(acquisition)) {
        ...
        const begin = yield* Effect.exit(
          restore(
            runRawQueryEffect(connection, input.deadline, "begin", "begin"),
          ),
        );
        ...
        const commit = yield* Effect.exit(
          runRawQueryEffect(connection, input.deadline, "commit", "commit"),
        );
    ...
    const runRecoveryTransactionEffect: PhysicalSessionDriver["runRecoveryTransactionEffect"] =
      Effect.fn("PostgresPhysicalSession.recoveryTransaction")(function* <
        Value,
        Failure,
      >(
        input: {
      ...
      ): Effect.fn.Return<PhysicalRecoverySettlement<Value, Failure>, never> {
        const acquisition = yield* Effect.exit(
          acquireRecoveryConnection(
            ownedPool,
            input.deadline,
    ...
    } satisfies PhysicalSessionDriver);
  }

  // Keep internal pipeline factories uninstrumented: per-query spans add
  // measurable short-transaction cost. Named lifecycle operations retain tracing.
  const acquireConnection = function acquireConnection(
    pool: PhysicalPostgresPool,
    deadline: PhysicalDeadline,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.Effect<CheckedOutConnection, ResourceIssue, never> {
    return remainingResourceMilliseconds(deadline, "acquire").pipe(
      Effect.flatMap((remainingMilliseconds) =>
        Effect.raceFirst(
          Effect.callback<CheckedOutConnection, ResourceIssue>((resume) => {
            let abandoned = false;
            let completed = false;
            const finish = (
              effect: Effect.Effect<CheckedOutConnection, ResourceIssue, never>,
            ) => {
              if (completed) return;
              completed = true;
              resume(effect);
            ...
            try {
              pool.connect((error, client) => {
                  ...
                  finish(
                    Effect.fail(
                      resourceIssue(
                        "acquire",
                        error ??
                          new Error("PostgreSQL returned no acquired client."),
                        cleanupCause,
                      ),
                    ),
                  );
                ...
                try {
                  finish(
                    Effect.succeed(
                      makeCheckedOutConnection(client, pool.options, options),
                    ),
                  );
                } catch (cause) {
                  const cleanupCause =
                    destroyAcquiredAfterConstructionFailure(client);
                  finish(
                    Effect.fail(resourceIssue("acquire", cause, cleanupCause)),
                  );
                }
              });
            } catch (cause) {
  ...
  const acquireRecoveryConnection = Effect.fn(
    "PostgresPhysicalSession.acquireRecoveryConnection",
  )(function* (
    pool: PhysicalPostgresPool,
    deadline: PhysicalDeadline,
    excludedIdentity: PhysicalConnectionIdentity,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<CheckedOutConnection, ResourceIssue> {
    while (true) {
      const connection = yield* acquireConnection(pool, deadline, options);
      if (connection.identity !== excludedIdentity) return connection;
      const discarded = yield* quarantineConnectionEffect(connection, options);
      if (discarded.kind === "failed") {
        return yield* Effect.fail(resourceIssue("quarantine", discarded.cause));
      }
    }
  });
  ...
  function makeCheckedOutConnection(
    ...
    const trackedQuery = (...args: readonly unknown[]): unknown => {
      if (!workFenceOpen) {
        return trackPromise(
          Promise.reject(
            new Error("PostgreSQL physical session callback work is closed."),
          ),
        );
      }
    ...
    const { database, transaction } = composition.access(trackedClient);
  ...
  const configureTransactionEffect = function configureTransactionEffect(
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    lockTimeoutMilliseconds: number,
  ): Effect.Effect<void, ResourceIssue, never> {
  ...
  const runRawQueryEffect = function runRawQueryEffect<
  ...
  >(
    ...
    text: string,
    values?: readonly unknown[],
  ): Effect.Effect<QueryResult<Row>, ResourceIssue, never> {
  ...
  const runCallbackEffect = function runCallbackEffect<
    Database,
    Value,
    Failure,
  >(
  ...
  ): Effect.Effect<Exit.Exit<Value, Failure>, ResourceIssue, never> {
  ...
  const runDrainEffect = function runDrainEffect(
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    phase: "read" | "callback",
  ): Effect.Effect<CheckedOutConnectionDrain, ResourceIssue, never> {
    return runBoundedPromiseEffect(deadline, phase, () => connection.drain());
  };

  function checkedOutConnectionDrainCause(
    drain: CheckedOutConnectionDrain,
    phase: "read" | "callback",
  ): Cause.Cause<ResourceIssue> | undefined {
  ...
  function checkedOutConnectionWorkFailureCause(
    ...
    phase: "read" | "callback",
  ): Cause.Cause<ResourceIssue> {
    return Cause.fail(resourceIssue(phase, failure.cause));
  }

  const runBoundedPromiseEffect = function runBoundedPromiseEffect<Value>(
    deadline: PhysicalDeadline,
    phase: PhysicalSessionPhase,
    start: (signal: AbortSignal) => Promise<Value>,
    connection?: CheckedOutConnection,
  ): Effect.Effect<Value, ResourceIssue, never> {
    return remainingResourceMilliseconds(deadline, phase).pipe(
      Effect.flatMap((remainingMilliseconds) =>
        Effect.raceFirst(
          Effect.callback<Value, ResourceIssue>((resume) => {
            const operationController = new AbortController();
            let completed = false;
            const finish = (
              effect: Effect.Effect<Value, ResourceIssue, never>,
            ) => {
              if (completed) return;
              completed = true;
              resume(effect);
  ...
  const remainingResourceMilliseconds = function remainingResourceMilliseconds(
    deadline: PhysicalDeadline,
    phase: PhysicalSessionPhase,
  ): Effect.Effect<number, ResourceIssue, never> {
    return deadlines
      .remaining(deadline, phase)
      .pipe(Effect.mapError((cause) => resourceIssue(phase, cause)));
  };

  const deadlineResourceFailure = function deadlineResourceFailure(
    deadline: PhysicalDeadline,
    phase: PhysicalSessionPhase,
  ): Effect.Effect<never, ResourceIssue, never> {
    return deadlines
      .fail(deadline, phase)
      .pipe(Effect.mapError((cause) => resourceIssue(phase, cause)));
  };
  ...
  function resourceIssue(
    ...
    return composition.errors.resource({
      phase,
      cause,
  ...
  const releaseConnectionEffect = function releaseConnectionEffect(
    connection: CheckedOutConnection,
    destroy: boolean,
    ...
    return Effect.sync(() => {
      if (connection.isReleased()) {
        return Object.freeze({
          kind: "failed",
          cause: new Error(
            "PostgreSQL control-session client was already released.",
          ),
        });
      }
  ...
  const quarantineConnectionEffect = Effect.fn(
    "PostgresPhysicalSession.quarantineConnection",
  )(function* (
    connection: CheckedOutConnection,
    options: NormalizedPostgresPhysicalSessionOptions,
    ...
    const quarantineExpiresAt =
      ...
      performance.now() + options.quarantineDrainTimeoutMilliseconds;
    ...
    const canAuthenticateCancellation =
    ...
    const earlyRelease =
      activeWorkRequiresDestruction && !cancellationConfirmed
    ...
    const release =
      earlyRelease ??
      (yield* releaseConnectionEffect(connection, true, "quarantine", options));
    const transportEnd =
      transportEndPromise === undefined
        ? undefined
  ...
  const boundedQuarantineDrainExitEffect =
  ...
  const boundedPlatformPromiseExitEffect =
  ...
  const failAfterQuarantine = function failAfterQuarantine<Failure>(
    connection: CheckedOutConnection,
  ...
  const notCommittedAfterQuarantine = function notCommittedAfterQuarantine(
    connection: CheckedOutConnection,
    primary: Cause.Cause<ResourceIssue>,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.Effect<PhysicalInitialSettlement<never, never>, never, never> {
    return Effect.map(
      quarantineConnectionEffect(connection, options),
      (quarantined) =>
        initialNotCommitted(
  ...
  const rollbackNotCommitted = Effect.fn(
    "PostgresPhysicalSession.rollbackNotCommitted",
  )(function* (
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    primary: Cause.Cause<ResourceIssue>,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<PhysicalInitialSettlement<never, never>, never> {
    connection.closeWorkFence();
    const rollback = yield* Effect.exit(
      runRawQueryEffect(connection, deadline, "rollback", "rollback"),
    );
    ...
    if (rollbackCause !== undefined) {
      ...
      return yield* notCommittedAfterQuarantine(connection, combined, options);
    }
    const release = yield* releaseConnectionEffect(
      connection,
      false,
  ...
  const settleCallbackFailure = Effect.fn(
    "PostgresPhysicalSession.settleCallbackFailure",
  )(function* <Failure>(
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    callbackCause: Cause.Cause<Failure>,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<PhysicalInitialSettlement<never, Failure>, never> {
    const rollback = yield* Effect.exit(
      runRawQueryEffect(connection, deadline, "rollback", "rollback"),
    );
    const rollbackCause = Exit.isFailure(rollback)
    ...
    if (rollbackCause !== undefined) {
      ...
      return Object.freeze({
        ...
        PhysicalInitialSettlement<never, Failure>,
  ...
  const initialUncertain = Effect.fn(
    "PostgresPhysicalSession.initialUncertain",
  )(function* <Value>(
    connection: CheckedOutConnection,
    value: Value,
    ...
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<PhysicalInitialSettlement<Value, never>, never> {
    const recoveryDeadline = yield* deadlines.start(
      "recovery",
      recoveryTimeoutMilliseconds,
    );
    const quarantine = yield* quarantineConnectionEffect(connection, options);
    const quarantineResult: PhysicalSessionQuarantine =
  ...
  function recoveryLifecycleUnresolved(
    cause: Cause.Cause<ResourceIssue>,
  ): PhysicalRecoverySettlement<never, never> {
  ...
  const recoveryLifecycleAfterQuarantine =
    function recoveryLifecycleAfterQuarantine(
      connection: CheckedOutConnection,
      primary: Cause.Cause<ResourceIssue>,
      options: NormalizedPostgresPhysicalSessionOptions,
    ): Effect.Effect<PhysicalRecoverySettlement<never, never>, never, never> {
      return Effect.map(
        quarantineConnectionEffect(connection, options),
        (quarantined) =>
          recoveryLifecycleUnresolved(
  ...
  const rollbackRecoveryLifecycle = Effect.fn(
    "PostgresPhysicalSession.rollbackRecoveryLifecycle",
  )(function* (
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    primary: Cause.Cause<ResourceIssue>,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<PhysicalRecoverySettlement<never, never>, never> {
    connection.closeWorkFence();
    const rollback = yield* Effect.exit(
      runRawQueryEffect(connection, deadline, "rollback", "rollback"),
    );
  ...
  const settleRecoveryCallbackFailure = Effect.fn(
    "PostgresPhysicalSession.settleRecoveryCallbackFailure",
  )(function* <Failure>(
    connection: CheckedOutConnection,
    deadline: PhysicalDeadline,
    callbackCause: Cause.Cause<Failure>,
    options: NormalizedPostgresPhysicalSessionOptions,
  ): Effect.fn.Return<PhysicalRecoverySettlement<never, Failure>, never> {
    const rollback = yield* Effect.exit(
      runRawQueryEffect(connection, deadline, "rollback", "rollback"),
    );
  ...
  function completeCallbackBoundaryCause<Failure>(
    boundaryCause: Cause.Cause<ResourceIssue>,
    settlement: Exit.Exit<unknown, Failure> | undefined,
  ): Cause.Cause<Failure | ResourceIssue> {
  ...
  function cleanupDefectCause(
    ...
    return Cause.die(composition.errors.cleanup({ phase, cause }));
  }
...
function connectionIdentity(client: PoolClient): PhysicalConnectionIdentity {
  const existing = connectionIdentities.get(client);
  if (existing !== undefined) return existing;
  const created = makePhysicalConnectionIdentity();
...
function initialNotCommitted<ResourceIssue>(
  cause: Cause.Cause<ResourceIssue>,
): InitialSettlement<never, never, ResourceIssue> {
...
function combineCausesWithoutDuplicateInterrupts<First, Second>(
  ...
  const newReasons = second.reasons.filter(
    (reason) =>
...
function aggregateCauses(causes: readonly unknown[], message: string): unknown {
  return causes.length === 1 ? causes[0] : new AggregateError(causes, message);
}

function normalizePostgresPhysicalSessionOptions(
  options: PostgresPhysicalSessionOptions,
): NormalizedPostgresPhysicalSessionOptions {
  const timeout =
