...
describe("additive metadata migration", () => {
  it("refuses corrupt canonical base-bearing plan bytes on cold reconstruction", async () => {
    ...
    await fixture.persistence.query("update fx_system_framework_migration_plan set canonical_bytes=canonical_bytes || decode('20','hex'), canonical_byte_length=canonical_byte_length+1 where frame_version=2");
  ...
  it("upgrades the previous head atomically and restores its schema on rollback", async () => {
    ...
    await db.transaction(async tx => { for (const migration of migrations.slice(0, -1)) for (const statement of migration.sql) await tx.exec(statement); });
