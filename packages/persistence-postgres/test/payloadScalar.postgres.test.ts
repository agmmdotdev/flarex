...
describe.skipIf(postgresUrl === null)("pinned Payload scalar Local API (PostgreSQL)", () => {
  ...
  it("proves supported Local API behavior, delete refusal and native settlement", () => withPersistence(persistence =>
    payloadScalarScenario(persistence, makePostgresRelationalSession(persistence), async ({ runtime, host, hostInput, inventory }) => {
      ...
      const duplicated = await Promise.allSettled([runEffect(host.run(sameKey, runtime.commands.create, input)), runEffect(host.run(sameKey, runtime.commands.create, input))]);
      ...
      const raced = await Promise.allSettled([runEffect(host.run(host.newRequestKey(), runtime.commands.create, race)), runEffect(host.run(host.newRequestKey(), runtime.commands.create, race))]);
      ...
      const recoverCalls = runtime.executions();
      const recoverKey = recovering.newRequestKey();
      const recoverInput = { data: { title: "native-recovery", publishedAt: "2026-01-01" } };
      const recovered = await runEffect(recovering.run(recoverKey, runtime.commands.create, recoverInput));
      expect(recovered).toMatchObject({ title: "native-recovery" });
      expect(runtime.executions()).toBe(recoverCalls + 1);
      expect(pids.length).toBeGreaterThanOrEqual(2);
      expect(pids[1]).not.toBe(failedPid);
      expect(await runEffect(recovering.run(recoverKey, runtime.commands.create, recoverInput))).toEqual(recovered);
      expect(runtime.executions()).toBe(recoverCalls + 1);
      const stable = await inventory();
      const pendingBefore = runtime.pendingReads();
      const fiber = Effect.runFork(host.run(host.newRequestKey(), runtime.commands.create, { data: { title: "id-unresolved", publishedAt: "2026-01-01" } }));
      try {
        const deadline = performance.now() + 5000;
        while (runtime.pendingReads() === pendingBefore && performance.now() < deadline) await new Promise(resolve => setTimeout(resolve, 10));
