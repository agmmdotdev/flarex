...
export function payloadScalarScenario(persistence: PGliteFlarexPersistence | PostgresFlarexPersistence, session: RelationalSession,
  ...
  return runEffect(Effect.scoped(makePayloadConformanceRuntime().pipe(Effect.flatMap(conformance => Effect.promise(async () => {
    ...
    expect(await runEffect(host.read(conformance.runtime.commands.find, {}))).toEqual({ docs: [], totalDocs: 0, limit: 10, totalPages: 1,
    ...
    expect(await runEffectFailure(mismatched.read(conformance.runtime.commands.find, {}))).toMatchObject({ reason: "invalidAuthority" });
    ...
    expect(await runEffect(host.read(conformance.runtime.commands.findByID, { id: created.id }))).toEqual(created);
    expect(await runEffect(host.read(conformance.runtime.commands.count, { where: { title: { equals: "first" } } }))).toEqual({ totalDocs: 1 });
    const updated = await runEffect(host.run(host.newRequestKey(), conformance.runtime.commands.update, { id: created.id, data: { title: "updated", score: 2, enabled: true } }));
    ...
    const invalid = await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.create, { data: { publishedAt: "2026-01-01" } }));
    ...
    const duplicate = await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.create, { data: { title: "updated", publishedAt: "2026-01-01" } }));
    ...
    await runEffect(host.run(host.newRequestKey(), conformance.runtime.commands.create, { data: { title: "nested-ok", publishedAt: "2026-01-01" } }));
    ...
    for (const title of ["nested-fail", "nested-caught"]) {
      await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.create, { data: { title, publishedAt: "2026-01-01" } }));
      expect(await inventory()).toEqual(nested);
    }
    expect(conformance.observations.hookRuns()).toBe(3);
    const page = await runEffect(host.read(conformance.runtime.commands.find, { limit: 1, page: 2 }));
    ...
    const deletion = await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.delete, { id: created.id }));
    expect(deletion).toMatchObject({ reason: "unsupportedProfile" });
    expect(await inventory()).toEqual(beforeDelete);
    expect(await runEffect(host.read(conformance.runtime.commands.findByID, { id: created.id }))).toEqual(updated);
    expect(await runEffect(host.read(conformance.runtime.commands.count, {}))).toEqual({ totalDocs: 3 });
    const unpaginated = await runEffect(host.read(conformance.runtime.commands.find, { limit: 1, page: 2, pagination: false }));
    ...
    for (const query of unsupportedQueries) await runEffectFailure(host.read(conformance.runtime.commands.find, query));
    ...
    expect(await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.update,
      { id: 1, data: null }))).toMatchObject({ reason: "unsupportedProfile" });
    expect(await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.update,
      { id: 1, data: {} }))).toMatchObject({ reason: "invalidInput" });
    expect(await runEffectFailure(host.run(host.newRequestKey(), conformance.runtime.commands.create,
      { data: { title: "unknown-field", extra: true } }))).toMatchObject({ reason: "unsupportedProfile" });
    expect(await runEffectFailure(host.read(conformance.runtime.commands.findByID,
      { id: 1, depth: 1 }))).toMatchObject({ reason: "unsupportedProfile" });
    expect(await runEffectFailure(host.read(conformance.runtime.commands.find,
      { where: null, limit: 0 }))).toMatchObject({ reason: "invalidInput" });
    expect(await runEffectFailure(host.read(conformance.runtime.commands.find,
      { where: { id: { in: [created.id] } } }))).toMatchObject({ reason: "invalidInput" });
    expect(await runEffectFailure(host.read(conformance.runtime.commands.count,
      { where: {}, depth: 0 }))).toMatchObject({ reason: "unsupportedProfile" });
    ...
    expect(await inventory()).toEqual(beforeDelete);
    const pending = await runEffect(host.run(host.newRequestKey(), conformance.runtime.commands.create, { data: { title: "id-pending", publishedAt: "2026-01-01" } }));
    ...
    await runEffect(Effect.scoped(Effect.gen(function* () {
      const ordinary = yield* makePayloadRuntime();
      ...
      for (const title of ["nested-fail", "id-foreign"]) {
        expect(yield* ordinaryHost.run(ordinaryHost.newRequestKey(), ordinary.commands.create,
          { data: { title, publishedAt: "2026-01-01" } })).toMatchObject({ title });
      }
    ...
    return async () => {
      ...
      expect(await runEffectFailure(host.read(conformance.runtime.commands.find, {}))).toMatchObject({ reason: "closed" });
