...
import { createPostgresLocatedReadCommittedTransactionRunnerV1 } from "../src/postgresLocatedReadCommitted";
  ...
  () => {
    ...
    it("pages the maximum exact identity and uses bounded owner indexes", async () => {
      await withTemporaryPostgresPersistence(async (persistence) => {
        ...
        await expect(
          incomingForeignKeys(persistence, "fx_app_index_entry_rev"),
        ).resolves.toEqual(["fx_app_index_entry_current_revision_fk"]);
        ...
        const first = await runEffect(
          compactRetainedIndexHistoryPageEffect(cleanup, deploymentId, {
            kind: "start",
          }),
        );
        ...
        expect(plans.anchor).toMatch(/fx_app_index_entry_rev_(?:range_idx|pk)/);
        ...
        const second = await runEffect(
          compactRetainedIndexHistoryPageEffect(cleanup, deploymentId, cursor),
        );
...
async function explainPlans(
  ...
  Readonly<Record<RetainedIndexHistoryCompactionQuery["name"], string>>
> {
