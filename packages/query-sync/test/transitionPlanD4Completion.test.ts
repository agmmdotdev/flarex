import { Result } from "effect";
import { describe, expect, it } from "vitest";

import {
  beginQueryEvaluation,
  capturePublicationAttemptInstant,
  completeQueryEvaluation,
  createEmptyQuerySyncState,
} from "@flarex/query-sync/internal/kernel";
import type {
  QueryState,
  QuerySyncState,
} from "@flarex/query-sync/internal/kernel";
import {
  planCompletePublication,
  resumeClaimPublicationPending,
  startClaimPublication,
} from "@flarex/query-sync/internal/transition-plan";
import type {
  AcceptedQueryPublicationEvidence,
  PublicationLifecycleFacts,
  PublicationOwnerQueryFacts,
} from "@flarex/query-sync/internal/transition-plan";
import {
  deriveGenerationRefreshEvidence,
} from "@flarex/query-sync/testing/reference-model";
import {
  admitAcceptedQueryPublicationAttempt,
  makeAcceptedQueryPublicationEvidenceForTesting,
} from "../src/transition-plan/PublicationWork.js";

import {
  cursor,
  descriptor,
  digest,
  evaluation,
  firstEvaluationRequest,
  getEvaluationAttempt,
  getSuccess,
  publicationArtifact,
  target,
} from "./fixtures.js";

function getFailure<A, E>(result: Result.Result<A, E>): E {
  return Result.match(result, {
    onFailure: (failure) => failure,
    onSuccess: () => {
      throw new Error("Expected a failed Result");
    },
  });
}

function instant(value: number) {
  return getSuccess(capturePublicationAttemptInstant(value));
}

function scopeFacts(state: QuerySyncState) {
  return {
    cursor: state.cursor,
    evaluationWork: state.evaluationWork,
    metrics: state.metrics,
  };
}

function ownerFacts(query: QueryState): PublicationOwnerQueryFacts {
  return {
    descriptor: query.descriptor,
    active: query.active === null
      ? null
      : {
        generation: query.active.generation,
        freshThroughSequence: query.active.freshThroughSequence,
        resultDigest: query.active.resultDigest,
      },
    currentCompletion: query.currentCompletion === null
      ? null
      : {
        identity: query.currentCompletion.identity,
        refreshedThroughSequence:
          query.currentCompletion.refreshedThroughSequence,
        resultDigest: query.currentCompletion.resultDigest,
        publicationDisposition:
          query.currentCompletion.publicationDisposition,
      },
  };
}

function stateWithPendingPublication(): QuerySyncState {
  const empty = getSuccess(createEmptyQuerySyncState(cursor()));
  const begun = getSuccess(beginQueryEvaluation(
    empty,
    firstEvaluationRequest(target({ descriptor: descriptor() })),
  ));
  const attempt = getEvaluationAttempt(begun);
  const evidence = evaluation({
    descriptor: attempt.descriptor,
    generation: attempt.generation,
    snapshot: begun.state.cursor.appliedThroughSequence,
  });
  const refresh = getSuccess(deriveGenerationRefreshEvidence(
    evidence,
    begun.state.cursor,
    [],
    evidence.authorityWitness,
  ));
  const completed = getSuccess(completeQueryEvaluation(
    begun.state,
    attempt,
    evidence,
    refresh,
    publicationArtifact("d4-complete"),
  ));
  if (completed._tag !== "completed") throw new Error("Expected completion");
  return completed.state;
}

function claimedFixture() {
  const state = stateWithPendingPublication();
  const publication = state.publicationWork.pending[0];
  const query = state.queries[0];
  if (publication === undefined || query === undefined) {
    throw new Error("Expected pending publication");
  }
  const started = getSuccess(startClaimPublication({
    scope: scopeFacts(state),
    lifecycle: state.publicationWork,
    capturedNow: instant(0),
  }));
  if (started.stage !== "pending") {
    throw new Error("Expected pending read");
  }
  const claimed = getSuccess(resumeClaimPublicationPending(started.resume, {
    publication,
    owner: ownerFacts(query),
  }));
  if (claimed._tag !== "write" || claimed.receipt._tag !== "claimed") {
    throw new Error("Expected claimed publication");
  }
  return Object.freeze({
    query,
    owner: ownerFacts(query),
    scope: claimed.nextScope,
    lifecycle: Object.freeze({
      ...state.publicationWork,
      inFlight: claimed.change.inFlight,
    }) satisfies PublicationLifecycleFacts,
    attempt: claimed.receipt.attempt,
  });
}

describe("QSYNC01-D4 complete-publication transition plans", () => {
  it("authenticates acceptance evidence before any field read", () => {
    const empty = getSuccess(createEmptyQuerySyncState(cursor()));
    let reads = 0;
    const rawForgery = new Proxy({}, {
      get: () => {
        reads += 1;
        throw new Error("Unauthenticated acceptance field read");
      },
    });
    // SAFETY: The forged nominal value is intentional adversarial test input;
    // the planner must reject it before dispatching any proxy field access.
    const forgery = rawForgery as AcceptedQueryPublicationEvidence;
    expect(getFailure(planCompletePublication({
      scope: scopeFacts(empty),
      lifecycle: empty.publicationWork,
      owner: null,
      evidence: forgery,
    }))).toMatchObject({
      _tag: "InvalidAcceptedPublicationEvidenceError",
      reason: "notStateIssued",
      queryKey: "",
      generation: 0n,
    });
    expect(reads).toBe(0);
  });

  it("clears exact in-flight work, installs latest delivered, and replays", () => {
    const fixture = claimedFixture();
    const evidence = admitAcceptedQueryPublicationAttempt(fixture.attempt);
    const completed = getSuccess(planCompletePublication({
      scope: fixture.scope,
      lifecycle: fixture.lifecycle,
      owner: fixture.owner,
      evidence,
    }));
    expect(completed).toMatchObject({
      _tag: "write",
      receipt: { _tag: "completed", identity: evidence.identity },
      nextScope: {
        metrics: {
          inFlightPublicationCount: 0,
          retainedPublicationContentBytes: 0,
          settlementEnvelopeBytes: 0,
        },
      },
      change: {
        _tag: "completeInFlightPublication",
        latestDelivered: {
          identity: evidence.identity,
          resultDigest: evidence.resultDigest,
        },
      },
    });
    if (completed._tag !== "write") throw new Error("Expected completion");
    const nextLifecycle: PublicationLifecycleFacts = {
      ...fixture.lifecycle,
      inFlight: null,
      latestDelivered: completed.change.latestDelivered,
    };
    const replay = getSuccess(planCompletePublication({
      scope: completed.nextScope,
      lifecycle: nextLifecycle,
      owner: fixture.owner,
      evidence,
    }));
    expect(replay).toEqual({
      _tag: "noWrite",
      receipt: { _tag: "replayed", identity: evidence.identity },
    });
    expect(Object.isFrozen(completed.change.latestDelivered)).toBe(true);
  });

  it("requires an owner only for matching lifecycle evidence", () => {
    const fixture = claimedFixture();
    const evidence = admitAcceptedQueryPublicationAttempt(fixture.attempt);
    expect(getFailure(planCompletePublication({
      scope: fixture.scope,
      lifecycle: fixture.lifecycle,
      owner: null,
      evidence,
    }))).toMatchObject({
      _tag: "QuerySyncTransitionFactError",
      reason: "publicationOwnerFactsInvalid",
    });

    const empty = getSuccess(createEmptyQuerySyncState(cursor()));
    const stale = getSuccess(planCompletePublication({
      scope: scopeFacts(empty),
      lifecycle: empty.publicationWork,
      owner: null,
      evidence,
    }));
    expect(stale).toEqual({
      _tag: "noWrite",
      receipt: { _tag: "superseded", identity: evidence.identity },
    });
  });

  it("rejects a wrong digest for exact current work", () => {
    const fixture = claimedFixture();
    const wrongDigest = makeAcceptedQueryPublicationEvidenceForTesting({
      identity: fixture.attempt.publication.identity,
      resultDigest: digest(250),
    });
    expect(getFailure(planCompletePublication({
      scope: fixture.scope,
      lifecycle: fixture.lifecycle,
      owner: fixture.owner,
      evidence: wrongDigest,
    }))).toMatchObject({
      _tag: "InvalidAcceptedPublicationEvidenceError",
      reason: "resultDigestMismatch",
    });
  });

  it("rejects lifecycle counters that cannot cover completion subtraction", () => {
    const fixture = claimedFixture();
    const evidence = admitAcceptedQueryPublicationAttempt(fixture.attempt);
    const corruptMetrics = [
      {
        ...fixture.scope.metrics,
        retainedPublicationContentBytes: 0,
      },
      {
        ...fixture.scope.metrics,
        countedCanonicalBytes: 0,
      },
    ];
    for (const metrics of corruptMetrics) {
      expect(getFailure(planCompletePublication({
        scope: { ...fixture.scope, metrics },
        lifecycle: fixture.lifecycle,
        owner: fixture.owner,
        evidence,
      }))).toMatchObject({
        _tag: "QuerySyncTransitionFactError",
        operation: "completePublication",
        reason: "publicationLifecycleFactsInvalid",
      });
    }
  });
});
