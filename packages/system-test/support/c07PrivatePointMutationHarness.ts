...
import type { FlarexMetadataDatabase } from "@flarex/persistence-postgres/internal/system-test/deployments";
...
} from "@flarex/persistence-postgres/internal/system-test/storedCommitAuthorityEvidence";
import {
...
import {
  ...
  setC07ActivationClockV1,
} from "./c07PointMutationFixtureV1";
import { issueSetupSeededSyscallValidatorProofV1 } from
  "./setupSeededSyscallValidatorProofV1";
import {
  runSystemTestEffectV1,
  runSystemTestEffectFailureV1,
} from "./systemTestEffectBoundaryV1";
...
export async function executePrivateRegisteredRevisionPointMutationThroughC07V1(
  ...
  const admitted = await runSystemTestEffectV1(createPointMutationStartAdmissionV1({
  ...
  const coldOutcome = await runSystemTestEffectV1(
...
export async function proveC07PrivatePointMutationCorrectnessV1(
  ...
  await setC07ActivationClockV1(lane.persistence, scopeId);
  ...
  const runtimeBinding = Object.freeze({
    run: async (
      ...
      if (runtimeExecutions === 1) {
        const competingResult = await runSystemTestEffectV1(
  ...
  const clonedActivationFailure = await runSystemTestEffectFailureV1(
    execution.executeInitialPointMutationAttempt(
      Object.freeze({ ...activated }),
    ),
  );
  const firstResult = await runSystemTestEffectV1(
    execution.executeInitialPointMutationAttempt(activated),
  );
  if (firstResult.kind === "expired") {
    throw new Error("C07 unexpectedly expired its committed result.");
  }
  const consumedActivationFailure = await runSystemTestEffectFailureV1(
    execution.executeInitialPointMutationAttempt(activated),
  );
  const scopeUuid = projectScopeIdUuidV1(scopeId).scopeUuid;
