import {
  createApplicationRelationQueryPGliteSystemTestFixture,
} from
...
describe("RQ01 private Standard relation query - PGlite", () => {
  it("reads the exact logical relation without Worker or mutation authority", async () => {
    ...
    const proof = await proveApplicationRelationQuery(() =>
      createApplicationRelationQueryPGliteSystemTestFixture(persistence)
    );
