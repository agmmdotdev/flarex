...
describe("AA-R6 checkpoint 5d3 Application Task System", () => {
  it("creates, durably replays, discovers, and accepts one Application Worker launch", async () => {
    ...
    const locatedRunTarget = createPGliteLocatedTaskSystemRunAttemptTargetV1(
      fixture.target,
    ...
    const created = await Effect.runPromise(
      createApplicationTaskRun(TASK_ID, request).pipe(
        Effect.provide(applicationTaskSystem),
      ),
    );
    const exactReplay = await Effect.runPromise(
      createApplicationTaskRun(TASK_ID, request).pipe(
        Effect.provide(applicationTaskSystem),
      ),
    );
    ...
    const pinnedReplay = await Effect.runPromise(
      createApplicationTaskRun(TASK_ID, request).pipe(
        Effect.provide(applicationTaskSystem),
      ),
    );
    ...
    const started = await Effect.runPromise(lifecycle.transactRunAttempt({
    ...
    const delivery = await Effect.runPromise(Effect.scoped(
      Effect.gen(function* () {
