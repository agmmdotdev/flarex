...
- Resolution is intentionally read-only and cannot make the control/target
  ...
  binds it only after an in-transaction clock recheck. O06 revalidates its
  epoch/generation fence inside the rollback-proven kernel; O07-B reuses that
  revalidation inside the first durable publication transaction.
...
- O06 owns the reusable revalidation/OCC/tentative-lowering kernel under forced
  rollback. O07-B now owns sequence advancement, commit/change publication,
  outcomes, idempotency, and outbox in one durable transaction.
