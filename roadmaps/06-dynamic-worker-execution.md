...
- Removed throwing execution request payload wrappers from
...
- Kept internal ExecutionDO and public Worker execution route boundaries on
...
Execution route boundaries already consume typed Effect decoders. The throwing
payload parser wrappers were compatibility exports that made the shared
execution request source boundary look partly exception-based even though
production route paths were already Effect-based.
...
- Flarex runs dynamic execution through Cloudflare public Worker routes and
  ExecutionDO session routes. This checkpoint does not change those semantics;
  it only removes legacy throwing payload wrappers from the shared request
  boundary.

Known limitations:

- ExecutionDO routing/session lifecycle, public Worker execution dispatch,
...
- Shared invoke helpers now model execution document syscall validation for
  get, insert, patch, replace, and delete as typed Effect failures.
- ExecutionDO syscall routes now use those helpers for document id parsing,
  table lookup, document validator checks, placement checks, and missing patch
  targets.
- Actual SingleShardTransaction reads/writes still run through
  `ExecutionRouteOperationError`.

Why it changed:

Document syscall validation is invoke-domain validation, not a generic
execution operation failure. This checkpoint keeps document validation failures
typed until the ExecutionDO adapter while preserving transaction IO failures in
the existing operation channel.

Known limitations:

- Indexed query syscall planning remains on the older reader/query path and is
  the next natural syscall validation slice.
- Execution start, finish, abort, public execution dispatch, PartitionDO
  SQL/OCC, protocol schemas, executor-http, DeploymentDO, SchedulerDO,
  DeliveryDO, ConnectionDO, and `ValidatorJson` behavior are unchanged.
...
- ExecutionDO start now loads active deployment/function metadata through
  `loadActiveFunctionMetadataEffect(...)` instead of the compatibility throwing
  `loadActiveFunctionMetadata(...)` wrapper inside
  `routeExecutionOperation("start", ...)`.
- `InvokeActiveDeploymentLoadError` and
  `InvokeActiveFunctionMetadataNotFoundError` are part of the ExecutionDO
  service error channel and map once at the internal route adapter edge.
- SingleShardTransaction schema setup and transaction begin failures remain in
  `ExecutionRouteOperationError`.

Why it changed:

Active deployment and active function metadata lookup are invoke-runtime
boundary failures, not transaction operation failures. This checkpoint keeps
those failures in typed invoke channels until the ExecutionDO response adapter,
while preserving the public `/executions/start` response status and body.
...
- ExecutionDO start now resolves partition scope through
  `resolveFunctionExecutionScopeEffect(...)` instead of a synchronous
  `Effect.try(...)` wrapper.
- `InvokePartitionValidationError` and `InvokeTableNotFoundError` are part of
  the ExecutionDO service error channel and map to the existing invoke
  validation HTTP responses at the internal route adapter edge.
- SingleShardTransaction schema setup and transaction begin failures remain in
  `ExecutionRouteOperationError`.

Why it changed:

Partition scope resolution is invoke-domain validation. Routing those failures
through `ExecutionRouteOperationError` blurred domain validation with
transaction operation failures. This checkpoint keeps scope validation typed
until the adapter response mapping edge and reserves route-operation errors for
actual async transaction work.
