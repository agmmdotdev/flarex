...
- `PartitionDO` now hydrates persisted storage JSON through
  `partition/StorageRows.ts` Effect decoders instead of local untyped
  `JSON.parse(...) as ...` casts.
- The decoded row families are idempotency commit responses, subscription read
  sets, table placement, table validators, write-log committed writes,
  write-log index writes, document JSON values, and index fields.
- The synchronous wrappers are local compatibility bridges for the existing
  Durable Object SQLite call sites; the reusable decoders return typed Effects
  and expose `PartitionStorageJsonError`.

Why it changed:

S-1 moves persisted JSON validation to the storage row boundary while keeping
PartitionDO transaction behavior stable. Invalid persisted blobs now fail as a
tagged storage-boundary error instead of silently entering OCC, subscription,
or document/index logic through unchecked casts.

Convex comparison:

Convex keeps equivalent read-set and write-log state typed in
`crates/database/src/reads.rs`, `crates/database/src/write_log.rs`, and
`crates/database/src/committer.rs`. Flarex's Cloudflare implementation stores
those shapes in Durable Object SQLite JSON columns, so runtime validation must
decode when rows are read.

Known limitations:

- This checkpoint does not move PartitionDO transaction/OCC logic into a
  service layer.
- This checkpoint does not change public Worker routing, deployment storage,
  executor-http, RegistryDO, SchedulerDO, DeliveryDO, or `ValidatorJson`
  semantics.
...
- Public deployment route failures now convert directly to `Response` inside
  `publicWorkerDeploymentRouteErrorToResponseEffect(...)`.
- The intermediate public deployment
  `publicWorkerDeploymentRouteErrorToHttpErrorEffect(...)` was removed.
- `PublicWorkerDeploymentNonInvokeRouteError` no longer includes `HttpError`;
  deployment route failures stay typed until the Worker response adapter edge.
- `PublicWorkerDispatchError` remains a typed dispatch failure and is converted
  to a response only inside the public deployment Worker adapter branch.
- The concrete migration checklist ticks `P-2` and sets `P-3` as the next
  active checkpoint.

Why it changed:

P-1 typed the public deployment push route inputs. P-2 moves the public
deployment Worker error path closer to the target shape: typed failures flow to
one response mapper at the route adapter edge instead of failing through an
intermediate `HttpError` effect.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex public
  Worker response-adapter migration checkpoint.

How Flarex differs:

- Flarex still uses shared `HttpError` compatibility helpers for other Worker
  route families. The public deployment branch now converts to responses at its
  own adapter edge while preserving existing response JSON.

Known limitations:

- Shared Worker-level `HttpError` cleanup remains for later Worker route error
  phases. Public start/finish artifact service response schemas, generated
  handler logic, DeploymentService/store lifecycle logic, artifact
  materializer/cache semantics, source analyzer behavior, PartitionDO SQL/OCC,
...
- `routeDeploymentDurableObject(...)` now decodes DeploymentDO API routes and
  dispatches all read and mutation inputs directly through typed Effect route
  inputs.
- The generated request compatibility adapter and request rebuilding helper APIs
  were removed from production DeploymentDO routing.
- `DeploymentDO.fetch(...)` no longer constructs a generated HttpApi web
  handler; it provides the deployment layer to the direct route effect at the
  Durable Object runtime edge.
- `HttpApiWebHandler` remains only as generated handler integration coverage.
- The concrete migration checklist ticks `D-6` and sets `P-1` as the next
  active checkpoint.

Why it changed:

D-4 and D-5 proved direct mutation and read dispatch. D-6 removes the now-dead
request bridge from production routing so the DeploymentDO runtime boundary has
one direct Effect route path instead of a decoded-input-to-rebuilt-request
compatibility loop.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentDO route-boundary cleanup checkpoint.

How Flarex differs:

- Flarex still keeps generated HttpApi handler effects and protocol schemas for
  integration coverage. The Durable Object adapter no longer depends on the
  generated web-handler request bridge for production dispatch.
...
- Generated handler logic, DeploymentService/store lifecycle logic, public
  ...
  analyzer semantics, PartitionDO SQL/OCC, executor-http, protocol parser
  compatibility wrappers, and `ValidatorJson` are unchanged.
...
- `DeploymentApiRouteInput` now distinguishes health, active deployment, and
  get-push read route inputs instead of collapsing them into a generic read
  route.
- `dispatchDeploymentApiReadRouteInputDirect(...)` maps typed read route inputs
  through existing generated handler effects and converts success/error values
  to HTTP `Response` objects at the internal route edge.
- `routeDeploymentDurableObject(...)` now direct-dispatches all DeploymentDO
  API reads and mutations without rebuilding generated HttpApi requests.
- The concrete migration checklist ticks `D-5` and sets `D-6` as the next
  active checkpoint.

Why it changed:

D-4 wired mutation routes directly. D-5 removes read-route dependence on the
generated request bridge while preserving health, active deployment, push read,
not-found, and storage-error response semantics.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  Effect route-boundary migration checkpoint.

How Flarex differs:

- Flarex still keeps generated HttpApi handlers as reusable Effect functions.
  The Durable Object route adapter now calls those handler effects directly
  instead of using the web-handler request bridge.

Known limitations:

- The compatibility adapter and generated web handler still exist for tests and
  the next D-6 cleanup decision. Generated handler logic,
...
- `routeDeploymentDurableObject(...)` now routes typed start/finish/abandon
  mutation inputs directly to `dispatchDeploymentApiMutationRouteInputDirect(...)`.
- `DeploymentDO.fetch(...)` provides the existing deployment layer at the DO
  runtime edge so mutation dispatch can use `DeploymentService`.
- Read routes remain on the generated HttpApi request compatibility bridge.
- The concrete migration checklist ticks `D-4` and sets `D-5` as the next
  active checkpoint.

Why it changed:

The previous checkpoint proved direct mutation dispatch in isolation. This
checkpoint wires that behavior into the production DeploymentDO route path while
preserving generated read-route compatibility.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  Effect route-boundary wiring checkpoint.

How Flarex differs:

- Flarex still has a generated Effect HttpApi web-handler bridge. Mutation
  routes now bypass the bridge, but read routes intentionally keep it until the
  D-5 direct read mapping checkpoint.

Known limitations:

- DeploymentDO direct read mapping, generated handler logic,
  DeploymentService/store lifecycle logic, public Worker deployment dispatch,
  artifact materializer/cache, source-package analyzer semantics, PartitionDO
  SQL/OCC, executor-http, protocol parser compatibility wrappers, and
  `ValidatorJson` are unchanged.
...
- Added `dispatchDeploymentApiMutationRouteInputDirect(...)`, a named Effect
  adapter for typed start/finish/abandon route inputs.
- The direct adapter calls the existing generated deployment handler effects
  with `DeploymentServiceApi` instead of rebuilding a generated HttpApi
  `Request`.
- The adapter maps generated handler success/error values to HTTP `Response`
  objects at one edge, preserving declared status semantics.
- Focused tests cover direct start success, rejected finish `409`, and abandon
  conflict `409`.

Why it changed:

The route surface now has typed decoded mutation inputs. This checkpoint proves
those inputs can dispatch through generated handler effects directly without
changing the Durable Object runtime wiring yet.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentDO direct-dispatch migration checkpoint.

How Flarex differs:

- Flarex still keeps the generated HttpApi request bridge wired in
  `routeDeploymentDurableObject(...)`, but the mutation direct-dispatch
  behavior is now implemented and testable before the runtime wiring decision.

Known limitations:

- `routeDeploymentDurableObject(...)` wiring, direct read dispatch, generated
  handler logic, DeploymentService/store lifecycle logic, public Worker
  deployment dispatch, artifact materializer/cache, source-package analyzer
  semantics, PartitionDO SQL/OCC, executor-http, protocol parser compatibility
  wrappers, and `ValidatorJson` are unchanged.
...
- `routeDeploymentDurableObject(...)` now decodes typed
  `DeploymentApiRouteInput` values directly.
- The generated HttpApi web-handler bridge is isolated in
  `dispatchDeploymentApiRouteInputViaRequestCompatibility(...)`.
- The compatibility dispatch adapter converts typed route inputs back to
  canonical `Request` values only at the generated HttpApi compatibility edge.
- Focused tests cover compatibility dispatch success, generated protocol
  failure propagation, and generated handler operation failure mapping.

Why it changed:

The previous checkpoint introduced typed DeploymentDO route inputs but still
hid them behind a `Request`-returning compatibility decoder. This checkpoint
makes the Durable Object route adapter operate on typed route input first,
while preserving the generated HttpApi web-handler runtime path.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentDO route-boundary migration checkpoint.

How Flarex differs:

- Flarex still uses generated Effect HttpApi for DeploymentDO route handling.
  The request bridge remains, but it is now explicit and isolated so the next
  direct-dispatch decision can be made against a typed boundary.

Known limitations:

- Direct generated handler dispatch, generated handler logic,
  DeploymentService/store lifecycle logic, public Worker deployment dispatch,
  artifact materializer/cache, source-package analyzer semantics, PartitionDO
  SQL/OCC, executor-http, protocol parser compatibility wrappers, and
  `ValidatorJson` are unchanged.
...
- DeploymentDO API route decoding now has a typed `DeploymentApiRouteInput`
  union for read routes, analyzed start-push bodies, finish-push bodies, and
  abandon-push bodies.
- `decodeDeploymentApiRouteInput(...)` validates mutation request bodies into
  typed route input values before compatibility conversion.
- `decodeDeploymentApiRequestForRoute(...)` remains the generated HttpApi web
  handler compatibility adapter by turning typed route inputs back into
  canonical `Request` values.
- Route-boundary tests now assert the typed route input payloads and push ids
  before request compatibility conversion.

Why it changed:

The migration is moving the Durable Object route surface away from request-only
adapter plumbing and toward typed decoded inputs. This checkpoint creates that
typed boundary without changing generated HttpApi runtime behavior or public
HTTP responses.
...
- Flarex currently routes DeploymentDO requests through Effect HttpApi. The new
  typed route input preserves the generated-handler compatibility path while
  preparing a future direct typed dispatch decision.
...
- Analyzed start-push service responses now flow through
  `deploymentStartAnalyzedPushResponseForHttpApi(...)`.
- The start response adapter accepts already validated service input, maps
  typed service failures, and validates the declared push-status response.
- The generated start handler now composes input validation with the response
  adapter while preserving declared bad-request and storage error channels.
- Direct adapter tests cover success, validation failure, storage failure, and
  malformed service responses.

Why it changed:

The generated HttpApi start handler was the last deployment mutation handler
with service response mapping inline. This checkpoint completes the generated
mutation adapter batch by keeping input validation and service-response
validation as separate typed Effect boundaries.
...
- Flarex starts deployment pushes from analyzed source-package metadata. The
  generated HttpApi adapter validates that response before it leaves the
  Durable Object route surface.
...
- Public Worker `/deployments/...` routing now catches deployment route
  failures through `publicWorkerDeploymentRouteErrorToResponseEffect(...)`.
- Worker-level deployment route HTTP conversion now uses
  `publicWorkerDeploymentRouteErrorToHttpErrorEffect(...)`.
- The Worker adapter reuses named mapper effects for deployment request
  JSON/protocol failures, dispatch failures, execution start failures, and
  partition payload failures where those mappers already exist.

Why it changed:

Deployment route request decoding already returns typed Effect failures. The
Worker branch now keeps those failures typed until a named adapter edge maps
them to HTTP responses, instead of doing inline `Effect.matchEffect(...)`
conversion in the main route branch.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  public Worker route adapter cleanup.

How Flarex differs:

- Flarex has a public Worker that fans deployment routes into Durable Objects,
  execution routes, partition routes, and sync routes. This adapter is
  Flarex-specific and keeps the public Worker boundary explicit.

Known limitations:

- DeploymentDO generated handler behavior, DeploymentService/store lifecycle
  logic, artifact materializer/cache, source-package analyzer semantics,
...
- Removed PartitionDO route `read*` request compatibility wrappers for
  schema-cache, commit, subscription registration, subscription target, and
  connection unregister bodies.
- Removed throwing partition route `parse*` compatibility wrappers and
  forwarding `parse*Effect` aliases.
- Removed the public Worker schema-cache `read*` and throwing `parse*`
  compatibility wrappers.
- Kept PartitionDO and public Worker request handling on Effect decoders and
  typed route errors.

Why it changed:

Partition route runtime validation already uses typed Effect decoders for JSON
body reads and payload validation. Removing the Promise/throwing wrappers keeps
partition route failures typed until the Durable Object or Worker adapter edge.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  runtime validation cleanup around existing PartitionDO and public partition
  route decoders.

How Flarex differs:

- Flarex exposes partition routes through Cloudflare Durable Objects and a
  public Worker forwarding path. This checkpoint preserves that host shape
  while narrowing request decoding to typed Effect paths.

Known limitations:

- PartitionDO SQL/OCC behavior, transaction response shapes, public Worker
  partition dispatch, DeploymentDO, RegistryDO, executor-http, protocol
  package parser compatibility, and `ValidatorJson` are unchanged.
...
- Removed throwing execution request payload compatibility wrappers from
  `execution/Requests.ts`.
- Removed the matching execution protocol parser imports from
  `execution/Requests.ts`.
- Kept production internal and public execution routes on
  `decodeExecution*Payload(...)` and `decodePublicExecution*Payload(...)`
  Effect decoders.
- Updated execution request tests to assert typed decoder success and
  `ExecutionProtocolValidationError` failures directly.

Why it changed:

The execution route boundaries already use Effect decoders for start, syscall,
finish, and public action payloads. Removing the throwing compatibility
wrappers keeps execution protocol validation failures typed until the
route/Worker adapter edge.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  execution request-boundary cleanup around existing protocol decoders.

How Flarex differs:

- Flarex uses public Worker dispatch plus ExecutionDO session routes for
  dynamic execution. This checkpoint narrows only the shared execution payload
  source boundary while preserving session and dispatch behavior.

Known limitations:

- This checkpoint does not change ExecutionDO routing/session lifecycle, public
  Worker execution dispatch, syscall/finish semantics, direct invoke, artifact
  runtime dispatch, PartitionDO SQL/OCC, executor-http, or `ValidatorJson`.
...
- Removed Promise-returning scheduler maintenance request wrappers from
  `scheduler/RouteBoundary.ts`.
- Removed public throwing scheduler maintenance route parse compatibility
  wrappers and the matching `parseScheduler*RequestEffect(...)` aliases.
- Removed the private `runSchedulerRouteEffect(...)` compatibility runner.
- Kept SchedulerDO production routing on
...
- Updated scheduler route-boundary tests to assert typed `RequestJsonError`
  and `SchedulerRoutePayloadError` channels directly before adapter mapping
  assertions.
...
- No Convex source files were inspected for this slice. This is a Flarex
  SchedulerDO adapter cleanup around existing scheduler maintenance decoders.

How Flarex differs:

- Flarex sync maintenance is split across Cloudflare Durable Object routes for
  delivery reconciliation, connection cleanup, subscription reruns, and
  dead-letter handling. This checkpoint narrows only those request boundaries
  while preserving scheduler runtime behavior.
...
- Removed public Promise-returning request wrappers from
  `invoke/PublicInvokeRouteBoundary.ts`,
  `delivery/PublicWakeRouteBoundary.ts`, and
  `liveQueryDelivery/RouteBoundary.ts`.
- Removed public throwing route-level parse wrappers from the same invoke,
  delivery wake, and live query delivery boundaries.
- Kept public Worker routing on Effect decoders:
...
The public Worker already uses typed Effect decoders for invoke, delivery wake,
and live query delivery routes. The remaining public Promise/throw wrappers
were compatibility surfaces exercised by tests rather than production routing.
Removing them keeps the boundary aligned with the migration quality bar:
request/body decoders return typed failures, and HTTP conversion stays at the
named route adapter edge.

Convex references:

- No Convex source files were inspected for this slice. This is a
  Cloudflare-specific public Worker adapter cleanup around existing Flarex
  protocol and delivery decoders.

How Flarex differs:

- Flarex keeps public invoke, delivery wake, and live query delivery traffic
  split across Cloudflare Worker routes, Durable Object dispatch, and
  route-specific request decoders. The checkpoint narrows the Worker adapter
  surface without changing the Cloudflare dispatch topology.

Known limitations:

- This checkpoint does not change public dispatch behavior, DeliveryDO
  behavior, live query delivery application behavior, invoke execution
  dispatch, PartitionDO SQL/OCC, executor-http, deployment behavior, or
  `ValidatorJson`.
- Lower-level payload parser compatibility wrappers remain until those package
  boundaries are selected directly.
...
- Removed unused Promise-returning public deployment push request wrappers from
  `deployment/PublicPushRouteBoundary.ts`.
- Removed unused throwing `parsePublic*PushRequest(...)` and
  `parsePublic*PushRequestEffect(...)` compatibility wrappers from the public
  deployment push route boundary.
- Kept the Worker-facing boundary on Effect decoders:
  `decodePublic*PushRequest(...)`, `decodePublic*PushJson(...)`, and
  `decodePublic*PushRoutePayload(...)`.
- Removed the redundant JSON-only adapter effect; public deployment route
  errors now share `publicDeploymentRouteErrorToHttpErrorEffect(...)`.
- Updated route-boundary tests to assert typed `RequestJsonError` and
  `DeploymentProtocolValidationError` channels directly before checking the
  route-level HTTP adapter mapping.

Why it changed:

The public deployment Worker routes had already moved to Effect decoders, but
the route-boundary module still exported unused Promise/throw compatibility
helpers. Removing those helpers makes the boundary match the migration quality
bar: typed request/body decoders first, and HTTP conversion at one adapter
edge.

Convex references:

- No Convex source files were inspected for this slice. This is a
  Cloudflare-specific Worker adapter cleanup around already-established
  Flarex deployment protocol decoders.

How Flarex differs:

- The public push route still keeps Cloudflare-specific analyzer and artifact
  preflight ordering in `worker.ts`: start-push reads JSON before analyzer
  availability checks, and finish-push reads JSON before artifact preflight,
  then both decode the protocol payload through typed Effect decoders.

Known limitations:

- This checkpoint does not change analyzer preflight, artifact preflight,
  public deployment dispatch, DeploymentDO/DeploymentService/store behavior,
  executor-http, PartitionDO SQL/OCC, or `ValidatorJson`.
- Synchronous parser compatibility wrappers remain in lower-level request
  modules and unrelated route boundaries until those boundaries are selected.
...
- Removed unused private result-returning wrappers around deployment validation
  internals.
- Kept the active validation path on named Effect decoders such as
  `decodeSchema(...)`, `decodeFunctions(...)`, `decodeAnalysis(...)`,
  `decodeCodegenAnalysis(...)`, and `decodePushStatusFromRow(...)`.
- Preserved public compatibility wrappers that still throw or return
  `DeploymentValidationResult` for existing callers.

Why it changed:

Deployment validation had already moved most active validation work to typed
Effect decoders, but stale private result adapters made the module look like it
still had parallel result-style domain flow. Removing those unused wrappers
keeps typed `DeploymentValidationError` propagation as the only active internal
validation path while leaving compatibility edges explicit.

Known limitations:

- This checkpoint does not change deployment SQL writes, active deployment
  metadata, push lifecycle service behavior, generated DeploymentDO HttpApi
  handlers, public Worker routing, PartitionDO SQL/OCC, executor-http, or
  `ValidatorJson`.
...
- Public invoke route request failures now recover through a named Effect HTTP
  adapter helper before returning `HttpError`.
- Top-level `/invoke` Worker response mapping now has a named Effect adapter
  while preserving `invokeErrorResponse(...)` bodies for route, execution, and
  artifact-dispatch failures.
- Reusable public Worker dispatch failures now expose a named Effect HTTP
  adapter helper, and top-level registry/scheduler pass-through route mapping
  uses named adapter effects.

Why it changed:

The top-level public Worker is the final HTTP edge for `/invoke`, registry
deployments, and public scheduler maintenance routes. Naming those adapters
keeps route/protocol/dispatch failures typed until the Worker edge that owns
HTTP response conversion, without changing the execution or Durable Object
runtime behavior behind those routes.

Known limitations:

- This checkpoint does not change invoke execution internals, artifact runtime
  invoke dispatch, deployment-scoped invoke response compatibility,
  SchedulerDO maintenance, RegistryDO behavior, PartitionDO SQL/OCC,
  executor-http, or `ValidatorJson`.
...
- Delivery wake request route errors now recover through named Effect adapter
  helpers before returning `HttpError`.
- Public delivery wake route mapping exposes a named adapter effect for typed
  request JSON and wake payload failures.
- Public DeliveryDO wake forwarding now runs through a named service-binding
  helper with the preserved `delivery-wake` failure source.

Why it changed:

Delivery wake is a route-service path: request JSON decoding, wake payload
validation, public Worker authorization, deployment-id overlay, and DeliveryDO
dispatch all meet at the route boundary. Naming the adapters and dispatch
helper keeps failures typed until the edge that owns conversion.

Known limitations:

- This checkpoint does not change public Worker route matching, DeliveryDO
  drain behavior, SchedulerDO wake scheduling, ConnectionDO/live-query fanout,
  PartitionDO SQL/OCC, executor-http, or `ValidatorJson`.
...
- Public/internal execution start and public action route errors now recover
  through named Effect adapter helpers before returning `HttpError`.
- Public execution path errors have a named adapter effect for Worker route
  mapping.
- Public execution service-binding dispatch now shares one named
  `dispatchExecutionEffect(...)` helper with operation-specific failure
  sources.

Why it changed:

Execution routes are another full route-service path: request JSON decoding,
protocol validation, path validation, service-binding dispatch, and start
response wrapping all meet at the public Worker boundary. Naming the route
adapters and dispatch helper keeps failures typed until the edge that owns
conversion.

Known limitations:

- This checkpoint does not change public Worker route matching, ExecutionDO
  internals, execution session behavior, deployment routing, PartitionDO
...
- `DeploymentService` now uses named Effect helpers for push existence
  preflight, finish artifact-ref resolution, abandon eligibility, and abandon
  reason normalization.
- Direct tests cover the helper success/failure channels before the generated
  HttpApi adapter maps service failures to response bodies.
- Service operations still propagate typed deployment/domain/storage failures
  without introducing `HttpError` into service logic.

Why it changed:

Finish and abandon service paths are the next deployment push lifecycle layer
after route validation. Their preflight checks should be reusable named Effect
boundaries so missing pushes, artifact-ref failures, invalid abandon states,
and storage failures remain typed until the generated handler or public Worker
adapter maps them once.

Known limitations:

- This checkpoint does not change DeploymentPushStore transaction bodies,
  push state transitions, active deployment activation, artifact storage,
  PartitionDO SQL/OCC, executor-http, or `ValidatorJson`.
...
- Public deployment push route compatibility helpers now convert typed
  route JSON/protocol failures to `HttpError` at the adapter edge.
- Generated DeploymentDO route compatibility helpers now convert typed
  route JSON/protocol failures to `HttpError` at the adapter edge.
- Direct Effect decoder functions still expose
  `DeploymentProtocolValidationError` so route/service code can remain typed
  until the final HTTP mapper.

Why it changed:

The route error mappers were named as HTTP conversion functions but could still
return protocol validation errors. Closing that leak makes the adapter boundary
more honest: protocol decoders emit typed failures, and compatibility/HTTP
adapters convert them once to the preserved 400 error envelope.

Known limitations:

- This checkpoint does not change public deployment push behavior, generated
  DeploymentApi request forwarding, DeploymentDO fallback responses,
  deployment service/store behavior, push lifecycle, PartitionDO SQL/OCC,
  executor-http, or `ValidatorJson`.
...
- `flarex-protocol/execution` now exposes Effect-returning decoders for start,
  syscall, and finish request payloads.
- The existing throwing execution protocol parsers remain compatibility
  wrappers over those Effect decoders.
- Backend execution request normalization now consumes the protocol Effect
  decoders directly instead of wrapping throwing parsers in local try/catch
  code.

Why it changed:

Execution request boundaries were returning `Effect` in the backend, but their
source protocol validation still depended on throwing parser adapters. Runtime
validation should emit typed `ExecutionProtocolValidationError` values at the
protocol boundary, then let route adapters map those failures once to HTTP.

Known limitations:

- This checkpoint does not change ExecutionDO session lifecycle, transaction
  setup, syscall behavior, public invoke, deployment routing, scheduler or
  delivery routing, PartitionDO SQL/OCC, executor-http, or `ValidatorJson`.
...
- Generated Deployment HttpApi response validation for active deployment,
  push-status, and finish-push responses now uses direct Effect Schema decoders
  instead of throwing protocol parsers inside `Effect.try(...)`.
- Public finish-push artifact preflight now reads the DeploymentDO push-status
  JSON response and decodes the push-status payload through named Effect
  helpers.
- Push-status JSON and semantic response failures stay in
  `PublicWorkerDispatchError` at the preflight source.

Why it changed:

Deployment response validation is a runtime boundary between Durable Object
service responses, generated HttpApi adapters, and public Worker finish-push
preflight behavior. Decoding these responses as Effect values keeps expected
boundary failures typed without changing adapter response shapes.

Known limitations:

- DeploymentDO SQL schema, push lifecycle transitions, public route path
  matching, analyzer behavior, public invoke/execution dispatch, PartitionDO
  SQL/OCC, executor-http, and `ValidatorJson` are unchanged.
...
- ExecutionDO query syscalls now keep missing indexes, unknown indexes, invalid
  index ranges, query placement failures, and returned document placement
  failures in typed invoke validation channels until the internal route
  adapter.
- Shared invoke `queryDocumentsEffect(...)` accepts the ExecutionDO transaction
  runner, so index reads still map through `ExecutionRouteOperationError`.
- HTTP adapter regressions now cover missing index, unknown index, invalid
  range, and missing placement filter failures.

Why it changed:

Runtime validation should distinguish query planning/placement failures from
transaction operation failures. This checkpoint removes query validation from
the generic operation wrapper without changing the public syscall response
contract.

Known limitations:

- Execution start/finish/abort, document syscalls, public execution dispatch,
  PartitionDO SQL/OCC, protocol schemas, executor-http, and `ValidatorJson` are
  unchanged.
