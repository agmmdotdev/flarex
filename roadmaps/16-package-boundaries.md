...
| `@flarex/payload-adapter` | Private pinned Payload profile, `BaseDatabaseAdapter` bridge, Local API runtime, normalization, population, and Payload-facing failure mapping | May consume only explicit private analysis and persistence facades; must not own raw SQL/Drizzle/Postgres, transaction settlement, lifecycle storage, materialization, commit/publication, public routes, or production activation |
