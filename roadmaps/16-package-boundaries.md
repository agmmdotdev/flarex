...
| `third_party/trigger.dev` | Frozen Trigger.dev run-engine/supervisor compatibility island plus source-only webapp reference used as migration input for durable execution and observability | Excluded from the Flarex workspace and runtime graph; the webapp is excluded from the island's nested build workspace as well. It has no Flarex imports, database authority, routing authority, public API, dashboard activation, or production path. Future adoption must port bounded behavior through Flarex-owned adapters and preserve the existing Postgres commit authority. |
