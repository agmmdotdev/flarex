...
- Public deployment route failures no longer convert directly to `Response`
  inside the deployment branch catch. They now become tagged Worker route
  adapter errors consumed by the top-level Worker response adapter.
- Existing deployment push/read/start/finish/abandon HTTP status and body
  behavior is preserved through the same compatibility mappers.
- The concrete migration checklist ticks `W-1` and sets `W-2` as the next
  active checkpoint.

Why it changed:

This keeps the public deployment branch aligned with the Worker-level route
error model. W-2 can now migrate route-family error unions toward the Worker
adapter without reintroducing branch-local response construction.

Known limitations:

- Deployment route-family compatibility mappers still exist. W-2 and later
  route-boundary phases own moving each family to native typed route errors.

Verification:

```sh
corepack pnpm --filter flarex-backend typecheck
corepack pnpm --filter flarex-backend exec vitest run test/publicWorkerRouteDispatchError.test.ts test/publicWorkerRoutePathBoundary.test.ts test/publicDeploymentPushRouteBoundary.test.ts test/publicDeploymentPushDispatchBoundary.test.ts test/push.test.ts --testTimeout=120000 --hookTimeout=120000 --maxWorkers=1
corepack pnpm --filter flarex-backend build
git diff --check
```
...
- Public deployment push/read/scheduler route failures now reach one Worker
  deployment response adapter instead of flowing through an intermediate
  `HttpError` effect.
- `PublicWorkerDispatchError` can be converted directly to a `Response` while
  preserving downstream status and error body.
- Public deployment path, protocol/body, execution, partition, sync, and
  dispatch failures are mapped to responses in the deployment branch adapter.

Why it changed:

This completes `P-2` in the concrete migration checklist. After P-1 typed the
public deployment push route inputs, P-2 makes the public deployment error path
more Effect-shaped by keeping failures typed until the Worker response adapter.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex public
  deployment push migration checkpoint.

How Flarex differs:

- Flarex still has public analyzer/artifact and multi-route Worker concerns.
  This checkpoint only changes response mapping for the public deployment
  branch and preserves existing lifecycle behavior.

Known limitations:

- Public start/finish artifact service response schemas are still unchecked by
  typed decoders; that is the next `P-3` checkpoint. Generated handler logic,
  DeploymentService/store lifecycle logic, artifact materializer/cache
  semantics, source-package analyzer behavior, DeploymentPushStore lifecycle
  state changes, PartitionDO SQL/OCC, executor-http, protocol parser
...
- Public deployment start, analyzed start, finish, abandon, and read push
  Worker routes now flow through tagged route-input objects.
- Source-only start keeps its existing analyzer-not-configured behavior while
  moving JSON parsing and protocol parsing behind named Effect route-input
  conversions.
- Finish keeps malformed JSON and artifact verification ordering while moving
  finish payload validation behind a route-input-to-dispatch conversion.
- Public push dispatch tests now assert preserved internal DeploymentDO routes
  from typed route inputs.

Why it changed:

This completes `P-1` in the concrete migration checklist. The public deployment
push route path now has typed Effect route inputs for the start/finish/abandon
body boundary, making the next `P-2` HTTP-error adapter cleanup smaller and
more direct.
...
- Flarex public push requests can perform analyzer and artifact checks before
  forwarding to the DeploymentDO. The new route-input types preserve that
  sequencing while reducing loose body dispatch.

Known limitations:

- Public deployment Worker HTTP response mapping still uses the current
  `HttpError` compatibility path. Generated handler logic, DeploymentService/store
  lifecycle logic, artifact materializer/cache semantics, source-package
  analyzer behavior, DeploymentPushStore lifecycle state changes, PartitionDO
...
- DeploymentDO production push API routing no longer rebuilds generated HttpApi
  `Request` objects after route decoding.
- The generated request compatibility dispatcher was removed from the internal
  route boundary after read and mutation routes both moved to direct typed
  dispatch.
- DeploymentDO no longer owns a generated web handler; generated handler
  integration tests keep using `HttpApiWebHandler`.
- Route-boundary tests now assert direct typed read/mutation route dispatch and
  no longer depend on compatibility adapter expectations.

Why it changed:

This completes Phase 1 DeploymentDO direct dispatch in the concrete migration
checklist. The next deployment push work can move outward to public Worker
deployment routes instead of keeping dead DeploymentDO bridge code alive.
...
- Flarex keeps generated handler effects as tested protocol adapters, but the
  DO route path now calls them directly from typed route inputs.

Known limitations:

- Public Worker deployment start/finish/abandon dispatch still uses existing
  route compatibility wrappers. Generated handler logic, DeploymentService/store
...
- DeploymentDO health, active deployment, and push read requests now route
  through explicit typed route inputs and direct generated handler effects.
- Active deployment success/not-found, push success/not-found, and storage
  failure mappings are covered in route-boundary tests.
- DeploymentDO production dispatch no longer calls the generated request bridge
  for read or mutation API routes.

Why it changed:

This completes D-5 in the concrete migration checklist. The deployment push
route boundary now uses typed direct dispatch for both reads and mutations,
leaving only cleanup/demotion of the compatibility bridge as the remaining
Phase 1 DeploymentDO checkpoint.
...
- Flarex still keeps generated handler effects and protocol schemas. The DO
  adapter now uses them directly rather than via generated web-handler request
  rebuilding.

Known limitations:

- Compatibility adapter cleanup, generated handler logic, DeploymentService/store
...
- Start, finish, and abandon typed route inputs can now dispatch directly to
  generated handler effects through
  `dispatchDeploymentApiMutationRouteInputDirect(...)`.
- The direct adapter maps generated handler results to HTTP responses without
  rebuilding a generated HttpApi `Request`.
- Rejected finish responses preserve the generated `409` status.
- Abandon conflict responses preserve the declared `409` error mapping.

Why it changed:

The deployment push route boundary now owns typed decoded mutation inputs. This
checkpoint proves those inputs can drive generated handler effects directly
while preserving push lifecycle response semantics.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentDO direct mutation dispatch checkpoint.

How Flarex differs:

- Flarex still keeps the generated HttpApi web-handler bridge wired for actual
  Durable Object requests. The direct mutation adapter is now available for the
  next wiring slice.

Known limitations:

- Actual `routeDeploymentDurableObject(...)` wiring, direct read dispatch,
  ...
  analyzer semantics, DeploymentPushStore lifecycle state changes,
...
- DeploymentDO push routing now enters through typed decoded route inputs.
- The generated HttpApi request bridge is isolated in
  `dispatchDeploymentApiRouteInputViaRequestCompatibility(...)`.
- The compatibility adapter preserves generated protocol failure propagation
  and maps generated handler operation failures to typed internal route errors.
- Focused tests cover analyzed start-push compatibility dispatch with decoded
  body payloads.

Why it changed:

The deployment push route surface now has typed decoded inputs. This checkpoint
makes the Durable Object route adapter consume those inputs directly while
keeping the generated web-handler compatibility path as the only remaining
request-rebuild edge.
...
- Flarex still routes push lifecycle requests through a generated HttpApi web
  handler. The compatibility bridge is explicit now, which prepares a later
  direct-dispatch decision without changing current push lifecycle behavior.
...
- Direct generated handler dispatch, generated handler logic,
  ...
  artifact materializer/cache, source-package analyzer semantics,
  DeploymentPushStore lifecycle state changes, PartitionDO SQL/OCC,
  executor-http, protocol parser compatibility, and `ValidatorJson` are
  unchanged.
...
- DeploymentDO push route decoding now produces typed route input values before
  generated HttpApi request compatibility conversion.
- The typed route input captures analyzed start-push bodies, finish-push bodies
  plus push ids, and abandon-push bodies plus push ids.
- The existing generated HttpApi request path remains intact through
  `decodeDeploymentApiRequestForRoute(...)`.
- Route-boundary tests now assert typed push ids and decoded mutation bodies at
  the route boundary.

Why it changed:

The deployment push migration has completed generated mutation response
adapters. The next step is preparing the Durable Object route surface to work
from typed decoded inputs instead of only rebuilt `Request` values, so a future
slice can decide whether direct generated handler dispatch is worth replacing
...
- Flarex routes analyzed deployment push lifecycle requests through a
  DeploymentDO and generated HttpApi layer. The new typed route input preserves
  that behavior while making push route payloads explicit at the DO boundary.
...
- Analyzed start-push generated handler response mapping now flows through
  `deploymentStartAnalyzedPushResponseForHttpApi(...)`.
- The start response adapter accepts validated service input and preserves
  typed service failure mapping before push-status protocol validation.
- The generated start handler keeps the existing protocol/domain input guard
  but delegates service response handling to the named adapter.
- Adapter tests now cover analyzed start success, bad-request validation
  failure, storage failure, and malformed push-status responses.

Why it changed:

The deployment push migration has now grouped the generated mutation handler
response edges into named Effect adapters. Start-push is distinct because it
also has a rich input-validation guard, so this checkpoint keeps that guard
separate from the service-response edge.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  generated deployment push response-adapter cleanup.

How Flarex differs:

- Flarex starts deployment pushes from analyzed backend metadata and validates
  the resulting push status before returning it through the Durable Object
  route surface.
...
- DeploymentDO route decoding, public Worker deployment dispatch,
  DeploymentService/store lifecycle logic, artifact materializer/cache,
  source-package analyzer semantics, DeploymentPushStore lifecycle state
...
- Finish-push generated handler response mapping now flows through
  `deploymentFinishPushResponseForHttpApi(...)`.
- Abandon-push generated handler response mapping now flows through
  `deploymentAbandonPushResponseForHttpApi(...)`.
- Both adapters preserve existing service failure mapping and protocol response
  validation while making the boundary reusable and directly testable.
- Adapter tests now cover activated finish responses, abandoned push responses,
  not-found/conflict service failures, and malformed service responses.

Why it changed:

The deployment push migration is moving from named validation helpers toward
true route/service Effect boundaries. Finish and abandon are the smallest
remaining generated mutation paths where service effects, typed failures, and
response protocol validation could be grouped into one coherent adapter slice.
...
- Flarex owns deployment push lifecycle routes for analyzed source packages.
  The generated HttpApi adapter validates mutation responses before returning
  them through the Durable Object route surface.

Known limitations:

- Generated start-push response handling, DeploymentDO route decoding, public
  Worker deployment dispatch, DeploymentService/store lifecycle logic,
  artifact materializer/cache, source-package analyzer semantics,
  DeploymentPushStore lifecycle state changes, PartitionDO SQL/OCC,
  executor-http, protocol parser compatibility, and `ValidatorJson` are
  unchanged.
...
- Generated analyzed start-push handler input no longer repeats full protocol
  payload decoding.
- A named protocol guard preserves analyzed start-push cross-field invariants
  before service/domain validation.
- Existing metadata validation still normalizes and rejects analyzed push
  source packages, diagnostics, schema, functions, partition metadata, and
  codegen metadata through the deployment validation layer.
- Source package decoding now accepts unknown boundary input and builds
  normalized module values from validated fields before service input assembly.

Why it changed:

The Effect migration quality bar asks generated route/service adapters to avoid
duplicated request decoding while keeping protocol and domain failures typed at
their source boundaries. This checkpoint makes the generated start-push handler
input path more precise without changing public push behavior.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  generated deployment push handler-input cleanup.

How Flarex differs:

- Flarex accepts analyzed push metadata from backend analyzer/public routes and
  turns it into service input. The generated HttpApi handler adapter keeps
  transport shape checks and deployment metadata validation distinct.
...
- Public deployment push route failures now flow through a named Worker
  response adapter effect after route decoding/dispatch returns typed failures.
- Worker-level deployment route HTTP mapping now reuses the existing named
  public deployment route mapper effect for request JSON and protocol
  validation failures.
- Existing public push tests continue to prove malformed start,
  start-analyzed, finish, and abandon route bodies preserve the same HTTP
  responses.

Why it changed:

The deployment migration quality bar requires typed request/body decoder
failures to propagate until a single adapter edge maps them to HTTP. This
checkpoint makes that Worker edge explicit for public deployment routes without
changing push lifecycle behavior.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  public deployment route adapter cleanup.

How Flarex differs:

- Flarex exposes deployment push lifecycle routes through a public Worker that
  dispatches into Cloudflare Durable Objects. The Worker adapter is therefore
  Flarex-specific, while typed request decoding stays in deployment modules.
...
- Active execution artifact metadata now decodes through
  `activeDeploymentExecutionArtifactRefFromMeta(...)` before active deployment
  response assembly.
- Active activation-time metadata now converts through
  `activeDeploymentActivatedAtFromMeta(...)`, preserving the existing fallback
  to the active push `updatedAt`.
- Focused deployment store tests cover the named metadata parsing helpers
  directly.

Why it changed:

Active deployment status is built from durable push rows plus raw metadata
values. Naming the metadata decoding step keeps malformed active deployment
state at the store boundary and keeps generated response validation separate.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  active deployment metadata cleanup around existing push activation behavior.

How Flarex differs:

- Flarex writes active deployment metadata during finish-push activation in a
  Cloudflare Durable Object. Reads must decode that metadata before public
  execution and deployment status routes use it.

Known limitations:

- DeploymentDO routing, generated DeploymentApi handler response mapping,
  public Worker deployment dispatch, service preflight, transaction SQL,
  artifact store implementation, DeploymentPushStore lifecycle state changes,
  PartitionDO SQL/OCC, executor-http, protocol parser compatibility, and
  `ValidatorJson` are unchanged.
...
- `DeploymentPushStore.finishPush(...)` now delegates stored push activation
  versus rejection classification to
  `deploymentFinishPushStoreDecision(...)`.
- `DeploymentPushStore.getActiveDeployment(...)` now delegates final active
  deployment response assembly to `activeDeploymentStatusFromStoreParts(...)`.
- Finish decisions now have direct tests for activation, invalid-state
  rejection, missing-analysis rejection, and missing-row typed failure.

Why it changed:

Finish push can be rejected without being a service error. Naming the store
decision keeps that lifecycle rule explicit while preserving generated handler
response mapping and public Worker dispatch behavior.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentPushStore lifecycle cleanup around existing push activation
  behavior.

How Flarex differs:

- Flarex stores deployment push lifecycle state in a Cloudflare Durable Object.
  Finish rejection responses are durable lifecycle decisions before any HTTP
  adapter mapping.
...
- DeploymentDO routing, generated DeploymentApi handler response mapping,
  ...
  implementation, DeploymentPushStore transaction SQL, PartitionDO SQL/OCC,
  executor-http, protocol parser compatibility, and `ValidatorJson` are
  unchanged.
...
- `DeploymentService.finishPush(...)` now delegates preflight input
  construction to `finishDeploymentPushStoreInput(...)`.
- The named preflight helper composes push lookup, execution artifact reference
  derivation, and clock reads into `FinishPushStoreInput`.
- Typed not-found, artifact reference, SQL, and validation failures propagate
  unchanged from their source boundaries.
- Added direct helper coverage for success, missing push, and artifact-ref
  failure before store writes.

Why it changed:

Finish push has two layers of behavior: service preflight and store
activation/rejection. Naming the service preflight helper makes that split
explicit while preserving the existing store-owned finish response semantics.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  DeploymentService preflight cleanup around existing push activation
  behavior.

How Flarex differs:

- Flarex derives an execution artifact reference before asking the
  DeploymentDO store to finish a push. Store activation and rejection remain
  Cloudflare Durable Object concerns.

Known limitations:

- DeploymentDO routing, generated DeploymentApi handler response mapping,
  public Worker deployment dispatch, artifact store implementation,
  DeploymentPushStore finish activation/rejection semantics, PartitionDO
  SQL/OCC, executor-http, protocol parser compatibility, and `ValidatorJson`
  are unchanged.
...
- Removed the legacy `deployment/HttpBoundary.ts` adapter module.
- Removed `deploymentFailureToHttpError(...)` and
  `finishPushHttpStatus(...)`.
- Removed the matching `deploymentHttpBoundary.test.ts` compatibility test.
- Removed the legacy `deploymentHttpErrorTo*Response(...)` helpers from
  `deployment/HttpApiHandlers.ts`.
- Kept generated Deployment HttpApi handler failures on typed response mappers
  and named Effect adapter wrappers.

Why it changed:

Deployment's generated HttpApi handlers already convert typed service,
protocol, and validation failures directly to declared DeploymentApi response
classes. The removed helpers kept an older `HttpError` compatibility adapter in
parallel with the typed response path, which made the deployment boundary look
less complete than the current Effect flow.

Convex references:

- No Convex source files were inspected for this slice. This is a Flarex
  deployment adapter cleanup around existing generated HttpApi handlers.

How Flarex differs:

- Flarex uses generated Effect HttpApi handlers inside a Cloudflare
  DeploymentDO route. This checkpoint keeps that adapter shape and removes only
  obsolete `HttpError` compatibility helpers.
...
- Public deployment push start/analyzed-start/finish/abandon route request
  errors now have explicit typed aliases and named adapter effects.
- Public deployment push dispatch to DeploymentDO now runs through a shared
  named dispatch helper that preserves operation-specific failure sources.
- Direct tests cover route adapter-effect failures and shared dispatch helper
  success/failure behavior.

Why it changed:

Public push routes connect external request decoding, analyzer/artifact
preflight behavior, and DeploymentDO service-binding dispatch. Naming the
route adapter and dispatch effects makes this full route-service path follow
the same typed boundary shape as the generated DeploymentApi handlers.

Known limitations:

- This checkpoint does not change analyzer behavior, artifact persistence or
  missing-artifact semantics, push state transitions, DeploymentService/store
  internals, generated route wiring, `DeploymentDO.fetch()`, PartitionDO
