...
9. **Retry classes remain separate.** O08-A exact-attempt replacement, O08-B
   OCC reruns, O08-C known-settled retry of an authenticated logical/closed
   command, and O08-D uncertain-outcome lookup cannot be collapsed into one
   generic retry loop.
...
epoch-provenance constraints, inert retained floor, and bounded contiguous
  ...
  orchestration and O08-B/C/D retry policy remain unimplemented; O08-A exact-
  attempt replacement is complete. The O04
...
logical point planning is complete. S08's additive native commit/change-feed
  ...
  C05-B's verified fresh-process reconstruction/composition, and O08-A exact-
  attempt replacement are complete; O08-B is the next Wave 2 prerequisite,
