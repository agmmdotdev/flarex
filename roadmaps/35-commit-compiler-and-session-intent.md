...
also complete. O08-B2b2b2b execution-claim liveness/renewal and production
scheduling/redelivery and dispatch plus C06-B endpoint/response policy remain pending; C04C2 remains conditional
and unapproved.
...
O07-B unless those first consumers later justify conditional C04C2.
...
O07-A committed-outcome resolver
  one bounded outcome/clock/floor/header statement; post-SQL result verification
             |
...
current transaction authority but forces rollback and publishes nothing. O07-B
...
dirty/failed-attempt disposition, O08-B2b2b2b retains execution-claim liveness/
renewal and scheduling/redelivery and production dispatch, and O11 first consumes active
floors.
...
user code at the new snapshot. O08-B2b1/C06-A now implements the accepted
Postgres exact-attempt execution claim: O03 activation and O08-A replacement
create it atomically before `running`, a directly settled outcome-first acquire
may mint only its same-factory opaque projection, every execution/journal/seal/
transition admission enforces its owner and fence, and C05-A consumes it when
entering `finishing`. Claim expiry remains distinct from snapshot-lease expiry
and terminalization. O08-B2b2a now composes exact-selector replay/expiry, live-
...
claim-fenced terminalization; O08-B2b2b2b execution-claim liveness/renewal and
production scheduling/redelivery and dispatch plus C06-B endpoint/response policy remain pending. A stale journal or
...
- C04A accepts only a live `running + sealed` attempt for initial planning or
  ...
  committed/non-plannable; O07-A owns lookup and O08-D/C06 own replay policy;
...
- C03A resolves only one opaque point-table capability from the session-pinned
  ...
  O06 retains the reusable rollback-proven physical row-lowering kernel; O07-B
...
- Initial activation creates the exact-attempt root with database time as its
  ...
  draws neither a new ID nor a new time. O08-A repeats root creation with a
  fresh database-time seed and zero accounting for every replaced attempt.
...
O07-A now resolves `missing`, matching `available`, or matching `expired`
receipts from one bounded statement. The closed lookup record is structural
matching evidence, not authority; production must derive it from authenticated
same-factory provenance. Old-epoch receipts remain valid, floor equality
requires a retained exact header, and canonical result verification runs after
SQL. O08-D/C06 still own replay and uncertain-outcome orchestration.
...
remain O08-D/C06 behavior, not S09-A schema behavior. After payload expiry the
...
The commit token has no foreign key to compactable S08 feed history. O07-B proves
...
The replacement keeps one supporting primitive and three coordinators separate:
...
5. **O08-B2b1/C06-A durable claim admission (complete):** migration 0032 adds
   the exact-attempt claim; O03 and O08-A create it atomically before `running`;
   outcome-first locked acquisition/takeover mints only a settled same-factory
   projection; all execution and journal/seal/transition admissions enforce the
   exact owner/fence; and C05-A consumes it when entering `finishing`.
6. **O08-B2b2a safe-state redispatch composition (complete):** one private
   exact-selector operation closes replay/expiry, busy, dirty-open, and failed-
   root states; runs one authenticated pristine attempt; finishes one sealed
   running attempt without user code; and delegates durable finishing recovery
...
8. **O08-B2b2b2a durable disposition (complete):** locked outcome-first C06-A
   acquisition may mint a distinct, single-use `abortOnly` capability only for
   an expired dirty-open or failed-root attempt with live authority. That family
   is rejected by every general execution/finish/journal/seal/C05/publication
   path; its dedicated consumer reloads the exact attempt and invokes existing
   claim-fenced terminalization. The closed lifecycle evidence grants no
   committed outcome, execution, or retry authority.
9. **O08-B2b2b2b and C06-B dispatch policy:** execution-claim liveness/renewal,
   scheduling/redelivery, endpoint response, routing, and runtime adapters
   remain deferred. They must compose the singular Postgres claim and may not
   infer authority from lifecycle or discovery evidence.
...
11. **O08-C known pre-decision SQL serialization/deadlock (complete):** the
...
12. **O08-D uncertain outcome (complete):** consume only a direct same-factory
...
and B2b2b2a supplies only dirty/failed-attempt disposition. B2b2b2b and C06-B
...
4. **B2b execution ownership is durable and singular.** Every execution and
   journal/syscall admission must match the exact current
...
the stored-attempt loader, commit-authority materializer, point-commit
...
- S09-A's final `(scope_uuid, request_key)` committed-success table and
  available/expired state constraints and O07-A read-only resolver are
  implemented. Its writer, replay orchestration, expiry transition, public key
...
trusted preparation and exact SnapshotToken
  ...
  -> O07-B durable publication through the same transaction kernel
...
operational point-journal consumer, and C04A's private stored-attempt
 ...
 DDL, S09-A private committed-success DDL, S09-B private commit-wake DDL and
  ...
  are complete. O08-B2b2b2b execution-claim liveness/renewal and production
  scheduling/redelivery and dispatch plus C06-B endpoint/response policy remain pending, and
...
activation authority, O08-B2b2b2b/C06-B/C07, the target-only caller and routing
