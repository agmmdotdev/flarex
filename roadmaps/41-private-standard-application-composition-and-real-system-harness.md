...
| `SAC01-F2l` | Prove multi-revision app-document schema evolution in cooking | `M01-A` pure compatibility and the protocol-only `M01-B` candidate-validation frames are complete; the scenario remains blocked on `M02` through `M03-C`. The eventual separate scenario consumes the live composed service and covers populated-field removal refusal/remediation, required-field expand/backfill/contract, validator tightening, concurrent shadow validation, supersession, readiness, activation, and stale-attempt retry without test-owned schema, receipt, activation, or commit authority |
