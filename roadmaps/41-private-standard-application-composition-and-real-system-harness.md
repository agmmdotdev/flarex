...
| `SAC01-F2l` | Prove multi-revision app-document schema evolution in cooking | Preflight complete in the managed-schema deployment roadmap; implementation is blocked on `M01-A` through `M03-C`. The eventual separate scenario covers populated-field removal refusal/remediation, required-field expand/backfill/contract, validator tightening, concurrent shadow validation, supersession, readiness, activation, and stale-attempt retry without test-owned schema or commit authority |
