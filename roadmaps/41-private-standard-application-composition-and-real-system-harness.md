...
| `SAC01-F2l` | Prove multi-revision app-document schema evolution in cooking | Preflight and the separate private `@flarex/managed-schema` ownership boundary are accepted; implementation is blocked on `M01-A` through `M03-C`. The eventual separate scenario consumes the live composed service and covers populated-field removal refusal/remediation, required-field expand/backfill/contract, validator tightening, concurrent shadow validation, supersession, readiness, activation, and stale-attempt retry without test-owned schema, receipt, activation, or commit authority |
