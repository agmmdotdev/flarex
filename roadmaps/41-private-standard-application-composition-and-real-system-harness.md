...
| `SAC01-F2t-D6` | Complete. Cooking creates a fourth isolated draft and run of the existing mutation-bearing Task, then uses the existing private unresolved-result-publication adapter. The application mutation and handler's post-commit query finish before the result-publication attempt and reconciliation read both fail. The existing supervisor returns typed uncertainty with the attempt still executing; an ordinary application query independently confirms the committed recipe. The harness fabricates no Task result and adds no result-store, lifecycle, retry, scheduler, or public API behavior | PGlite and authenticated ordinary-role PostgreSQL prove one result-publication attempt, one reconciliation read, one Worker load/start/settlement, no retained or terminal Task result, five runs with exactly four terminal and one executing, and four confirmed child-mutation outcomes. Exact commits 20 and 21 plus runtime counts, rows, revisions, indexes, feed, and outbox prove the application mutation committed once without converting publication uncertainty into Task success |

Cooking's query-bearing, mutation-bearing, concurrent duplicate-delivery,
lost-completion-response, reconciled-result-publication, and unresolved-result-
publication Task scenarios now pass after C0 through C4. Further app Task
