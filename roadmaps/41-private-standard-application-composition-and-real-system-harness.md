...
| `SAC01-F2t-C` | Reuse hosted Task delivery and test-owned lifecycle/fault controls in the unified environment | Complete through C0, C1, C2q, C2m, C3r, C3c, C3d, and C4. The scoped hosted kit backs one explicit private `client.tasks.deliver` path through the caller's current Application fixture, persisted attempt lifecycle, resource event host, generated Application Worker, stored result validator, selection-bound Application query owner, existing Task mutation/external-effect owners, handler-failure retry decision, bounded cancellation ordering, explicit fault adapters, and fresh-host recovery. The implementation reuses the production-compatible Application delivery/event-host, lifecycle, result, query, mutation, manual-wake, and takeover owners without exposing persistence/provider authority, adding a Standard API, or activating scheduling |
...
| --- | --- | --- |
| `SAC01-F2t-C0` | Complete. The exact hosted R2-resource, Worker-session, bounded-policy, and disposal mechanics live in one system-test-owned scoped host kit. The connected proof consumes that kit for both original and fresh hosts, owns its sole runtime bridge, and retains no displaced local copies | System-test typecheck and lint gates pass. Existing DTE06-F1/F2 evidence passes as 1 hosted PGlite test, 14 connected/takeover PGlite tests, and 17 zero-skip connected/takeover tests against authenticated ordinary-role PostgreSQL 18.3 |
| `SAC01-F2t-C1` | Complete. The private top-level `createTaskRun` method was removed without an alias and replaced by `client.tasks.create`. `client.tasks.deliver` accepts only an exact creation registered by that per-simulation capability, manually starts its stored attempt, drives the existing resource event host once, reads the committed result, and validates it through the exact registered Task definition before returning typed output plus bounded redacted evidence. Existing lifecycle, host, supervisor, and result-store tagged failures propagate unchanged; only reference, control-acquisition, output, and evidence failures belong to the harness. The R2 stores, Worker loader, control target, lifecycle, provider, host, and physical locator remain scoped and do not escape | One PGlite and two zero-skip authenticated ordinary-role PostgreSQL 18.3 checks retain exact create/replay and prove one hosted success, one `application_v1` Worker load/start, `{ prepared: true }`, one result settlement, exact host/supervision counts, one attempt and dispatch, zero pending rows, zero Legacy definition rows, and zero Legacy runtime-object reads. The no-Task unified environment also retains 15 PGlite and four zero-skip PostgreSQL cooking/English/environment checks |
| `SAC01-F2t-C2q` | Complete. `ApplicationQuerySystem` exposes its already-owned selection-bound query port, and scoped manual delivery requires that service from the unified Application Layer. The Task authority continues to reacquire and correlate the active selection before delegating; neither the Task handler nor harness receives selection or identity choice, and no query implementation or fake response remains | The typed Task fixture seeds one recipe, executes a real registered query from its generated handler, returns the exact stored title and launch-bound user subject, records exactly one query runtime execution, and leaves rows, revisions, commits, idempotency outcomes, feed, outbox, and mutation runtime count unchanged. Both PGlite and authenticated ordinary-role genuine PostgreSQL gates are retained alongside the connected and no-Task regressions |
| `SAC01-F2t-C2m` | Complete. Scoped manual delivery acquires a lane-owned temporary external-effect target and binds the generated Task handler's mutation callback to the existing launch-bound `ApplicationMutationSystem`, Task mutation authority, and Task external-effect authority. The handler reads one recipe and commits one preparation through a separately registered mutation; no mutation implementation, retry policy, scheduling, or persistence schema was added to the harness | PGlite and authenticated ordinary-role PostgreSQL prove one Task run creation replay, exactly one child-mutation runtime execution and second authoritative commit, aligned idempotency/feed/outbox sequences, one confirmed child-mutation effect with stored outcome evidence, and unchanged connected/no-Task regressions |
| `SAC01-F2t-C3r` | Complete. The unified simulation registers a distinct test-owned Task whose real generated handler throws unconditionally; no payload field, function-name dispatch, runtime branch, fallback, or fabricated settlement selects the failure. The existing Worker maps the handler exception to `handler_failed`, and the existing supervisor and Application lifecycle accept the bound retry. Private `client.tasks.deliver` now returns a bounded discriminated success-or-retry receipt rather than treating the admitted retry state as a harness failure | PGlite and authenticated ordinary-role PostgreSQL prove exact run-creation replay, attempt 1 as the accepted previous attempt, `ready/start_attempt` retry state, zero result reads/writes for the failed delivery, no terminal outcome for that run, and no duplicate application mutation/commit/feed/outbox effects; supervisor/mapping, connected Task, and no-Task regressions remain green |
| `SAC01-F2t-C3c` | Complete. `client.tasks.deliver` now requires an explicit private completion-or-cancellation mode. Cancellation mode composes the existing connected runner, lifecycle decision, provider, supervisor, and scoped hosted Worker under one invocation without exposing any of those capabilities. Two separately registered real handlers prove cancellation before completion and completion before cancellation; neither payload fields nor function names select runtime behavior. The receipt reports the exact cancellation generation and either `acknowledged` cancellation or `superseded_by_completion` success | PGlite and authenticated ordinary-role PostgreSQL prove exact run-creation replay, one delivered generation-1 cancellation with an interrupted Worker and zero result reads/writes, one rejected late cancellation whose completed result wins durably, two consumed cancellation rows, zero pending rows, three terminal runs, scoped cleanup, and unchanged connected/backend/no-Task regressions |
| `SAC01-F2t-C3d` | Complete. Private `client.tasks.deliver` admits one explicit fault mode at a time. A system-test result-bucket adapter can lose a committed create response or leave publication unresolved, while a lane-owned transaction adapter can hide one committed completion response. Duplicate delivery invokes the same connected runner again while its first accepted Worker remains live. The adapters only remove response evidence; they do not retry, settle, choose a lifecycle outcome, or expose a mutable fault controller | PGlite and authenticated ordinary-role PostgreSQL prove exact create/replay for four distinct real Task runs: duplicate delivery performs zero additional candidate/provider work; lost completion response replays the same completion object exactly once and returns the lifecycle owner's `idempotent` decision; lost result publication reconciles through the result-store owner; unresolved publication returns the typed `reconcileRead` uncertainty receipt with the attempt still executing, one publication attempt, one reconciliation read, no retained result, and no fabricated terminal outcome. Existing application commits, feed, outbox, child-mutation effects, connected/backend/no-Task paths remain the regression boundary |
| `SAC01-F2t-C4` | Complete. Private recovery mode abandons Host A after its persisted attempt enters execution, then constructs Host B with a distinct control target, delivery target, evidence reader, directory authority, mutation authority target, Worker loader/session owner, and forked ports over the same immutable R2 resources. The existing scope-bound manual wake owner alone expires attempt 1 and admits attempt 2; the harness adds no run selector, timer, scheduler trigger, retry decision, or settlement path | PGlite and authenticated ordinary-role PostgreSQL prove Host B cannot deliver before expiry, manual lease expiry produces the bound durable retry, manual retry wake grants attempt 2, stale Host A heartbeat/completion leave the exact encoded replacement aggregate unchanged, Host B reads the original input/principal resources and settles the validated result, and a final run observes no resurrection. The exact receipt proves distinct host identities, attempt 1 to attempt 2 takeover, zero abandoned settlement, one replacement settlement, and zero Legacy runtime-object reads |
| `SAC01-F2t-D1` | Complete. Cooking owns its first real typed Task beside the existing application definition. The generated `buildServingGuide` handler calls the application's public `recipeViews:assessment` query, which retains its nested internal-query and authoritative recipe-read path. The workload creates and exactly replays one durable run, drives one explicit hosted completion, and validates the rich nested output through the Task definition. No application mutation, retry, cancellation, scheduler, public Task syntax, or new production route is added | The focused PGlite and authenticated ordinary-role PostgreSQL lanes prove one catalog, definition, request, run, attempt, dispatch, and terminal outcome; zero Legacy definition revisions and pending deliveries; one `application_v1` Worker load/start/settlement; and one additional query runtime execution with application rows, revisions, commits, feed, and outbox unchanged by the Task |
| `SAC01-F2t-D2` | Complete. Cooking adds one separately registered `publishServingGuide` Task after the pre-existing workload lifecycle. The workload first creates an isolated publishable draft, then exactly creates/replays and explicitly delivers the Task. Its generated handler delegates to the existing public publication workflow through Task mutation authority and performs a post-commit assessment through Task query authority. The workflow retains its internal query/mutation, OCC, commit, feed, outbox, and application-error owners; the harness adds no mutation implementation, dual write, retry, scheduler, or production route | PGlite and authenticated ordinary-role PostgreSQL prove one catalog with two definitions, two exactly replayed terminal runs, two attempts and dispatches, zero pending or Legacy definition rows, one confirmed child-mutation effect with stored outcome digest, commits 14 and 15 for the isolated draft and publication, exact published receipt plus post-commit assessment, and the expected application-row/revision/index/feed/outbox expansion without disturbing the earlier Cooking invariants |
| `SAC01-F2t-D3` | Complete. Cooking delivers its mutation-bearing Task through the existing private duplicate-delivery fault mode. The connected runner performs a second delivery cycle while the first accepted Worker still owns the attempt; that cycle must observe zero dispatch or cancellation candidates, zero provider calls, and zero candidate failures. This is concurrent duplicate suppression, not a post-terminal redelivery API, and it adds no retry, scheduler, lifecycle decision, or application idempotency mechanism | PGlite and authenticated ordinary-role PostgreSQL prove the Task still has one Worker load/start/settlement, one result publication, one confirmed child-mutation effect and outcome digest, and only commits 14 and 15 for draft creation and publication. The exact duplicate receipt plus unchanged application rows, revisions, indexes, commit feed, and outbox prove the competing delivery produced no second mutation or commit |
| `SAC01-F2t-D4` | Complete. Cooking creates a second isolated draft and run of the existing mutation-bearing Task, then uses the existing private lost-completion-response adapter. The Worker publishes its result and the application mutation commits before the adapter removes the first lifecycle response; the existing supervisor replays the identical completion object and the lifecycle owner alone returns `idempotent`. No Worker retry, application replay, scheduler, fallback, or new completion API is introduced | PGlite and authenticated ordinary-role PostgreSQL prove two completion attempts over the same object, one Worker load/start/settlement, one result publication, and one additional confirmed child-mutation outcome for the new run. Exact commits 16 and 17, runtime counts, rows, revisions, indexes, feed, and outbox prove the lost response did not rerun the Worker or duplicate the application mutation |
| `SAC01-F2t-D5` | Complete. Cooking creates a third isolated draft and run of the existing mutation-bearing Task, then uses the existing private lost-result-publication-response adapter. The immutable result is stored before the adapter rejects the response; the existing supervisor performs one reconciliation read and continues with that stored value. The harness neither retries the Worker nor fabricates a result, and adds no result-store, lifecycle, scheduler, or public API behavior | PGlite and authenticated ordinary-role PostgreSQL prove one publication attempt, one reconciliation read, one Worker load/start/settlement, one result write, and one additional confirmed child-mutation outcome. Exact commits 18 and 19, runtime counts, rows, revisions, indexes, feed, and outbox prove result reconciliation did not rerun or duplicate the application mutation |
| `SAC01-F2t-D6` | Complete. Cooking creates an isolated draft and run of the existing mutation-bearing Task, then uses the existing private unresolved-result-publication adapter. The application mutation and handler's post-commit query finish before the result-publication attempt and reconciliation read both fail. The existing supervisor returns typed uncertainty with the attempt still executing; an ordinary application query independently confirms the committed recipe. The harness fabricates no Task result and adds no result-store, lifecycle, retry, scheduler, or public API behavior | PGlite and authenticated ordinary-role PostgreSQL prove one result-publication attempt, one reconciliation read, one Worker load/start/settlement, no retained or terminal Task result, six runs with exactly five terminal and one executing, and six confirmed child-mutation outcomes across the full Cooking corpus. Exact commits 22 and 23 plus runtime counts, rows, revisions, indexes, feed, and outbox prove the application mutation committed once without converting publication uncertainty into Task success |
| `SAC01-F2t-D7` | Complete. Cooking creates a separate mutation-bearing Task before the retained D6 uncertainty probe and gives that Task an explicit two-attempt author policy. Host A executes the generated handler, commits the application mutation, completes its post-commit query, and then observes unresolved result publication. The existing manual wake owner expires attempt 1 and grants attempt 2 to a fresh host. Attempt 2 derives the same child-mutation request key from scope, run, and operation ordinal, so the existing application mutation owner replays the committed outcome while the new attempt retains its own external-effect evidence. Host B republishes and settles the validated Task result. No harness-owned retry, application mutation, lifecycle decision, scheduler trigger, or public API is added | Focused PGlite and fresh authenticated ordinary-role PostgreSQL 18.3 prove one failed Host A supervision with the exact typed result-publication-uncertainty cause, one settled Worker and one unresolved write/read pair, attempt 1 to attempt 2 takeover, stale Host A fencing, distinct Host B resources, and one successful replacement settlement. The full corpus records seven attempts and dispatches but only commits 20 and 21 for this draft/publication pair; mutation runtime count advances once for the child mutation while both attempts execute the post-commit query. Two confirmed child-effect attempts share one authoritative application outcome, and exact rows, revisions, indexes, feed, outbox, and final assessment prove no duplicate application mutation |

Cooking's query-bearing, mutation-bearing, concurrent duplicate-delivery,
lost-completion-response, reconciled-result-publication, unresolved-result-
publication, and fresh-host mutation-replay Task scenarios now pass after C0
through C4. Further app Task
coverage must remain separately bounded.
Public Task syntax, generated
references, scheduling, outbound actions, run reads or subscriptions, and
production routing remain owned by their later roadmaps.
