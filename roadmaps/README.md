...
| [`46-private-standard-edge-action-vertical.md`](./46-private-standard-edge-action-vertical.md) | Docs-first preflight and durable-task ownership reconciliation complete; `AAV-A1` approval gate | Existing `edge_action` publication, R2 materialization, readiness, and active selection compose. AAV-A1 must add direct action request/outcome authority plus one shared external-effect evidence boundary without duplicating Task System run/attempt/orchestration state. `AAV-A2` and private route-independent `SAP07` remain later gates; FSV07, schedules, Node actions, durable-task orchestration, and production routing remain excluded. |
