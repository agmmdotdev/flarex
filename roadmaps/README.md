...
| [`durable-task-engine/README.md`](./durable-task-engine/README.md) | DTE-IP01 private package complete; persistence remains closed | Reuse-first Trigger.dev run-attempt logic now lives in the production-inert Flarex package with closed schemas, pure decisions, services, provenance, and executable compatibility receipts; persistence, host, observability, public API, and activation remain separately gated. |
...
| [`43-first-flarexdb-system-api-vertical.md`](./43-first-flarexdb-system-api-vertical.md) | Active; `FSV01`-`FSV06`, `C03-V`, `FSV06-A1`, and `FSV06-A2` complete | The first relation-free private Standard vertical now reaches an authoritative SAP04 point mutation through shared-primary activation, scoped validation, candidate-bound R2 exact runtime dispatch, and the existing C07 owners. The next gate is the separate FSV07 production-routing preflight; unsupported target layouts and every production caller remain open. |
...
| [`46-private-standard-edge-action-vertical.md`](./46-private-standard-edge-action-vertical.md) | Docs-first preflight and durable-task ownership reconciliation complete; AAV-A1 implementation gate | Existing `edge_action` publication, R2 materialization, readiness, and active selection compose. Roadmap 47 now owns the exact AAV-A1 protocol/storage preflight; AAV-A2 and private route-independent SAP07 remain later gates. |
| [`47-aav-a1-direct-action-and-shared-effect-authority.md`](./47-aav-a1-direct-action-and-shared-effect-authority.md) | Implementation-bearing preflight complete; implementation approval required | Direct action request/outcome authority uses one invocation row; shared external-effect uncertainty uses one evidence row and distinct parent subjects. Canonical bodies remain in R2, Task System lifecycle is not duplicated, and AAV-A2/SAP07/routes remain excluded. |
