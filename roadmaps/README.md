...
| [`43-first-flarexdb-system-api-vertical.md`](./43-first-flarexdb-system-api-vertical.md) | Active; `FSV01`-`FSV05`, `C03-V`, and `FSV06-A1` complete | Function-first private composition now includes inactive registration, readiness, one authoritative point mutation, shared-primary activation/coherent reads, scoped syscall validation, and candidate-bound route-independent exact runtime dispatch. The next gate is the separately approved continuation of `FSV06`/SAP04 composition; unsupported target layouts and production routing remain open. |
