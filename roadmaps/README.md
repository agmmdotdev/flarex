...
| [`43-first-flarexdb-system-api-vertical.md`](./43-first-flarexdb-system-api-vertical.md) | Active; `FSV01`-`FSV05` and `C03-V` complete | Function-first private composition now includes inactive registration, readiness, one authoritative point mutation, shared-primary activation/coherent reads, and scoped syscall-time validation. The next gate is the separate `FSV06`/SAP04 implementation preflight; unsupported target layouts and production routing remain open. |
