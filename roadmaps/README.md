...
| [`46-private-standard-edge-action-vertical.md`](./46-private-standard-edge-action-vertical.md) | Docs-first preflight complete; `AAV-A1` approval gate | Existing `edge_action` publication, R2 materialization, readiness, and active selection compose, but a durable action request/result/effect-attempt owner is required before accepting crash/retry/response-loss semantics. `AAV-A2` and private route-independent `SAP07` remain later gates; FSV07, schedules, Node actions, durable tasks, and production routing remain excluded. |
