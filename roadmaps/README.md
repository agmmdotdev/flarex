...
| [`09-sdk-and-cli-fork.md`](./09-sdk-and-cli-fork.md) | Public SDK, generated APIs, clients/React, CLI/codegen/deploy, test package relationship, npm distribution, and later public relation ergonomics only after the native `SV-R` gate. |
...
| [`42-standard-application-apis.md`](./42-standard-application-apis.md) | Stable workspace-internal application stage APIs. Definition preparation remains available; the historical static-verifier surfaces are Legacy-only, and the current unversioned Standard facade composes Application Analysis. |
...
| --- | --- | --- |
| [`flarexdb-foundation/README.md`](./flarexdb-foundation/README.md) | Focused execution index | Master status, wave order, hosted gate, cross-plan invariants, and the docs-only native relation sequence from current Application authority through `SV-R`. |
...
| [`flarexdb-foundation/04-payload-relational-contract.md`](./flarexdb-foundation/04-payload-relational-contract.md) | Active private native relation plan; `R01`, `R01-P`, `R02`, `S12`, `C09`, production-inert `E01`, private system-core `O10-R`, `RA01`, direct read-only Standard query `RQ01`, and atomic fact slice `R03-A` are complete | Current private authenticated executable modules -> Application Analysis/Application Manifest -> R02 binding -> S12/C09 storage maintenance -> E01 physical and Application readiness -> O10-R exact relation read/OCC -> RA01 -> RQ01 -> R03-A atomic relation facts -> R03-B fenced target-sync invalidation -> SV-R. R03-A reuses one commit/feed/retention owner with no second wake or stream. R03-B remains gated on roadmap 21's target sync owner; the Legacy timestamp subscription/Scheduler path is not an interim owner, and no Worker API, public syntax, Payload adapter, or production cutover was added. |
| [`flarexdb-foundation/05-managed-schema-deployment.md`](./flarexdb-foundation/05-managed-schema-deployment.md) | Accepted deferred contract; `M01-A`, protocol-only `M01-B`, and read-only `M02` complete | Migrationless developer experience, private `@flarex/managed-schema` ownership, conservative compatibility classification, deterministic pinned planning, canonical candidate-validation progress/evidence/receipt contracts, one future persisted validation head, concurrent shadow validation, and readiness/activation gates; guarded target-local `M03-A` persistence/scanning is next and separately gated. |
