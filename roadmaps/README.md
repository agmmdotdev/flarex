...
| [`44-second-flarexdb-system-api-point-query-vertical.md`](./44-second-flarexdb-system-api-point-query-vertical.md) | Active; `PQV-A1` complete, `PQV-A2` and `SAP05` separately gated | The second private vertical first establishes a Scope-owned target-native point-query snapshot, then a candidate-bound exact query runtime/ABI, then one Standard point query. It is route-independent and does not reopen roadmap 43 or authorize FSV07. |
