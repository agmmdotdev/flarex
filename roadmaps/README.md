...
| [`shared-transaction-core/`](./shared-transaction-core/README.md) | Cross-profile transaction/publication/recovery ownership reconciliation, replacement and cleanup obligations, and performance/completion gates. Publication extraction, source ownership audit and R1 neutral physical ownership are complete; R2 CMS participant/materialization and representative measured validation remain pending. |
...
| [`relational-standard-apis/`](./relational-standard-apis/README.md) | Active focused plan and capability ledger for raising the implemented private native relation core through typed Standard Application definition and function-runtime APIs, then composing those same APIs in the system-test facade. It records the supported initial profile, every deliberately unsupported capability and rationale, and the gates required before public or reactive claims. |
...
| [`flarexdb-framework-integration/README.md`](./flarexdb-framework-integration/README.md) | Active accepted roadmap domain; artifact repository, consumer audits, private value-only relational schema, and production-inert coordinator checkpoints 1 and 2 complete with a manifest-owned serial PGlite storage/repository gate; checkpoint-3 target-session/coordinator decision and binding remain later gates | Shared framework-storage boundaries and the ordered, separately gated Payload and Medusa adapter program. It consumes the application foundation without redefining it. |
