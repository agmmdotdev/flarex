...
| [`43-first-flarexdb-system-api-vertical.md`](./43-first-flarexdb-system-api-vertical.md) | Active; `FSV01`-`FSV05` complete | Function-first private composition now includes inactive registration, readiness, one authoritative point mutation, and shared-primary activation/coherent reads. The next gate is the separate `C03-V` implementation preflight; `FSV06`/SAP04, unsupported target layouts, and production routing remain open. |
