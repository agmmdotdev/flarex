...
| [`45-private-internal-user-code-calls.md`](./45-private-internal-user-code-calls.md) | `SAP06-A1` and `SAP06-A2` complete privately; `SAP06-A3` implementation preflight complete and separately gated | The proposed combined mutation internal-call profile keeps query and mutation children inside the exact SAP04 Worker, parent C03 journal/overlay, OCC, and outcome. Its three identities and explicit no-child-savepoint first-slice semantics require approval; no schema, route, production caller, or child publication is added. |
