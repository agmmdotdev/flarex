...
| [`44-second-flarexdb-system-api-point-query-vertical.md`](./44-second-flarexdb-system-api-point-query-vertical.md) | Active; `PQV-A1` and `PQV-A2` complete, `SAP05` next and separately gated | The second private vertical now has a Scope-owned target-native point-query snapshot and candidate-bound exact query runtime/ABI; its next gate is one private System/Standard point query. It is route-independent and does not reopen roadmap 43 or authorize FSV07. |
