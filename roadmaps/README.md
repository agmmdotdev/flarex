...
| [`46-private-standard-edge-action-vertical.md`](./46-private-standard-edge-action-vertical.md) | AAV-A1 complete; AAV-A2 implementation preflight complete | Existing `edge_action` publication, R2 materialization, readiness, and active selection compose with direct invocation and shared uncertainty authority. Roadmap 48 pins the separate exact runtime gate; implementation and route-independent SAP07 remain later. |
...
| [`48-aav-a2-candidate-bound-edge-action-runtime.md`](./48-aav-a2-candidate-bound-edge-action-runtime.md) | Implementation preflight complete; approval required | Pins the private candidate-bound action target/profile/ABI, host-owned canonical egress policy, fresh Worker Loader isolation, authenticated outbound/callback bridges, resource ceilings, and cleanup with no schema, SAP07, route, or production behavior. |
