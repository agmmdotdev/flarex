...
| [`flarexdb-foundation/05-managed-schema-deployment.md`](./flarexdb-foundation/05-managed-schema-deployment.md) | Accepted deferred contract; `M01-A` and protocol-only `M01-B` complete | Migrationless developer experience, private `@flarex/managed-schema` ownership, conservative compatibility classification, canonical candidate-validation progress/evidence/receipt contracts, one future persisted validation head, concurrent shadow validation, and readiness/activation gates; read-only `M02` planning is next. |
