...
| [`flarexdb-foundation/05-managed-schema-deployment.md`](./flarexdb-foundation/05-managed-schema-deployment.md) | Accepted deferred contract; `M01-A` pure compatibility foundation complete | Migrationless developer experience, private `@flarex/managed-schema` ownership, conservative document/physical/identity classification, one future candidate-validation head, concurrent shadow validation, and readiness/activation gates; `M01-B` is next and implementation continues in separately approved bounded slices. |
