...
| [`flarexdb-foundation/04-payload-relational-contract.md`](./flarexdb-foundation/04-payload-relational-contract.md) | Accepted deferred native contract; docs-only `REL-P0` complete | Current private authenticated executable modules -> Application Analysis/Application Manifest -> publication/readiness authority, then `R01` -> `R01-P` -> `R02` -> `S12` -> `C09` -> `E01` -> `O10-R` -> `RA01` -> `RQ01` -> `R03` -> `SV-R`. No relation implementation, public syntax, or production cutover is claimed. Payload mapping and the trusted framework relational persistence/transaction SPI remain separate later adapter work. |
