...
| [`flarexdb-foundation/04-payload-relational-contract.md`](./flarexdb-foundation/04-payload-relational-contract.md) | Active private native relation plan; `R01`, `R01-P`, `R02`, `S12`, `C09`, production-inert `E01`, private system-core `O10-R`, `RA01`, and the direct read-only Standard query `RQ01` are complete | Current private authenticated executable modules -> Application Analysis/Application Manifest -> R02 binding -> S12/C09 storage maintenance -> E01 physical and Application readiness -> O10-R exact relation read/OCC -> RA01 -> RQ01 -> R03-A atomic relation facts -> R03-B fenced target-sync invalidation -> SV-R. The Legacy timestamp subscription/Scheduler path is not an interim R03 owner; RQ01 adds no Worker API, journal dependency, public syntax, Payload adapter, or production cutover. |
