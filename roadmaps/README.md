...
| [`43-first-flarexdb-system-api-vertical.md`](./43-first-flarexdb-system-api-vertical.md) | Active; `FSV01`-`FSV04` complete | Function-first private composition from the accepted replacement analyzer through inactive registration, readiness, and one authoritative point mutation; the next gate is the `S04`/`FSV05` activation implementation preflight. `FSV05` remains private and non-routing; `FSV06`/SAP04 and production activation remain open. |
