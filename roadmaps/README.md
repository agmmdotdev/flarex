...
| [`42-standard-application-apis.md`](./42-standard-application-apis.md) and [`50-clean-application-apis.md`](./50-clean-application-apis.md) | Stable workspace-internal application stages plus the accepted clean API migration. `CAPI-A` provides the private unversioned Application Definition facade and `CAPI-B` moves developer and reusable system-test definition/source producers onto it; invocation, facade migration, public, and production gates remain pending. Concrete wire and persisted contracts keep versions where exact compatibility requires them. |
...
| [`flarexdb-foundation/04-payload-relational-contract.md`](./flarexdb-foundation/04-payload-relational-contract.md) | Active private native relation plan; `R01`, `R01-P`, `R02`, `S12`, `C09`, production-inert `E01`, private system-core `O10-R`, `RA01`, direct read-only Standard query `RQ01`, and atomic fact slice `R03-A` are complete | The next relational slice is non-reactive `SV-R Core`: current private authenticated executable modules -> Application Analysis/Application Manifest -> R02 binding -> S12/C09 storage maintenance -> E01 physical and Application readiness -> O10-R exact relation read/OCC -> RA01 -> RQ01 -> R03-A facts -> one real function-runtime commit/query vertical. R03-B remains gated by the separate Query Sync Engine plus roadmap-21 Flarex adapter lane and gates only `SV-R Live` plus reactive/reconnectable claims. The Legacy timestamp subscription/Scheduler path is not an interim owner, and no public syntax, Payload adapter, or production cutover is authorized. |
