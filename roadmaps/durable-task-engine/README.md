...
The final SAP-TRP5 persistence review found that the current private
task-runtime receipt's Declarative candidate/package/artifact/semantic
commitments have no independent parent in the Application Analysis authority
chain. The approved correction is recorded in
[`preflight/42-application-task-runtime-authority.md`](./preflight/42-application-task-runtime-authority.md).
Because this recent private path has no production composition consumer, it
corrects the current runtime binding/receipt in place and uses plain semantic
names rather than creating V2 beside V1. It forbids a foreign key or fallback
to the displaced Declarative candidate table. `Legacy...` retention is allowed
only if the required consumer and environment inventory proves a real
compatibility obligation. Implementation remains separately gated.
