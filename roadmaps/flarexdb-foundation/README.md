...
| Managed schema | Private `M01-A` through `M04-C`, retirement preflight `M05-P`, exact workspace recovery `M05-A`, atomic supersession reclamation `M05-A2`, docs-only retirement gate reconciliation `M05-B0`, docs-only scope-local storage preflight `M05-B1-P`, and private scope-local lifecycle authority `M05-B1` are complete and production-inert; bounded current-pin proof in `M05-B2` is next, while irreversible retirement and physical/evidence purge remain separate |
