...
15. `S12` (complete): stable current edge occurrences, the preflight-selected
    ...
    genuine PostgreSQL.
16. `C09` (complete privately, production-inert): one same-factory R02 binding
    capability and deterministic prior/final lowerer now maintain S12 current
    edges and endpoint versions inside the existing point-commit transaction,
...
18. `O10-R` (complete privately, production-inert): one exact bounded incoming
...
19. `RA01` (implementation preflight frozen): evolve the existing private
    Application activation history and single head into unversioned,
    readiness-contract-discriminated storage, then activate one exact
    relation-bearing revision after E01/O10-R. No route, production caller,
    fallback, second head, or candidate-query bypass is authorized.
...
retirement and physical purge remain deferred. Private `M05-A2` is
...
or purge. The docs-only `M05-B0` preflight reflects the current pin owners,
and `M05-B1-P` corrects the storage boundary: logical retirement is a manual
per-scope `active -> draining -> retired` physical-availability operation, not
a deployment-global tenant drain. Private `M05-B1` now implements only the
scope-local current authority, opaque exact-definition preparation, read-only
inspection, and reversible `active -> draining` / `draining -> active`
transitions in additive migration `0067`. It remains production-unwired;
`retired` is reachable only through the private proof-bearing finalizer, while
`reactivating` remains representable but has no callable transition. The
immutable control-catalog definition remains
reusable by other scopes. One bounded current row records lifecycle, definition
digest, authority pins, transition fence, and replay digest; it does not copy
session/action/task pins or add an event log. Private `M05-B2` composes the
lifecycle into readiness and admission, and `M05-B3` provides bounded checks
over active/candidate selection, nonterminal Application mutation sessions,
admitted/executing actions, nonterminal durable tasks, and O11 live-retention
pins before fenced finalization. Private `M05-B4` adds one explicit step: it
begins draining, returns on a pin, and cold-replays the exact prior transition
before finalizing or reporting completion. It adds no timer, queue, cron,
scheduler, route, public caller, or automatic trigger. Automatic O11
scheduling, hypothetical future reconnect or rollback features, and immutable-
evidence deletion are not current logical-retirement prerequisites. Future
consumers must register a pin before activation; immutable evidence and
physical rows remain retained until the separately decomposed purge owners are
approved. Reusing a retired content-addressed definition requires explicit
reactivation plus the existing build/backfill/validation proof; silent flag
clearing is forbidden. The docs-only `M05-C0` preflight now rejects one combined
purge: the lifecycle row has no purge phase or cursor, O11 owns ordered-index
