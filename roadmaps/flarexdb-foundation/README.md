...
| Managed schema | Private `M01-A` through `M04-C`, retirement preflight `M05-P`, exact workspace recovery `M05-A`, atomic supersession reclamation `M05-A2`, docs-only retirement gate reconciliation `M05-B0`, and docs-only scope-local storage preflight `M05-B1-P` are complete and production-inert; logical retirement next requires one additive scope-local lifecycle and bounded current-pin proof, while physical/evidence purge remains separate |
...
a deployment-global tenant drain. The immutable control-catalog definition
remains reusable by other scopes. One bounded current row records lifecycle,
definition digest, authority pins, transition fence, and replay digest; it does
not copy session/action/task pins or add an event log. Bounded checks cover the
scope's active/candidate selection, nonterminal Application mutation sessions,
admitted/executing actions, nonterminal durable tasks, O11 live retention pins,
and any currently supported resumable adapter. Automatic O11 scheduling,
hypothetical future reconnect or rollback features, and immutable-evidence
deletion are not current logical-retirement prerequisites. Future consumers
must register a pin before activation; immutable evidence and physical rows
remain retained until the separate `M05-C` purge owner is approved. Reusing a
retired content-addressed definition requires explicit reactivation plus the
existing build/backfill/validation proof; silent flag clearing is forbidden.
