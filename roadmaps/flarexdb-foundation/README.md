...
| Managed schema | Private `M01-A` through `M04-C`, retirement preflight `M05-P`, exact workspace recovery `M05-A`, atomic supersession reclamation `M05-A2`, docs-only retirement gate reconciliation `M05-B0`, docs-only scope-local storage preflight `M05-B1-P`, private scope-local lifecycle authority `M05-B1`, private readiness/admission composition `M05-B2`, and bounded current-pin finalization gate `M05-B3` are complete and production-inert; the manual coordinator in `M05-B4` and irreversible physical/evidence purge remain separate |
...
`retired` and `reactivating` are representable storage states but have no
callable transition yet. The immutable control-catalog definition remains
...
session/action/task pins or add an event log. `M05-B2` next owns bounded checks
over the scope's active/candidate selection, nonterminal Application mutation
sessions, admitted/executing actions, nonterminal durable tasks, O11 live
retention pins, and any currently supported resumable adapter. Automatic O11
