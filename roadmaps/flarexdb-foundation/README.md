...
- `A1b2c0b2c2b` remains blocked behind an authenticated command-plan authority
  ...
  `elapsedMilliseconds`. The remaining dimensions still have no accepted
  parser-, semantic-, diagnostic-, or evidence-multiplicity theorem. In
  particular, repeated caller or function text in per-call evidence can make
  canonical, frame, hash, or output demand quadratic in source
  length. A uniform source-length worst case may therefore exceed the
  u32-addressable arena while rejecting a same-length module whose terminal
  actual usage is low. Reserved arena, destination, frame, or other peak
