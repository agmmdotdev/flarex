...
39. [`preflight/38-medusa-product-tags.md`](./preflight/38-medusa-product-tags.md)
    ...
    authenticated DTO echoes: 84 originals per driver at that checkpoint.
...
43. [`preflight/42-medusa-product-categories.md`](./preflight/42-medusa-product-categories.md)
    adds the complete 21-case public Categories family, tree hydration, rank and
    path maintenance, scoped Product membership and leaf deletion; the two
    internal Product/Category files are next.
44. Current code, migrations, any source snapshot already admitted by its own
...
| [`preflight/14-transaction-execution-profiles.md`](./preflight/14-transaction-execution-profiles.md) | Accepted direction; individual publication paths and private no-event Currency/CMS/Application command implemented; broader composition gated | Shared transaction ownership, separate Application/framework/workflow execution profiles, nested failure and event boundaries, and explicitly gated cross-domain atomic commands |
