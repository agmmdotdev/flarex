...
| Currency static manifest | The [Currency runtime inventory](./medusa-currency-runtime-inventory.json) reproduces 76 inputs with recorded bundler/version, explicit source aliases and externalization policy | This is the manifest's runtime graph; static bootstrap, erased types, repositories and the full promotion closure remain distinct |
...
The Drizzle number remains exploratory discovery evidence. The Currency
preflight regenerates the static-manifest runtime graph with the exact policy
in its companion inventory. A promotion still needs a checked-in verifier and
complete type/build/test closure; neither runtime count is a complete package map.
