...
transition chain, and the availability-head repository adds its exact mutable
CAS pointer. No live target authority is minted.
...
The immutable schema-readiness repository accepts one exact restored
installation and one captured readiness receipt. It corroborates the complete
stored installation and successful-terminal lineage in the caller's
transaction, then resolves the unique installation storage identity before
consulting the independent global readiness digest. A conflict occupant is
restored through its actual stored installation chain before classification.

An untargeted immutable insert stores the exact installation identity and
receipt, readiness and validation digests, validated structure, and canonical
frame. Exact replay returns the committed storage identity; a second authentic
readiness receipt for the same installation is an immutable conflict. Missing
caller parents are refused, while malformed normalized projections, changed
canonical bytes, and over-limit stored bytes fail as stored corruption without
repair or mutation. The read path applies the four-megabyte SQL byte gate
before transfer and preserves caller transaction ownership and exact foreign
driver causes.

Focused PGlite functional evidence covers source privacy, absence,
ensure/read/exact and separately captured replay, exact normalized storage,
one-receipt-per-installation conflict, missing-parent refusal, projection and
canonical corruption without healing, the length-first byte gate, caller
rollback, and exact driver-cause projection. This remains production-inert
...
The mutable migration-collision-head repository accepts one exact restored
collision and admission, nullable restored current-attempt and last-event
authorities, and one already-captured head. Initialization corroborates the
complete supplied graph in the caller's transaction, inserts only when the
collision lane is absent, and reloads the committed occupant. Exact replay
returns that occupant; a different authentic incumbent returns `staleHead`
without mutation.

A cold read restores a non-null current attempt first and derives its exact
collision, plan, and admission identities before restoring the last event
against that same collision. With no current attempt it restores the admission
first. The restored head retains its nullable attempt and event dependencies in
source-private authority so checkpoint 3 can continue an authenticated lane
without exposing storage handles through the package surface.
...
initialization/read/replay, populated and all-null optional projections, exact
CAS including clearing those projections, same-revision/different-digest stale
behavior, different-initializer refusal, cold identity re-anchoring, normalized
raw storage, cross-store expected-authority refusal, projection corruption
without healing, the canonical length-first gate, caller rollback, and exact
foreign driver-cause projection. This does not prove genuine-PostgreSQL
...
The mutable schema-availability-head repository accepts one exact restored
history and its already-captured head. Initialization fully corroborates the
history chain in the caller's transaction, inserts only when the installation
lane is absent, and reloads the committed occupant. Exact replay returns that
occupant; a different authentic initializer returns `staleHead` without
mutation. A cold read first corroborates the supplied installation, then
restores the head's referenced history, readiness, and installation identities
before authenticating the head itself.

Compare-and-swap authenticates the outer expected restored head, corroborates
its complete private history chain, and exactly matches the current stored
occupant before preparing the next history. The update predicate is precisely
`(installation_storage_id, old_availability_sequence,
old_availability_head_sha256)`. A zero-row update is `staleHead`; a successful
update is reloaded and checked against the next captured head. The kernel adds
no sequence increment, monotonicity, latest-history, status-transition, retry,
lock, or freshness policy.

Focused PGlite evidence covers source privacy, absence, exact initialization,
read and replay, normalized raw storage, cold identity reconstruction, direct
and backward CAS, stale expected and initializer behavior, forged and cross-
store private-history refusal, corrupt normalized projections and history
references without healing, the length-first one-megabyte byte gate, caller
rollback, and exact foreign driver-cause projection. This does not prove
