...
The separately approved `QSYNC01-D1` implementation is now complete. It adds
the private source-only transition-plan foundation, one shared accounting
owner, initialization and begin planners, the first staged admitted-batch
planner, planner-backed aggregate/reference integration, and independent
normalized equivalence proof. It adds no package export, SQLite, backend, or
production caller. D2-D4 remain separately gated.
...
| `QSYNC01-D` | Operation-scoped transition-plan seam with bounded fact inputs, explicit logical changes, exact counters, and equivalence to the aggregate reducers | D0 accepted; D1 complete and private; D2-D4 not implemented or authorized |
...
- [`preflight/09-qsync01-d-operation-scoped-transition-plans.md`](./preflight/09-qsync01-d-operation-scoped-transition-plans.md)
  is the accepted D0 architecture freeze and completed D1 implementation
  record for the private planner boundary, facts, staged apply, shared
  accounting, initialization/begin integration, and proof matrix.

## Next Correctness Gate

The next correctness decision is whether to approve `QSYNC01-D2`, moving
`completeQueryEvaluation` alone onto the private planner seam. D1 is complete;
D2-D4 must complete all nine planners before a fresh FX01 checkpoint may
discuss SQLite.
