...
| [Payload proof runtime](../../packages/persistence-postgres/src/payloadScalar/runtime.ts) and [profile](../../packages/persistence-postgres/src/payloadScalar/profile.ts) | Real Local API calls use `overrideAccess: false`, but the fixture collection allows all access and installs title-triggered fault hooks. These are conformance behavior, not an application authorization or ordinary content runtime. |
