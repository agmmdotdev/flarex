...
Status: preflight complete for the first atomic workflow's events; the proposed
shared publication/persistence extension is pending approval as part of
[native Medusa workflow integration](./10-medusa-workflow-integration.md).
...
## Existing Owners And Missing Capabilities
...
| [Atomic host](../../packages/persistence-postgres/src/atomicCommerce/host.ts) | Refuses events. Extend selected participant admission/validation without weakening the existing event-free profile. |
| [Shared publication](../../packages/persistence-postgres/src/commitPublication/publication.ts) | Reuse one sequence, header, retained result, common wake and physical commit. Add an authenticated event contribution; the current model has none. |
...
requires typed event intents tied to the same commit and common wake. That is
a target; current code and local Product proof do not already implement it.
...
Release marks it eligible; it does not invoke subscribers or commit. Failure
discards the group with the database rollback.

No persisted open-group state machine is needed for this atomic profile.
Release/clear authority is bound to the current root, not inferred from a group
string. Any retained event-name clearing affects only its uncommitted group.
No cross-run clearing, pause across commits or after-commit release is admitted.
...
The proposed migration adds an immutable committed-event family and durable
...
it is not an independently writable second event feed. Production polling/wake
fan-out remains a later host decision.
...
Migration, capture, durable pump and actual workflow form one proposed completion
