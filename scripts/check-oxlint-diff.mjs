...
const OXLINT_POLICY_PATHS = Object.freeze([
  "oxlint.config.ts",
  "package.json",
  "tools/oxlint",
]);
 ...
 */
const assertPolicyMatchesSnapshot = (sourceMode) => {
  ...
  if (result.status === 1) {
    ...
    throw new Error(
      `Oxlint policy files differ from the ${snapshot} snapshot; stage or revert those policy edits before running this mode.`,
    );
  }
