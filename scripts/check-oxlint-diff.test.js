...
const createPolicyRepository = () => {
  const repositoryRoot = mkdtempSync(join(tmpdir(), "flarex-oxlint-policy-test-"));
  mkdirSync(join(repositoryRoot, "scripts"), { recursive: true });
  mkdirSync(join(repositoryRoot, "tools", "oxlint"), { recursive: true });
  writeFileSync(join(repositoryRoot, "package.json"), "{}\n", "utf8");
  writeFileSync(join(repositoryRoot, "oxlint.config.ts"), "export default {};\n", "utf8");
  writeFileSync(
    join(repositoryRoot, "scripts", "check-oxlint-diff.mjs"),
    "export {};\n",
    "utf8",
  );
  writeFileSync(join(repositoryRoot, "tools", "oxlint", "README.md"), "policy\n", "utf8");

  for (const args of [
    ["init", "--quiet"],
    ["config", "user.email", "oxlint-test@flarex.invalid"],
    ["config", "user.name", "Flarex Oxlint Test"],
    ["add", "."],
...
describe("snapshot policy integrity", () => {
  it("rejects untracked policy files in staged and committed modes", () => {
    const repositoryRoot = createPolicyRepository();
    try {
      writeFileSync(
        join(repositoryRoot, "tools", "oxlint", "untracked-rule.ts"),
        "export {};\n",
        "utf8",
      );
      expect(() => assertPolicyMatchesSnapshot("index", repositoryRoot)).toThrow(
        "staged snapshot",
      );
      expect(() => assertPolicyMatchesSnapshot("head", repositoryRoot)).toThrow(
  ...
  it("treats the gate implementation as snapshot policy", () => {
    const repositoryRoot = createPolicyRepository();
    try {
      writeFileSync(
        join(repositoryRoot, "scripts", "check-oxlint-diff.mjs"),
