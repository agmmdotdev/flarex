...
const testLockPath = (file: string) => path.join("/test/path", file)
...
describe("PackageManager", () => {
  ...
  describe("removeLockFiles", () => {
    ...
    it("should remove yarn.lock, pnpm-lock.yaml, and .yarn when using npm", async () => {
      ...
      expect(mockExistsSync).toHaveBeenCalledWith(testLockPath("yarn.lock"))
      expect(mockExistsSync).toHaveBeenCalledWith(
        testLockPath("pnpm-lock.yaml")
      )
      expect(mockExistsSync).toHaveBeenCalledWith(testLockPath(".yarn"))
      expect(mockRmSync).toHaveBeenCalledWith(testLockPath("yarn.lock"), {
        force: true,
        recursive: true,
      })
      ...
      expect(mockRmSync).toHaveBeenCalledWith(testLockPath(".yarn"), {
        force: true,
        recursive: true,
      })
    })
    ...
    it("should remove package-lock.json and pnpm-lock.yaml when using yarn", async () => {
      ...
      expect(mockExistsSync).toHaveBeenCalledWith(
        testLockPath("package-lock.json")
      )
      expect(mockExistsSync).toHaveBeenCalledWith(
        testLockPath("pnpm-lock.yaml")
      )
