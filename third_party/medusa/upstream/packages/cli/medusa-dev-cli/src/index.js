...
You typically only need to configure this once.`
      ...
      .describe(
        `external-registry`,
        `Run package-manager add commands without the --registry flag.`
      )
      .alias(`C`, `copy-all`)
      .nargs(`C`, 0)
      .describe(
...
`
    ...
    try {
      const workspaceManifest = yaml.load(
        fs.readFileSync(path.join(medusaLocation, "pnpm-workspace.yaml"), "utf8")
      )
      for (const workspace of workspaceManifest.packages) {
      ...
      console.error(
        `Unable to read and parse the workspace definition from pnpm-workspace.yaml`
      )
      process.exit(1)
    }
