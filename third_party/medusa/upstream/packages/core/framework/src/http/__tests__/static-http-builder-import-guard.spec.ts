...
const forbiddenStaticBuilderInputs = [
...
describe("static HTTP builder import graph", () => {
  it("does not pull logger, Express, filesystem discovery, or Node built-ins into the browser bundle", async () => {
    const result = await build({
      entryPoints: [resolve(__dirname, "../static.ts")],
      bundle: true,
      format: "esm",
      metafile: true,
      platform: "browser",
    ...
    const forbiddenInputs = bundledInputs.filter((input) =>
      forbiddenStaticBuilderInputs.some((pattern) => pattern.test(input))
    )
  ...
  it("exposes the Worker-safe static HTTP package subpath", () => {
    expect(frameworkPackageJson.exports["./http/static"]).toEqual({
      types: "./dist/http/static.d.ts",
      default: "./dist/http/static.js",
    })
  })
})
