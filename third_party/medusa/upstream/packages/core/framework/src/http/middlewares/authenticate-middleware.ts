...
type AuthConfigModule = {
  projectConfig: {
    http: {
      ...
      jwtOptions?: SignOptions | VerifyOptions
    }
  }
}

type JsonWebTokenRuntime = {
  verify: (
    token: string,
...
export const authenticate = (
  ...
  const authenticateMiddleware = async (
    ...
    // We try to extract the auth context from an upstream runtime hook, the
    // session, or a JWT token.
    let authContext: AuthContext | null = getPermittedExistingAuthContext(
      req,
      actorTypes
    )

    authContext ??= getAuthContextFromSession(
      req.session,
      authTypes,
      actorTypes
    )
    ...
    if (!authContext) {
      const {
        projectConfig: { http },
      } = req.scope.resolve<AuthConfigModule>(
...
const getPermittedExistingAuthContext = (
  req: MedusaRequest,
  actorTypes: string[]
): AuthContext | null => {
  ...
  if (matches) {
    ...
    if (tokenType.toLowerCase() === BEARER_AUTH) {
      ...
      try {
        ...
        const verified = loadJsonWebTokenRuntime().verify(
          token,
...
function loadJsonWebTokenRuntime(): JsonWebTokenRuntime {
  const requireFunction = getCommonJsRequire()
  if (!requireFunction) {
    throw new Error(
      "JWT bearer authentication requires a JSON Web Token verifier in this runtime"
    )
  }

  const runtime = requireFunction("jsonwebtoken")
  if (!isJsonWebTokenRuntime(runtime)) {
    throw new Error("Invalid JSON Web Token verifier runtime")
  }
...
function getCommonJsRequire(): CommonJsRequire | undefined {
  if (
    typeof module !== "undefined" &&
    typeof module.require === "function"
  ) {
    return module.require.bind(module) as CommonJsRequire
  }

  return undefined
}

function isJsonWebTokenRuntime(
  value: unknown
): value is JsonWebTokenRuntime {
  return (
    typeof value === "object" &&
    value !== null &&
    "verify" in value &&
