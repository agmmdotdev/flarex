...
import { defineJoinerConfigFromModels } from "../portable-joiner-config-builder"

describe("joiner-config-builder", () => {
  it("keeps explicit-model portable config equivalent to the Node entrypoint", () => {
    const models = [dmlFulfillmentSet]

    expect(
      defineJoinerConfigFromModels(Modules.FULFILLMENT, { models })
    ).toEqual(defineJoinerConfig(Modules.FULFILLMENT, { models }))
  })
