...
describe("Internal Module Service Factory", () => {
  ...
  describe("Internal Module Service Methods", () => {
    ...
    it("should dispatch delete events through the selected persistence adapter", async () => {
      const dispatchMutationEvent = jest.fn()
      const repository = {
        ...containerMock[modelRepositoryName],
        transaction: (task) =>
          task({
      ...
      const service = new IMedusaInternalService({
        ...containerMock,
        [modelRepositoryName]: repository,
      ...
      expect(dispatchMutationEvent).toHaveBeenCalledWith(
        "afterDelete",
        {
          entity: { id: "1" },
