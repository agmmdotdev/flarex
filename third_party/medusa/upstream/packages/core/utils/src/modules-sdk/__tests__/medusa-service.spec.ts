...
describe("Abstract Module Service Factory", () => {
  ...
  it("uses the selected persistence adapter to create mutation subscribers", () => {
    class SelectedSubscriber {}

    const setEventSubscriber = jest.fn()
    const createEventSubscriber = jest.fn(() => SelectedSubscriber)
    ...
    const serviceClass = MedusaService({ MainModelMock })
    serviceClass.prototype.__joinerConfig = () =>
      ({ serviceName: "selected" } as ModuleJoinerConfig)
    ...
    expect(createEventSubscriber).toHaveBeenCalledTimes(1)
    expect(setEventSubscriber).toHaveBeenCalledWith(SelectedSubscriber)
  })
