...
import { mapObjectTo } from "../common/map-object-to"
import type { MapToConfig } from "../common/map-object-to"
import { pluralize } from "../common/plurailze"
import { upperCaseFirst } from "../common/upper-case-first"
import { DmlEntity } from "../dml/entity"
import { CommonEvents } from "../event-bus/common-events"
...
import { buildModelsNameToLinkableKeysMap } from "./build-models-name-to-linkable-keys-map"
import { isMedusaInternalService } from "./medusa-internal-service-marker"
import type {
  BaseMethods,
  ...
  class AbstractModuleService_ {
    ...
    constructor(container: Record<any, any>) {
      ...
      if (joinerConfig?.serviceName !== "index") {
        const persistenceAdapter = container[
          ContainerRegistrationKeys.MODULE_PERSISTENCE_ADAPTER
        ] as ModulePersistenceAdapter | undefined
          ...
          .forEach((key: string) => {
            try {
              ...
              if (
                ...
                isMedusaInternalService(service)
              ) {
                globalSubscriber ??= persistenceAdapter.createEventSubscriber(
     ...
     *
     * class MyService extends ModulesSdkUtils.MedusaService(models) {
     *   interceptEntityMutationEvents(event: "afterCreate" | "afterUpdate" | "afterUpsert" | "afterDelete", args: ModulePersistenceMutationEventArgs, context: Context) {
