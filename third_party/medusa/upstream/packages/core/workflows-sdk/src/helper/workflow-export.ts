import { MedusaModule } from "@medusajs/modules-sdk/medusa-module"
import {
...
import { LocalWorkflow } from "@medusajs/orchestration/workflow/local-workflow"
import type {
  Context,
...
} from "@medusajs/utils/common/container"
import { createPortableId } from "@medusajs/utils/common/create-portable-id"
import { isPresent } from "@medusajs/utils/common/is-present"
import { MedusaContextType } from "@medusajs/utils/modules-sdk/decorators/context-parameter"
...
> &
  ...
  const newRun = async ({
    ...
    const uniqId = createPortableId("workflow")
  ...
  const newRegisterStepSuccess = async (
    ...
    context.eventGroupId ??= createPortableId("event_group")
  ...
  const wrappedOnFinish = async (args: {
    ...
    if (logOnError) {
      ...
      transaction.getErrors().forEach((err) => {
        ...
        logger.error(
          `${workflowName}:${err?.action}:${err?.handlerType}${errMsg}\n${err?.error?.stack}`
        )
      })
    }
