...
>): StepFunctionResult<TInvokeResultOutput> {
  return function (this: CreateWorkflowComposerContext) {
    ...
    stepConfig.uuid = createPortableId("step")
    ...
    refRet.config = (
      ...
      ] as { steps: Array<{ __step__?: string }> } | undefined

      if (conditionContext) {
        const flagSteps = conditionContext.steps
    ...
    if (conditionContext) {
      conditionContext.steps.push(refRet)
    }
