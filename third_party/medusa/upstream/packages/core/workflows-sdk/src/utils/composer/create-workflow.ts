...
import {
  WorkflowHandler,
  WorkflowManager,
} from "@medusajs/orchestration/workflow/workflow-manager"
import type {
...
import * as OrchestrationUtils from "@medusajs/utils/orchestration/symbol"
import { exportWorkflow, WorkflowResult } from "../../helper"
import {
  getCallerFilePath,
  registerDevServerResource,
} from "./helpers/dev-server-resource"
import { createStep } from "./create-step"
import { proxify } from "./helpers/proxy"
import { composerGlobal } from "./helpers/composer-global"
...
composerGlobal[OrchestrationUtils.SymbolMedusaWorkflowComposerContext] = null
...
const buildTransactionId = (
  ...
  return (
    step.__step__ +
    "-" +
    (stepContext.transactionId ?? createPortableId("txn")) +
...
export function createWorkflow<TData, TResult, THooks extends any[]>(
  ...
  composerGlobal[OrchestrationUtils.SymbolMedusaWorkflowComposerContext] =
    context
  ...
  delete composerGlobal[OrchestrationUtils.SymbolMedusaWorkflowComposerContext]
  ...
  mainFlow.runAsStep = ({
    ...
    const workflowCompositionContext =
      ...
      ] as CreateWorkflowComposerContext | undefined

    if (!workflowCompositionContext) {
      throw new Error("runAsStep must be used inside a createWorkflow definition")
    }
