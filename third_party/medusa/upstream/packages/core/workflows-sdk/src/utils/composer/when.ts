import { isDefined } from "@medusajs/utils/common/is-defined"
...
export function when(...args) {
  ...
  composerGlobal[OrchestrationUtils.SymbolMedusaWorkflowComposerCondition] = {
    input,
    condition,
    steps: [],
  }
  ...
  return {
    then: (fn) => {
      ...
      const conditionContext = composerGlobal[
        OrchestrationUtils.SymbolMedusaWorkflowComposerCondition
      ] as { steps: Array<{ if(input: unknown, condition: unknown): void }> }

      const applyCondition = conditionContext.steps
      ...
      if (
        ...
        if (!isDefined(name)) {
          name = "when-then-" + createPortableId("condition")
          const context =
            composerGlobal[
