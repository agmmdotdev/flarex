...
type NodeUploadMiddleware = (
  req: MedusaRequest,
  res: MedusaResponse,
  next: MedusaNextFunction
) => void

type NodeMulterFactory = {
  (options: { storage: unknown }): {
    single(fieldName: string): NodeUploadMiddleware
  }
  memoryStorage(): unknown
}

function createNodeMulterUpload() {
  return {
    single(fieldName: string): NodeUploadMiddleware {
      return (req, res, next) => {
        ...
        if (!nodeRequire) {
          next(new Error("File uploads require the Node HTTP adapter"))
          return
        }

        const multer = resolveNodeMulterFactory(nodeRequire("multer"))
        if (!multer) {
          next(new Error("Unable to load multer for product import uploads"))
          return
        }
...
function resolveNodeMulterFactory(value: unknown): NodeMulterFactory | null {
  if (isNodeMulterFactory(value)) {
    return value
  }
  ...
  if (
    ...
    value !== null &&
    "default" in value &&
    isNodeMulterFactory(value.default)
  ) {
...
function isNodeMulterFactory(value: unknown): value is NodeMulterFactory {
  return (
    typeof value === "function" &&
    "memoryStorage" in value &&
    typeof value.memoryStorage === "function"
  )
}

const upload = createNodeMulterUpload()
