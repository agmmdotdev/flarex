...
type PromotionRefinementInput = {
  campaign?: unknown
  campaign_id?: unknown
  type?: PromotionType
  application_method?: {
    buy_rules?: unknown
    apply_to_quantity?: unknown
    buy_rules_min_quantity?: unknown
  } | null
  is_automatic?: boolean
  limit?: number | null
}

const promoRefinement = (promo: PromotionRefinementInput) => {
  ...
  if (promo.type === PromotionType.BUYGET) {
    ...
    const buyRuleCount = Array.isArray(appMethod?.buy_rules)
      ? appMethod.buy_rules.length
      : 0
