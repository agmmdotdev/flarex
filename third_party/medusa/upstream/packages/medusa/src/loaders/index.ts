...
type Options = {
  ...
  apiResourceResolver?: HttpResourceResolver
}
...
async function loadEntrypoints(
  ...
  expressApp.use((req: Request, res: Response, next: NextFunction) => {
    setupMedusaHttpRequest(req, {
      container,
      requestId: getExpressRequestId(req) ?? v4(),
    })
    next()
  })
  ...
  const [{ shutdown }] = await promiseAll([
    ...
    apiLoader({
      ...
      plugins,
      app: expressApp,
      resourceResolver: apiResourceResolver,
    }),
  ])
...
function getExpressRequestId(req: Request): string | undefined {
