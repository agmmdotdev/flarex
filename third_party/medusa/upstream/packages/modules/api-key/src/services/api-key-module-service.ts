...
import { randomHex, scryptHex } from "../utils/api-key-crypto"
import { joinerConfig } from "../joiner-config"

type InjectedDependencies = {
  baseRepository: DAL.RepositoryService
  apiKeyService: ModulesSdkTypes.IMedusaInternalService<
  ...
  implements IApiKeyModuleService
{
  ...
  async createApiKeys(
    ...
    const persistedApiKeys = await this.apiKeyService_.list(
      {
        id: createdApiKeys.map((apiKey) => apiKey.id),
      },
      {},
      sharedContext
    )
    ...
    const responseWithRawToken = serializedResponse.map((key) => ({
        ...
        token:
          ...
          key.token,
        updated_at: key.created_at ?? key.updated_at,
        salt: undefined,
      }))
  ...
  async updateApiKeys(
    ...
    const response = serializedResponse.map((key) => ({
      ...key,
      salt: undefined,
    }))

    return isString(idOrSelector) ? response[0] : response
  }
  ...
  protected async updateApiKeys_(
    ...
    const existingApiKeys = await this.apiKeyService_.list(
      {
        id: normalizedInput.map((k) => k.id),
      },
      {},
      sharedContext
    )
    const existingById = new Map(
      existingApiKeys.map((apiKey) => [apiKey.id, apiKey])
    )
    const updateRequest = normalizedInput
      .filter((k) => {
        const existing = existingById.get(k.id)
        return !existing || (k.title !== undefined && existing.title !== k.title)
      })
      .map((k) => ({
        id: k.id,
        title: k.title,
    ...
    const updatedApiKeys = updateRequest.length
      ? await this.apiKeyService_.update(updateRequest, sharedContext)
      : []
    const updatedById = new Map(
      updatedApiKeys.map((apiKey) => [apiKey.id, apiKey])
    )

    return normalizedInput.flatMap((input) => {
      const updated = updatedById.get(input.id)
      if (updated) {
        return [updated]
      }
  ...
  protected static async calculateHash(
    ...
    return await scryptHex(token, salt, 64)
  }
}
