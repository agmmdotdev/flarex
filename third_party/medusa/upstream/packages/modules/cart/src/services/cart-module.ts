  ...
  implements ICartModuleService
{
  ...
  private addRelationsToCalculateTotals(config: FindConfig<any>, totalFields) {
    ...
    const requiredScalarFieldsForTotals = [
      ...requiredFieldsForTotals.map((relation) => `${relation}.*`),
      "items.id",
      "items.quantity",
      "items.unit_price",
      "items.title",
    ...
    config.select = deduplicate([
      ...config.select.filter((field) => {
        return (
