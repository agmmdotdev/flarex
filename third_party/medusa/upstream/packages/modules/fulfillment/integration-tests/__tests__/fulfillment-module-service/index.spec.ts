  ...
  testSuite: ({
    ...
    describe("Fulfillment Module Service", () => {
      ...
      it("should load and save all the providers on bootstrap with the correct is_enabled value", async () => {
        if (persistenceAdapter.name === "drizzle") {
          const fulfillmentProviders = await service.listFulfillmentProviders(
            {}
          )
          expect(fulfillmentProviders).toEqual(
            expect.arrayContaining([
        ...
        let initializedModules = await initModules(moduleOptions)
        let shutdown = initializedModules.shutdown
        let fulfillmentProviders =
          await initializedModules.medusaApp.modules[
            Modules.FULFILLMENT
        ...
        initializedModules = await initModules(moduleOptions)
        shutdown = initializedModules.shutdown
