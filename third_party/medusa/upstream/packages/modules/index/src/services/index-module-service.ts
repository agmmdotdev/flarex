...
import type { IndexMetadata, IndexSync } from "../models"
...
import type { DataSynchronizer } from "./data-synchronizer"

export type IndexTransactionManager = unknown

export type IndexResetHandler = {
  reset(transactionManager: IndexTransactionManager): Promise<void>
}

export type IndexBaseRepository = {
  transaction<TResult>(
    task: (
...
type IndexTypeGenerator = (
  executableSchema: GraphQLUtils.GraphQLSchema
) => Promise<void>

type IndexConfigurationChecker = {
  checkChanges(): Promise<InferEntityType<typeof IndexMetadata>[]>
}

type IndexInternalService = ModulesSdkTypes.IMedusaInternalService<object>

type IndexConfigurationCheckerFactory = (input: {
  logger: Logger
  schemaObjectRepresentation: IndexTypes.SchemaObjectRepresentation
  indexMetadataService: IndexInternalService
  indexSyncService: IndexInternalService
...
type InjectedDependencies = {
  ...
  indexTypeGenerator?: IndexTypeGenerator
  indexConfigurationCheckerFactory?: IndexConfigurationCheckerFactory
  indexMetadataService: IndexInternalService
  indexSyncService: IndexInternalService
  dataSynchronizer: DataSynchronizer
}
  ...
  implements IndexTypes.IIndexService
{
  ...
  private readonly baseRepository_: IndexBaseRepository
  ...
  private configurationChecker_: IndexConfigurationChecker

  private get indexMetadataService_(): IndexInternalService {
    return this.container_.indexMetadataService
  }

  private get indexSyncService_(): IndexInternalService {
  ...
  protected async onApplicationStart_() {
    try {
      ...
      if (executableSchema && this.container_.indexTypeGenerator) {
        await this.container_.indexTypeGenerator(executableSchema)
      }
      ...
      if (this.#isWorkerMode) {
        ...
        if (!this.container_.indexConfigurationCheckerFactory) {
          throw new Error(
            "Index worker mode requires an index configuration checker factory"
          )
        }

        this.configurationChecker_ =
          this.container_.indexConfigurationCheckerFactory({
            logger: this.logger_,
            schemaObjectRepresentation: this.schemaObjectRepresentation_,
  ...
  private async resetSync() {
    if (!this.#isWorkerMode) {
      await this.baseRepository_.transaction(
        async (transactionManager: IndexTransactionManager) => {
          await this.getIndexResetHandler_().reset(transactionManager)
          await this.eventBusModuleService_.emit({
  ...
  private getIndexResetHandler_(): IndexResetHandler {
    const indexResetHandler = this.container_.indexResetHandler

    if (!indexResetHandler) {
      throw new Error(
        "Index reset requires an index reset handler for the selected storage provider"
      )
    }

    return indexResetHandler
  }
}
