  ...
  testSuite: ({ service }) => {
    describe("Order Module Service", () => {
      ...
      it("should delete entirely an order, shipping method and items. Including taxes and adjustments associated with them and credit lines", async function () {
        ...
        const createdWithAddresses = await service.retrieveOrder(created.id, {
        ...
        async function findAllOrderRelatedEntities(orderId: string) {
          ...
          const [
            ...
            service.listOrders({ id: created.id }),
            service.listOrderItems(orderIdClause),
            service.listOrderSummaries(orderIdClause),
            service.listOrderShippings(orderIdClause),
            service.listOrderTransactions(orderIdClause),
            service.listOrderCreditLines(orderIdClause),
            service.listOrderAddresses({
              id: createdWithAddresses.shipping_address!.id,
            }),
            service.listOrderAddresses({
              id: createdWithAddresses.billing_address!.id,
            }),
          ])

          return {
            order: orders[0] ?? null,
            orderItems: orderItems[0] ?? null,
            orderSummary: orderSummaries[0] ?? null,
            orderShippingMethods: orderShippingMethods[0] ?? null,
            orderTransactions: orderTransactions[0] ?? null,
            orderCreditLines: orderCreditLines[0] ?? null,
            orderShippingAddress: orderShippingAddresses[0] ?? null,
            orderBillingAddress: orderBillingAddresses[0] ?? null,
          }
        }
