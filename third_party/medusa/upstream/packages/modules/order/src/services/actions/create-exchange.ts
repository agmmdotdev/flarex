...
function createExchangeAndReturnEntities(data, order): {
  exchangeReference: MutableOrderEntity
  returnReference: MutableOrderEntity
} {
  const exchangeReference: MutableOrderEntity = {
    id: generateEntityId(undefined, "oexc"),
  ...
  const returnReference: MutableOrderEntity = {
  ...
  data.additional_items?.forEach((item) => {
    ...
    if (hasItem) {
      ...
      additionalItems.push(
        {
          id: generateEntityId(undefined, "oexcitem"),
          item_id: item.id,
          quantity: item.quantity,
          note: item.note,
          metadata: item.metadata,
...
export async function createExchange(
  ...
  const { exchangeReference, returnReference } =
    createExchangeAndReturnEntities(data, order)
  ...
  const returnId = exchangeReference.return_id
  delete exchangeReference.return_id

  await this.createOrderExchanges([exchangeReference], sharedContext)
  await this.createReturns([returnReference], sharedContext)
  if (returnId) {
    await this.orderExchangeService_.update(
      [{ id: exchangeReference.id, return_id: returnId }],
      sharedContext
    )
    exchangeReference.return_id = returnId
  }
