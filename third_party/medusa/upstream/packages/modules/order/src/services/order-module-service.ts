  ...
  implements IOrderModuleService
{
  ...
  protected async createOrderShippingMethodsBulk_(
    ...
    // The nested new shipping method is not cascade-persisted when creating
    // the order shipping row, so it would stay invisible to reads inside the
    // same transaction until commit. Create both levels explicitly so they
    // are queued on the entity manager before any subsequent read.
    const methods = await this.orderShippingMethodService_.create(
      data.map((d) => d.shipping_method) as any,
      sharedContext
    )

    const shippings = await this.orderShippingService_.create(
      data.map((d, index) => ({
        shipping_method: methods[index].id,
        order_id: d.order_id,
        version: d.version,
        return_id: (d as any).return_id,
        claim_id: (d as any).claim_id,
        exchange_id: (d as any).exchange_id,
      })),
      sharedContext
    )
    ...
    let orders = await this.listOrders_(
      { id: deduplicate(ordersIds) },
      {
        select: [
          ...
          "version",
          "items.detail",
          "shipping_methods.detail",
          "summary",
          "total",
        ],
        relations: ["transactions", "credit_lines", "shipping_methods"],
      },
      sharedContext
    )
