  ...
  testSuite: ({ service }) => {
    describe("PricingModule Service - Calculate Price", () => {
      describe("calculatePrices", () => {
        beforeEach(async () => {
          const priceSetsData: SeedPriceSetDTO[] = [
            { id: "price-set-EUR" },
            { id: "price-set-PLN" },
            { id: "price-set-ETH" },
          ]

          const pricesData: SeedPriceDTO[] = [
            {
              id: "price-PLN",
              title: "price PLN",
              price_set_id: "price-set-PLN",
          ...
          const priceRuleData: SeedPriceRuleDTO[] = [
            {
          ...
          await seedCalculatePriceData(service, {
            priceSetsData,
            pricesData,
            priceRuleData,
          })
        ...
        it("should successfully calculate prices with complex context", async () => {
          ...
          const calculatedPrice = await service.calculatePrices(
            ...
            { context: context as PricingTypes.PricingContext["context"] }
          )
...
type SeedPriceDTO = CreatePricesDTO & {
  id: string
  price_set_id: string
  title?: string
  rules_count?: number
...
async function seedCalculatePriceData(
  service: IPricingModuleService,
  {
    priceSetsData,
    pricesData,
    ...
    priceSetsData: SeedPriceSetDTO[]
    pricesData: SeedPriceDTO[]
    priceRuleData: SeedPriceRuleDTO[]
  }
) {
  const rulesByPriceId = new Map<string, NonNullable<CreatePricesDTO["rules"]>>()
  for (const rule of priceRuleData) {
    if (!rule.price_id) {
      continue
    }

    const priceRules = rulesByPriceId.get(rule.price_id) ?? {}
    priceRules[rule.attribute] = rule.value
    rulesByPriceId.set(rule.price_id, priceRules)
  }

  const pricesByPriceSet = new Map<string, CreatePricesDTO[]>()
  for (const price of pricesData) {
    const { price_set_id, ...priceData } = price
    const prices = pricesByPriceSet.get(price_set_id) ?? []
    prices.push({
      ...priceData,
      rules: rulesByPriceId.get(price.id),
    })
    pricesByPriceSet.set(price_set_id, prices)
  }

  await service.createPriceSets(
    priceSetsData.map((priceSet) => ({
      ...priceSet,
      prices: pricesByPriceSet.get(priceSet.id) ?? [],
    }))
  )
}
