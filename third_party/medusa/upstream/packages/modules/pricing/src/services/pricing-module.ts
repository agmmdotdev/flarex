...
type PriceListDateInput = (
  | PricingTypes.CreatePriceListDTO
...
type NormalizedPriceListMutationInput = Omit<PriceListDateInput, "rules"> & {
  rules?: undefined
  price_list_rules?: {
    attribute: string
...
type RelationList<T> = T[] | { toArray(): T[] }
...
const generateMethodForModels = {
  ...
  Price,
  PricePreference,
  // Both rule models use the prule prefix; the linkable PriceRule must win.
  PriceListRule,
  PriceRule,
}
  ...
  implements PricingTypes.IPricingModuleService
{
  ...
  protected async updatePriceLists_(
    ...
    const normalizedData = this.normalizePriceListDate(data).map(
      (priceList) => {
        const entry: NormalizedPriceListMutationInput = {
          ...priceList,
          rules: undefined,
    ...
    const normalizedById = new Map<string, NormalizedPriceListMutationInput>()
    for (const input of normalizedData) {
      if ("id" in input && typeof input.id === "string") {
        normalizedById.set(input.id, input)
      }
    }

    return entities.map((entity) => {
      const normalized = normalizedById.get(entity.id)
      if (!normalized) {
        return entity
      }
      ...
      return {
        ...entity,
        starts_at:
          typeof normalized.starts_at === "string"
            ? normalized.starts_at
            : entity.starts_at,
        ends_at:
          typeof normalized.ends_at === "string"
            ? normalized.ends_at
            : entity.ends_at,
      }
    })
    ...
    const priceListsUpsert = priceLists
      .map((priceList) => {
        const allRules = new Map(
          relationListToArray(priceList.price_list_rules).map((r) => [
            r.attribute,
            r.value,
          ])
        )
...
function relationListToArray<T>(relation?: RelationList<T> | null): T[] {
  if (!relation) {
    return []
  }

  return Array.isArray(relation) ? relation : relation.toArray()
}
