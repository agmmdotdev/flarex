...
moduleIntegrationTestRunner<Service>({
  ...
  testSuite: ({ service: moduleService }) => {
    ...
    describe("Product Service", () => {
      ...
      describe("retrieve", () => {
        beforeEach(async () => {
          ;[productOne] = await moduleService.createProducts([{
            id: "product-1",
            title: "product 1",
            handle: "product-1",
      ...
      describe("list", () => {
        ...
        describe("relation: tags", () => {
          beforeEach(async () => {
            products = await createProductFixtures(moduleService, productsData)
          })
          ...
          it("should filter by id and including relations", async () => {
            ...
            productsResult.forEach((product, index) => {
              const tags = relationItems(product.tags)
        ...
        describe("relation: categories", () => {
          ...
          beforeEach(async () => {
            ...
            workingProduct = await moduleService.updateProducts(
              workingProduct.id,
              {
          ...
          it("should filter by categories relation and scope fields", async () => {
            ...
            expect(relationItems(product.categories)).toEqual([
              {
                id: "category-0",
                name: "category 0",
                handle: "category-0",
        ...
        describe("relation: collections", () => {
          ...
          beforeEach(async () => {
            const collections = await moduleService.createProductCollections(
              collectionData
            )
            workingCollection = collections.find(
              (collection) => collection.id === "test-1"
            )!
            workingCollectionTwo = collections.find(
              (collection) => collection.id === "test-2"
            )!
        ...
        describe("relation: variants", () => {
          beforeEach(async () => {
            products = await createProductFixtures(moduleService, productsData)
            await moduleService.createProductVariants(
          ...
          it("should filter by id and including relations", async () => {
            ...
            productsResult.forEach((product, index) => {
              const variants = relationItems(product.variants)
...
async function createProductFixtures(
  ...
  for (const entry of data) {
    const tags = entry.tags?.length
      ? await moduleService.createProductTags(entry.tags)
      : []
    const product = await moduleService.createProducts({
      id: entry.id,
      title: entry.title,
      handle: entry.handle,
      status: entry.status,
...
function relationItems<T>(
  ...
  if (relation?.getItems) {
    return relation.getItems(false)
  }
