...
> {
  buildFindOptions(
    ...
    // Do not invent an empty `fields` array: MikroORM treats `fields: []` as a
    // field-limited load, which drops mpath/parent_category_id and breaks tree
    // hydration on PostgreSQL after a prior select-limited find.
    if (
      familyOptions.includeDescendantsTree ||
      familyOptions.includeAncestorsTree
    ) {
      const treeFields = (findOptions_.options.fields ??= [])
      treeFields.indexOf("mpath") === -1 && treeFields.push("mpath")
      treeFields.indexOf("parent_category_id") === -1 &&
        treeFields.push("parent_category_id")
    }
    ...
    const shouldExpandParent =
      ...
      (Array.isArray(fields) &&
        fields.some(
          (field) =>
            typeof field === "string" && field.startsWith("parent_category.")
        ))
    ...
    const shouldExpandChildren =
      ...
      (Array.isArray(fields) &&
        fields.some(
          (field) =>
            typeof field === "string" && field.startsWith("category_children.")
        ))
  ...
  async create(
    ...
    const categories = await Promise.all(
      data.map(async (entry, i) => {
        ...
        if (Array.isArray((entry as { products?: { id: string }[] }).products)) {
          const productRefs = (
            entry as unknown as { products: { id: string }[] }
          ).products.map((product) =>
            manager.getReference(Product.name, product.id)
          )
          ;(categoryData as { products?: unknown }).products = productRefs
        }
