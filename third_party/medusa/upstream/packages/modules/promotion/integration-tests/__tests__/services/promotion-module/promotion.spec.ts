...
import { createDefaultCampaigns } from "../../../__fixtures__/campaigns"
import {
...
moduleIntegrationTestRunner({
  ...
  testSuite: ({ service }: SuiteOptions<IPromotionModuleService>) => {
    describe("Promotion Service", () => {
      beforeEach(async () => {
        await createDefaultCampaigns(service)
      })
      ...
      describe("listAndCount", () => {
        beforeEach(async () => {
          await createSeedPromotions(service, [
            {
              id: "promotion-id-1",
              code: "PROMOTION_1",
              type: PromotionType.STANDARD,
