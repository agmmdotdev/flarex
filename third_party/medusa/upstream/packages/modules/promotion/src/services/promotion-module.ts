  ...
  implements PromotionTypes.IPromotionModuleService
{
  ...
  constructor(
    ...
    const listPromotions = this.listPromotions.bind(this)
    this.listPromotions = async (filters, config, sharedContext = {}) => {
      return await listPromotions(
        filters,
        withDefaultRelations(config, ["application_method"]),
        sharedContext
      )
    }

    const listAndCountPromotions = this.listAndCountPromotions.bind(this)
    this.listAndCountPromotions = async (
      filters,
      config,
      sharedContext = {}
    ) => {
      return await listAndCountPromotions(
        filters,
        withDefaultRelations(config, ["application_method"]),
        sharedContext
      )
    ...
    const listCampaigns = this.listCampaigns.bind(this)
    this.listCampaigns = async (filters, config, sharedContext = {}) => {
      return await listCampaigns(
        filters,
        withDefaultRelations(config, ["budget"]),
        sharedContext
      )
    }

    const listAndCountCampaigns = this.listAndCountCampaigns.bind(this)
    this.listAndCountCampaigns = async (
      filters,
      config,
      sharedContext = {}
    ) => {
      return await listAndCountCampaigns(
        filters,
        withDefaultRelations(config, ["budget"]),
        sharedContext
      )
  ...
  protected async createPromotionRulesAndValues_(
    ...
    const promotionType =
      "code" in relation ? relation.type : relation.promotion?.type
    ...
    for (const ruleData of rulesData) {
      ...
      const promotionRuleValuesData = ruleValues.map((ruleValue) => ({
        value: ruleValue,
        promotion_rule: createdPromotionRule.id,
      }))
...
function withDefaultRelations<TEntity>(
  config: FindConfig<TEntity> | undefined,
  relations: string[]
): FindConfig<TEntity> | undefined {
  ...
  return {
    ...(config ?? {}),
    relations: [...new Set([...(config?.relations ?? []), ...relations])],
  }
}
