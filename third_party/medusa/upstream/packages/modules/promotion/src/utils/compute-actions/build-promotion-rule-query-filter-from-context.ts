...
export async function buildPromotionRuleQueryFilterFromContext(
  ...
  if (numberOfAttributes > 10) {
    const manager = (sharedContext.transactionManager ??
      sharedContext.manager) as unknown

    if (isKnexBackedManager(manager)) {
...
type KnexBackedManager = Pick<SqlEntityManager, "getKnex">

function isKnexBackedManager(value: unknown): value is KnexBackedManager {
  return (
    value !== null &&
    typeof value === "object" &&
    "getKnex" in value &&
    typeof value.getKnex === "function"
  )
}
