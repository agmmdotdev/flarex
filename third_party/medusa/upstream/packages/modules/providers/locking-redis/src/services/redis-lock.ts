...
export class RedisLockingProvider implements ILockingProvider {
  ...
  async execute<T>(
    ...
    const ownerId = `execute:${crypto.randomUUID()}`
    ...
    const ONE_MINUTE = 60
    const acquisitionPromise = this.acquire_(
      keys,
      {
        awaitQueue: true,
    ...
    promises.push(acquisitionPromise)
    ...
    try {
      ...
      void acquisitionPromise.catch(() => undefined)
      throw error
    } finally {
    ...
    const acquirePromises = keys.map(async (key) => {
      ...
      const acquireLock = async () => {
        while (true) {
          ...
          if (result === 1) {
            if (cancellationToken?.cancelled) {
              await this.redisClient.releaseLock(keyName, ownerId)
              throw new Error(errMessage)
            }
  ...
  private async getTimeout(
    ...
    await setTimeout(seconds * 1000, undefined, { signal })
    cancellationToken.cancelled = true
