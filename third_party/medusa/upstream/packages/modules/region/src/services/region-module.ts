...
type InjectedDependencies = {
  ...
  regionService: ModulesSdkTypes.IMedusaInternalService<typeof Region>
  countryService: ModulesSdkTypes.IMedusaInternalService<typeof Country>
}
  ...
  implements IRegionModuleService
{
  ...
  async createRegions_(
    ...
    result.forEach((region, index) => {
      if (
        !normalizedInput[index].countries?.length &&
        !("countries" in region)
      ) {
        Object.assign(region, { countries: [] })
      }
    })
  ...
  async deleteRegions(
    ...
    return await this.deleteRegions_(ids, sharedContext)
  }

  @InjectTransactionManager()
  protected async deleteRegions_(
    ids: string | string[],
    @MedusaContext() sharedContext: Context = {}
  ) {
    await this.countryService_.update(
    ...
    return await super.deleteRegions(ids, sharedContext)
  }
