  ...
  implements ITranslationModuleService
{
  ...
  async listTranslations(
    ...
    const configWithReference =
      TranslationModuleService.ensureReferenceFieldInConfig(config)
    const queryConfig = filters.q
      ? withoutPagination(configWithReference)
      : configWithReference
    ...
    const filtered = filterTranslationsByQuery(serialized, filters.q)

    return filterTranslationFields(
      filters.q ? applyPagination(filtered, config) : filtered,
  ...
  async listAndCountTranslations(
    ...
    return [
      filterTranslationFields(
        filters.q ? applyPagination(filtered, config) : filtered,
        translatableFieldsConfig
      ),
      filters.q ? filtered.length : count,
    ]
  }
  ...
  listAndCountTranslationSettings = async (
    filters: FilterableTranslationSettingsProps = {},
    config?: FindConfig<TranslationTypes.TranslationSettingsDTO>,
    sharedContext: Context = {}
  ...
  retrieveTranslationSettings = async (
    id: string,
    config?: FindConfig<TranslationTypes.TranslationSettingsDTO>,
    sharedContext: Context = {}
  ): Promise<TranslationTypes.TranslationSettingsDTO> => {
  ...
  deleteTranslationSettings = async (
    input: string | object | string[] | object[],
    sharedContext: Context = {}
  ): Promise<void> => {
    const ids = normalizeTranslationSettingsDeleteInput(input)

    if (!ids.length) {
      return
    }

    await this.settingsService_.delete(ids, sharedContext)
  }
  ...
  async getStatistics(
    ...
    const translations = await this.translationService_.list(
      {
        reference: entityTypes,
        locale_code: normalizedLocales,
      },
      {
        select: ["reference", "locale_code", "translated_field_count"],
      },
      sharedContext
    )
    const translatedFieldCounts = new Map<string, number>()

    for (const translation of translations) {
      const key = `${translation.reference}:${translation.locale_code}`
      translatedFieldCounts.set(
        key,
        (translatedFieldCounts.get(key) ?? 0) +
          Number(translation.translated_field_count ?? 0)
      )
    }
    ...
    for (const [key, translatedCount] of translatedFieldCounts) {
      const [entityType, localeCode] = key.split(":")
      if (
        !entityType ||
        !localeCode ||
        !result[entityType]?.by_locale[localeCode]
      ) {
        continue
      }
...
function filterTranslationsByQuery(
  ...
  return translations.filter((translation) =>
    JSON.stringify(translation.translations ?? {})
      .toLowerCase()
...
function withoutPagination(
  ...
  const { skip: _skip, take: _take, ...rest } = config
  return rest
}
...
function applyPagination(
  ...
  const end = config.take == null ? undefined : start + config.take

  return translations.slice(start, end)
}

function normalizeTranslationSettingsDeleteInput(
  input: string | object | string[] | object[]
): string[] {
  ...
  if (!Array.isArray(input)) {
    throw new MedusaError(
      MedusaErrorTypes.INVALID_DATA,
      "deleteTranslationSettings expects translation setting IDs"
    )
  }

  const inputArray: readonly unknown[] = input
  const ids = inputArray.filter(
    (item): item is string => typeof item === "string"
  )

  if (ids.length !== inputArray.length) {
    throw new MedusaError(
      MedusaErrorTypes.INVALID_DATA,
      "deleteTranslationSettings expects translation setting IDs"
    )
