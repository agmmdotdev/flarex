...
import { randomJwtId, signJwt, verifyJwt, type JwtOptions } from "../utils/jwt"
  ...
  implements UserTypes.IUserModuleService
{
  ...
  async validateInviteToken(
    ...
    const decoded = await verifyJwt(token, {
      secret:
        typeof this.config.jwtPublicKey === "string"
          ? this.config.jwtPublicKey
          : this.config.jwtSecret,
      jwtOptions: options,
    })

    const inviteId = decoded.payload.id
    if (typeof inviteId !== "string") {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "The invite token payload is invalid"
      )
    }
  ...
  async refreshInviteTokens_(
    ...
    const updates = await Promise.all(invites.map(async (invite) => {
      return {
        id: invite.id,
        expires_at: expiresAt,
        token: await this.generateToken({ id: invite.id, email: invite.email }),
      }
    }))
  ...
  private async createInvites_(
    ...
    const toCreate = await Promise.all(data.map(async (invite) => {
      const id = generateEntityId((invite as { id?: string }).id, "invite")
      return {
        ...invite,
        id,
        expires_at: expiresAt,
        token: await this.generateToken({ id, email: invite.email }),
      }
    }))
  ...
  private async generateToken(data: Record<string, unknown>): Promise<string> {
    const jwtId = this.config.jwtOptions.jwtid ?? randomJwtId()
