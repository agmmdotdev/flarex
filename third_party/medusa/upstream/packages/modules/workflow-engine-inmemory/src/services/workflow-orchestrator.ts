...
import {
  WorkflowScheduler,
} from "@medusajs/orchestration/workflow/scheduler"
import type {
  ContainerLike,
  Context,
  Logger,
...
import { createPortableId } from "@medusajs/utils/common/create-portable-id"
import type { FlowRunOptions } from "@medusajs/workflows-sdk/helper/type"
import { MedusaWorkflow } from "@medusajs/workflows-sdk/medusa-workflow"
import { resolveValue } from "@medusajs/workflows-sdk/composer"
import type { ReturnWorkflow } from "@medusajs/workflows-sdk/composer"
import { WorkflowOrchestratorCancelOptions } from "../types"
import {
  InMemoryDistributedTransactionStorage,
  type WorkflowDelayedActionRecoveryResult,
  type WorkflowScheduleRecoveryResult,
} from "../utils"
...
export class WorkflowOrchestratorService {
  ...
  async recoverDueSchedules(
    ...
    return await this.inMemoryDistributedTransactionStorage_.recoverDueSchedules(
      now
    )
  }
  ...
  async recoverDueDelayedActions(
    ...
    return await this.inMemoryDistributedTransactionStorage_.recoverDueDelayedActions(
      now
    )
  }

  async clearExpiredExecutions(): Promise<void> {
    await this.inMemoryDistributedTransactionStorage_.clearExpiredExecutions()
  }
  ...
  async run<T = unknown>(
    ...
    throwOnError ??= true
    context ??= {}
    context.transactionId = transactionId ?? createPortableId("auto")
  ...
  async cancel(
    ...
    const workflowContainer = (container ?? this.container_) as MedusaContainer
    ...
    })) as Awaited<ReturnType<typeof exportedWorkflow.cancel>> & {
      thrownError?: unknown
    }
  ...
  private notify(options: NotifyOptions) {
    ...
    queueMicrotask(() => {
      void this.processSubscriberNotifications(options)
    })
  }
